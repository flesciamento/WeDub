...
function MinutaggioCliccatoNellaClip(ELTCliccato, X) {
...
function DividiClip_puntatorerilasciatoFuori(e) {
    const ELT = ELTCliccato, datiAudio = DatiAudioRegistrato[ELT.dataset.RiferimentoRegistrazione], MinutaggioPrimoClick = MinutaggioCliccatoNellaClip(ELT, +ELT.dataset.posizionePuntatoreInizioTaglio), MinutaggioRilascio = ((e.clientX > +ELT.dataset.posizionePuntatoreInizioTaglio) ? +datiAudio.taglioFinale + 1 : 0);
    ElaboraDivisioneClipRichiestaConPuntatore(ELT.dataset.RiferimentoRegistrazione, MinutaggioPrimoClick, MinutaggioRilascio);
}

function DividiClip_puntatorerilasciato(e) {
    e.stopPropagation();
    const ELT = e.currentTarget, MinutaggioPrimoClick = MinutaggioCliccatoNellaClip(ELT, +ELT.dataset.posizionePuntatoreInizioTaglio), MinutaggioRilascio = MinutaggioCliccatoNellaClip(ELT, e.clientX);
    ElaboraDivisioneClipRichiestaConPuntatore(ELT.dataset.RiferimentoRegistrazione, MinutaggioPrimoClick, MinutaggioRilascio);
}

function ElaboraDivisioneClipRichiestaConPuntatore(Numero, MinutaggioPrimoClick, MinutaggioRilascio) {
    const TrascinatoVersoDestra = MinutaggioPrimoClick < MinutaggioRilascio;
    document.body.removeEventListener('mouseup', DividiClip_puntatorerilasciatoFuori); document.getElementById('ELTReg' + Numero).removeEventListener('mouseup', DividiClip_puntatorerilasciato);
    DisabilitaELT_tranne(Numero, false);
    DividiClip(Numero, (TrascinatoVersoDestra? MinutaggioPrimoClick : MinutaggioRilascio), (TrascinatoVersoDestra? MinutaggioRilascio : MinutaggioPrimoClick));
    CambiaTool(toolStandard);
}

function DividiClip(Numero, secondiInizioTaglio, secondiFineTaglio) {
    Messaggio(Numero + " " + secondiInizioTaglio + " " + secondiFineTaglio);
    DisabilitaSchermata();
            ...
            function(e) {
                ...
                if ((Righello.dataset.DisattivaClick == "no") || ((ELTDaSpostare == ELTCliccato) && (StrumentoMouse == toolStandard))) { // Attiva il trascinamento col mouse se il righello è attivo oppure in ogni caso se l'ELT era stato in precedenza cliccato.
                    switch(StrumentoMouse) {
                        ...
                        case toolDividiClip:
                            const MinutaggioSelezionatoClip = MinutaggioCliccatoNellaClip(ELT, e.clientX);
                            if ((datiAudio.taglioIniziale < MinutaggioSelezionatoClip) && (MinutaggioSelezionatoClip < datiAudio.taglioFinale)) {
                                ...
                                DisabilitaELT_tranne(NumeroAudio, true);
                                ELT.addEventListener('mouseup', DividiClip_puntatorerilasciato);
                                document.body.addEventListener('mouseup', DividiClip_puntatorerilasciatoFuori);
                            }
                        break;
...
function DisabilitaELT_tranne(Numero, Disabilita) {
    const totAudio = DatiAudioRegistrato.length, StylePointerEvents = (Disabilita? "none" : "auto"), StyleOpacity = (1 * !Disabilita);
    for (let I = 0; I < totAudio; I++) {
        if (I == Numero) {continue;}
        document.getElementById('ELTReg' + I).iStyle({pointerEvents: StylePointerEvents, opacity: StyleOpacity});
    }
}
