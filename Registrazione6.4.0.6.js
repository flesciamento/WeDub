...
function SegnaVariazioneAltezzaTracce(e) {
    ...
    NuovaAltezzaTracce = (+AltezzaIndicata * CondizioneSogliaMinima * CondizioneSogliaMassima) + (+AltezzaTracceMinima * !CondizioneSogliaMinima) + (+AltezzaTracceMassima * !CondizioneSogliaMassima);
    AltezzaTracce = NuovaAltezzaTracce - ComandiPlayer.offsetHeight;

    document.getElementById('indicatoreVariazioneAltezzaTracce').style.top = ((+NuovaAltezzaTracce) + (+ComandiPlayer.offsetHeight)) + "px";
}
            ...
            if (RuoliDaAssegnare_NumeroTraccia !== false) {
                ...
                if (OrientamentoSchermoOrizzontale()) {
                    if (NumeroTotaleTracce == 1) {
                        ...
                        if (!SonoCreatoreProgetto && !SessioneOspite) {AltezzaTracce = window.innerHeight - NomiTracce.children[0].offsetHeight - 200; if (AltezzaTracce > ContenitoreVideoGuida.offsetHeight) {VariaAltezzaTracce();}}
                    }
                }
            }
