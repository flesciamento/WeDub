...
function GestisciImmissioneTesto(e) {
    ...
    function MandaACapoEAttivaSceltaPersonaggio() {
        nuovatextarea = T;
        RigaCompleta = NodoAttuale.textContent;
        NodoAttuale.textContent = RigaCompleta.slice(0, PosizioneCursore);
        ElementoAttuale.parentElement.insertAdjacentElement('afterend', br = document.createElement('br'));
...
/*		case "[": T.innerHTML = T.innerHTML.slice(0, PosizioneCursore) + VisualizzaMinutaggioAttuale() + "] " + T.innerHTML.slice(PosizioneCursore); T.selectionEnd = PosizioneCursore; setTimeout(() => {T.selectionStart = T.selectionEnd = T.innerHTML.length;}, 100); return 2;
		...
		case "Enter":
            ...
            const DatiTestoAttuale = TrovaDatiCopioneID(UltimaTextareaUtilizzata.dataset.numid), DatiTestoSuccessivo = DatiCopione[DatiCopione.indexOf(DatiTestoAttuale) + 1];
                ...
                if (+Minutaggio <= ((+T.dataset.minutaggio) + (+SecondiRitornoIndietro))) {
                    ...
                    AJAX("TrascrizioneVideo_AggiungiNuovoMinutaggio.php", "N=" + encodeURIComponent(N) + "&Minutaggio=" + encodeURIComponent(Minutaggio),
                        (Dati) => {
                            ...
                            PosizionaCursore(document.getElementById('RifCopione' + Dati.NumID).children[0].children[0].childNodes[1], 0);
                            MandaA_AttivaSceltaPersonaggio();
...
async function ScriviPersonaggio(T, Personaggio) {
    ...
    const NodoCursore = UltimaPosizioneCursore.nodo, span = NodoCursore.parentElement;

    if (Personaggio) {
        span.innerHTML = `<b style='background-color: lightgreen;'>${Personaggio}</b><span>: </span><span contenteditable='true' onkeydown='GestisciImmissioneTesto(event);'>${NodoCursore.textContent || " "}</span>`;
        /* span.textContent = `${Personaggio}: ${NodoCursore.textContent.trim()}`; 
           PosizionaCursore(span.childNodes[0], +Personaggio.length + 2); */
        span.contentEditable = false; span.onkeydown = "";
        PosizionaCursore(span.children[2].childNodes[0], 0);

    } else {
        PosizionaCursore(span.childNodes[0], 0);
    }
    ...
    span.focus();
    await pausa(300);
    if (opzRiprendiVideoSecondi.checked) {VideoGuidaPlay();}
    await pausa(100);
...
function MandaAdAggiornaTestoGuida() {
    ...
    if (UltimaTextareaUtilizzata) {
        ...
        DatiTestoSuccessivo = DatiCopione[DatiCopione.indexOf(DatiTestoAttuale) + 1];
...
function AggiornaTestoGuida_slow() {
    ...
    AggiornaTestoGuida_slow.tmr = setTimeout(() => {FunzioniCopione.Visualizza(); FunzioniCopione.AggiornaTestoGuida(); if (RiproduzioneInCorso) {FunzioniCopione.AttivaTestoGuida();}}, 2000);
}
