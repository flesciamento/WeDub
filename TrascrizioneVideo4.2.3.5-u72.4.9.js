...
function GestisciImmissioneTesto(e) {
	const ElementoAttuale = e.currentTarget, T = ElementoAttuale.parentElement.parentElement, InfoCursore = PosizioneDelCursore(), PosizioneCursore = InfoCursore.p;
    var nuovatextarea;
    ...
    if (Object.keys(TastiIndicazioni).includes(e.key)) {
        ...
        if (e.key == "F1") {
            ...
            PosizionaCursore(ElementoAttuale, 0);
            nuovatextarea = T;
    ...
    TestoPrec = ElementoAttuale.textContent.trim();
...
/*		case "[": T.innerHTML = T.innerHTML.slice(0, PosizioneCursore) + VisualizzaMinutaggioAttuale() + "] " + T.innerHTML.slice(PosizioneCursore); T.selectionEnd = PosizioneCursore; setTimeout(() => {T.selectionStart = T.selectionEnd = T.innerHTML.length;}, 100); return 2;
        ...
        case "ArrowUp":   ((NumeroNodo(InfoCursore.T, ElementoAttuale) == 0) && (precedentetextarea = TrovaTextareaVicina(T, -1)) && (precedentetextarea.focus())); break;
        case "ArrowDown": ((NumeroNodo(InfoCursore.T, ElementoAttuale) == (InfoCursore.T.childNodes.length - 1)) && (textareasuccessiva = TrovaTextareaVicina(T, 1)) && (PosizionaCursore(textareasuccessiva.childNodes[0], 0)) && (textareasuccessiva.focus())); break;
		case "Enter":
            ...
            function MandaACapoEAttivaSceltaPersonaggio() {
                nuovatextarea = T;
                RigaCompleta = ElementoAttuale.textContent;
                const NodoBattuta = ElementoAttuale.parentElement, NumNodoBattuta = NumeroNodo(T, NodoBattuta);
                ElementoAttuale.textContent = RigaCompleta.slice(0, PosizioneCursore);
                ElementoAttuale.insertAdjacenteElement('afterend', br = document.createElement('br'));
                /* NodoBattuta.after(document.createElement('br')); */
                const s = document.createElement('span'); s.contentEditable = true; s.onkeydown = GestisciImmissioneTesto; s.textContent = RigaCompleta.slice(PosizioneCursore);
                /* T.childNodes[+NumNodoBattuta + 2].append(s); */
                br.insertAdjacentElement('afterend', s);
                PosizionaCursore(s.childNodes[0], 0);

                /* T.childNodes[NumNodoBattuta + 1].insertAdjacentHTML('afterend', `<span contenteditable='true' onkeydown='GestisciImmissioneTesto(event);'>${RigaCompleta.slice(PosizioneCursore)}</span>`); */
                /* PosizionaCursore(T.childNodes[+NumNodoBattuta + 2], 0); */
                MandaA_AttivaSceltaPersonaggio();
            }

            if (textareasuccessiva = TrovaTextareaVicina(T, 1)) {
                if (Minutaggio < textareasuccessiva.dataset.minutaggio) {
                    MandaACapoEAttivaSceltaPersonaggio();
                } else {
                    nuovatextarea = textareasuccessiva;
                    nuovatextarea.innerHTML = " <br>" + nuovatextarea.innerHTML;
                    PosizionaCursore(nuovatextarea.childNodes[0], 0);
                    MandaA_AttivaSceltaPersonaggio();
                }
            } else {
    ...
    setTimeout(() => {if (TestoPrec != ElementoAttuale.textContent.trim()) {RiprendiVideoAutomaticamente(T);}}, 100);
}

async function AttivaSceltaPersonaggio(T, FunzioneAllaSceltaPersonaggio) {
    await pausa(100);
    /* UltimaPosizioneCursore = PosizioneDelCursore(); UltimaPosizioneCursore.numeroNodo = NumeroNodoCursore(UltimaPosizioneCursore.T, UltimaPosizioneCursore.nodo); console.log("UltimaPosizioneCursore:", UltimaPosizioneCursore); */
...
async function ScriviPersonaggio(T, Personaggio) {
    ...
    var NuovaPosizioneCursore = UltimaPosizioneCursore.p, NodoCursore = UltimaPosizioneCursore.nodo;

    if (Personaggio) {
        const span = NodoCursore.parentElement;
        span.innerHTML = `<b style='background-color: lightgreen;'>${Personaggio}</b>: ${NodoCursore.textContent}`;
        NuovaPosizioneCursore += (+Personaggio.length) + 2;
        span.focus();
        PosizionaCursore(span.childNodes[1], 2);
    }
...
function SalvaCopione(Textarea, AggiornaTesto = false, PosCursore = {}) {
    /* var Testo = "";
    ...
    /* if (AggiornaTesto) {
        const ID_ElementoContenitore = Textarea.id, NumNodoCursore = ;
