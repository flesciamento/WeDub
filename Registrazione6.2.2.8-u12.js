...
function OpacitaRighello(LivelloOpacita) {
	const divOpacitaTraccia = document.getElementsByName('divOpacitaTraccia'), totDivOpacitatraccia = divOpacitaTraccia.length;
    for (let I = 0; I < totDivOpacitatraccia; I++) {
        divOpacitaTraccia[I].style.backgroundColor = "rgba(255, 255, 255, " + (1 - LivelloOpacita).toString() + ")";
    }
}
        ...
        switch(StrumentoMouse) {
            case toolStandard:
                ...
                [document.getElementById(ID_Opzioni + 'TabellaOpzioniRigaVolume'), document.getElementById(ID_Opzioni + 'tdOpzioneEffetti')].forEach((opzionemodificamultipla) => {
                    ...
                    opzionemodificamultipla.onclick = (e) => {e.currentTarget.style.opacity = 1;};
                });

                EvidenziaELTSelezionato(ELTCliccato, !ELTSelezionatoInPrecedenza || SingoloELTDaModificare);
...
function DisattivaSpostamentoManualeClip() {
    ...
    if (ELTDaSpostare) {VisualizzazioneGraficaTaglioClip(ELTDaSpostare.dataset.RiferimentoRegistrazione); document.getElementById(ELTDaSpostare.id + 'StratoColore').ontouchstart = "";}
}
...
function OpzioniClip(Numero, Apri, SalvaAllaChiusura) {
    const datiAudio = DatiAudioRegistrato[Numero], IDELT = 'ELTReg' + datiAudio.numero, ELT = document.getElementById(IDELT);
