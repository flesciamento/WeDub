...
function DisabilitaElementiGenerali(Disabilita) {
    ...
    if (!ModalitaStreaming) {document.getElementById('fieldsetContenitoreStrumenti').abilita(!Disabilita);}
...
function GestioneEventoCommentoELTCliccato(e) {
    if ((e.button != 0) || (StrumentoMouse != toolStandard)) {return;}

    GestioneEventoELTCliccato({currentTarget: document.getElementById('ELTReg' + e.currentTarget.dataset.RiferimentoRegistrazione + 'StratoColore'), clientX: e.clientX, button: 0});
}
...
function RiposizionamentoAutomaticoClipSovrapposte(ELTConsiderato) {
    if (ELTDaSpostare == false) {
        const tolleranza = 1; // Secondi di tolleranza per la sovrapposizione della parte arancione delle clip.
...
function VisualizzaCommentiELT(Numero) {
    ...
    if ((CommentiDoppiatore != "") || (CommentiCreatoreProgetto != "")) {
        ...
        const commentoELT = CreaElementoSeInesistente('div', id_commentoELT, id_ELT, '', 'CommentoELT'); commentoELT.style.left = document.getElementById(id_ELT + 'StratoColore').style.left; commentoELT.dataset.RiferimentoRegistrazione = Numero; commentoELT.onclick = GestioneEventoCommentoELTCliccato;
