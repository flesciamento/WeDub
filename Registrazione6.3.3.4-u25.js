...
function GeneraBufferCI(datiAudio, buffer, FunzioneAlTermine = () => {}) {
    ...
    const b = buffer, SampleRate = b.sampleRate, numCanali = b.numberOfChannels, tempomixBufferPrec = 1;
    ...
    if (TrovaSpezzoneVicino(datiAudio, 1)) {
        ...
        for(c = 0; c < numCanali; c++) {
            ...
            for(s = datiAudio.buffer.duration - tempomixBufferPrec; s < SampleRate; s++) {
                bufferCanale[s] *= (SampleRate - s) / SampleRate;
            }
    ...
    console.log("Spezzone CI", datiAudio.numspezzoneCI, "minutaggio", datiAudio.MinutaggioRegistrazione, "durata", datiAudio.Durata);
