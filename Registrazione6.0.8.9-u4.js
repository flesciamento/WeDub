...
function MinutaggioCliccatoNellaClip(ELTCliccato, X) {
    return (X + window.scrollX - ContenitoreRighello.offsetLeft - ELTCliccato.offsetLeft) / ELTCliccato.clientWidth * DatiAudioRegistrato[NumeroAudio].Durata;
}

function DividiClip_puntatorerilasciato(e) {
    const ELT = e.currentTarget, MinutaggioPrimoClick = MinutaggioCliccatoNellaClip(ELT, ELT.dataset.posizionePuntatoreInizioTaglio), MinutaggioRilascio = MinutaggioCliccatoNellaClip(ELT, e.clientX), TrascinatoVersoDestra = MinutaggioPrimoClick < MinutaggioRilascio;
    DividiClip(ELT.dataset.RiferimentoRegistrazione, (TrascinatoVersoDestra? MinutaggioPrimoClick : MinutaggioRilascio), (TrascinatoVersoDestra? MinutaggioRilascio : MinutaggioPrimoClick));
    CambiaTool(toolStandard);
}
