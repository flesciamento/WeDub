...
function CreaNuovaTextarea(NumID, Minutaggio) {
    ...
    const tr = CreaElemento('tr', 'RigoBloccoTesto' + NumRigo, "ContenitoreTesto");
            td = CreaElemento('td', 'tdInfoMinutaggio' + NumRigo, tr.id); td.iStyle({width: "100px", backgroundColor: "white", textAlign: "center", verticalAlign: "top", fontSize: "50%"});
                    /** Minutaggio **/
                    ...
                    M = CreaElemento('span', 'MinutaggioMinuti' + NumRigo, td.id, MinutaggioAttuale.Minuti); M.iStyle({width: "fit-content", textAlign: "right"});
                    ...
                    fs = CreaElemento('span', 'freccias' + NumRigo, td.id); fsu.className = "btn btn-default btn-xs fa fa-arrow-up";   fsu.iStyle({fontSize: "50%", float: "right"}); fs.dataset.differenza = 1;  fs.dataset.numid = NumID; fs.onclick = AggiornaMinutaggio;
                    fg = CreaElemento('span', 'frecciag' + NumRigo, td.id); fsu.className = "btn btn-default btn-xs fa fa-arrow-down"; fsu.iStyle({fontSize: "50%", float: "right"}); fg.dataset.differenza = -1; fg.dataset.numid = NumID; fg.onclick = AggiornaMinutaggio;
