...
function PrecaricaClip(DalMinutaggio, FinoAlMinutaggio, FunzioneAlTermine = false, AltreOpzioni = {FunzioneAlTermineSingoloPrecaricamento: () => {}, SoloBuffer: false}) {
    const totAudio = DatiAudioRegistrato.length, SecondiPrecaricamentoMax = 300;
    ...
    if (totClipDaPrecaricare == 0) {
        if (FunzioneAlTerminePrecaricamento) {
            ...
            setTimeout(() => {MinutaggioPrecaricamentoClip = FinoAlMinutaggio; PrecaricaClipSuccessiva(); console.log("Timeout avviato.", MinutaggioPrecaricamentoClip);}, (+FinoAlMinutaggio - VideoGuidaMinutaggioCorrente() - (SecondiPrecaricamentoMax / 2)) * 1000);
            console.log("Settato Timeout fra", (SecondiPrecaricamentoMax / 2), "sec. Precaricamento clip dal secondo", +FinoAlMinutaggio - VideoGuidaMinutaggioCorrente() - (SecondiPrecaricamentoMax / 2));
        }
    }
}
