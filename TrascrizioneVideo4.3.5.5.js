...
/*		case "(": NodoAttuale.textContent = NodoAttuale.textContent.slice(0, PosizioneCursore) + ")" + NodoAttuale.textContent.slice(PosizioneCursore); ; return 2;
    ...
    setTimeout(() => {if (TestoDaComparare(TrovaDatiCopioneID(T.dataset.numid).testo) != TestoDaComparare(T.textContent)) {RiprendiVideoAutomaticamente(T);} else {((TestoPrec != T.textContent) && SalvaCopione_slow(T));}}, 100);
}
...
function SalvaCopione(divContenitore, AggiornaTesto = false, NumeroSpanBattutaCursore = false, NumeroSpanCursore = 0) {
    const Testo = AcquisisciTestoCopione(divContenitore), NumID = divContenitore.dataset.numid;
...
function CreaMenuOpzioniAvanzate() {
    CreaMenuADiscesa([{DoveInserirlo: 'PannelloOpzioni', ID_Menu: 'OpzioniAvanzate',
        Elementi: [
            {dato: "", stringa: strOpzioniAvanzate, predefinito: true, nascosto: true},
            {dato: "esporta", stringa: "<span class='fa fa-file-text-o'></span> " + strEsportaSRT},
            {dato: "caricanuovo", stringa: "<span class='fa fa-upload'></span> " + strCaricaFileSottotitoliInSostituzione}
        ], FunzioneAlClick: (dato) => {
            EliminaElemento(document.getElementById('SuperContenitoreCaricaSottotitoli'));
            switch(dato) {
                case "esporta": EsportaSRT(); break;
                case "caricanuovo": FormSottotitoli('PannelloOpzioni', '', false); break;
            }
        }
    }]);
}
...
function EsportaSRT() {
    const totDatiCopione = DatiCopione.length;
    let strFileSRT = "";
