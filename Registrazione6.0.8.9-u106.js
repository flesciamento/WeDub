 ...
 * @param {Boolean} CropOnda    (facoltativo) true: visualizza solo l'onda sonora relativa alla parte tagliata, false: visualizza tutta l'onda sonora. Se il parametro non viene passato, si comporta in base al parametro globale impostato. */
function VisualizzazioneGraficaTaglioClip(Numero, CropOnda = !VisualizzazioneGraficaTaglioClip.OndaSonoraCompleta) {
...
function CreaElementoLineaTemporale(ID, DoveInserirlo, PartenzaRegistrazione, LunghezzaRegistrazione, Stringa, RiferimentoRegistrazione, Posizione, Altezza, imgOndaSonora, DaVisualizzare) {
	...
	if ( ( (DatiAudioRegistrato[RiferimentoRegistrazione].ID_Utente == ID_Utente) && (SolaVisione == false) ) || (SonoCreatoreProgetto)) {
        ...
        ELT.addEventListener('mouseleave', (e) => {clearTimeout(tmrRiposizionamentoAutomaticoELT); VisualizzazioneGraficaTaglioClip(e.currentTarget.dataset.RiferimentoRegistrazione);});
...
function AnteprimaSelezioneClip(ELTConsiderato, ClientX, FunzioneTouchEnd = () => {}) {
    ...
    ELT.style.zIndex = 500000000; DisabilitaELT_tranne(ELT, true); VisualizzazioneGraficaTaglioClip.OndaSonoraCompleta = true;
    ...
    AnteprimaSelezioneClip.Elimina = () => {EliminaElemento(div); VisualizzazioneGraficaTaglioClip(ELT.dataset.RiferimentoRegistrazione, VisualizzazioneGraficaTaglioClip.OndaSonoraCompleta = false); ELT.style.zIndex = ""; DisabilitaELT_tranne(ELT, false); clearTimeout(AnteprimaSelezioneClip.tmr);};
}
