...
function RiproduciClipInSync(MinutaggioVideo) {
    ...
    ClipDaRiprodurre.forEach((I) => {
        ...
        datiAudio.alPlay.forEach((DatiEventoAlPlay, N) => {
            ...
            const Num = N, LatenzaSecondi = DatiEventoAlPlay.latenzaEventoAlPlay.secondi, RiduciSeClipNelMinutaggio = DatiEventoAlPlay.latenzaEventoAlPlay.riduciSeClipNelMinutaggio;
            datiAudio.tmrEventoAlPlay[N] = setTimeout(() => {datiAudio.alPlay[Num].FunzioneAlPlay(datiAudio); console.log("AttivataFunzioneAlPlay", Num, datiAudio.alPlay[Num].FunzioneAlPlay);}, ((+SecondiPartenzaAudio) + (+LatenzaSecondi) - (InizioClip * ClipNelMinutaggioAttuale * RiduciSeClipNelMinutaggio)) * 1000);
...
function GeneraBufferCI(datiAudio, buffer, FunzioneAlTermine = () => {}) {
    ...
    function TrovaSpezzoneSuccessivo(datiAudio, distanza) {
        return AudioBufferColonnaInternazionale[AudioBufferColonnaInternazionale.indexOf(datiAudio) + distanza];
    }

    function TrovaSpezzoneVicino(datiAudio, distanza) {
        const datiAudioVicino = TrovaSpezzoneSuccessivo(datiAudio, distanza);
        return ((datiAudioVicino && (+datiAudioVicino.minutaggiodefault == (+datiAudio.minutaggiodefault + (75 * distanza)))) ? datiAudioVicino : false);
    }
    ...
    function TerminaBufferCI() {
        ...
        if (TrovaSpezzoneVicino(datiAudio, 1)) {
            ...
            if (Math.floor((+datiAudio.MinutaggioRegistrazione) + (+datiAudio.Durata)) < Math.floor(totDurataVideoGuida - 1)) {
                ...
                datiAudio.alPlay.push({FunzioneAlPlay: CI_AttivaAudioOriginale, latenzaEventoAlPlay: {secondi: datiAudio.Durata - tempomixCIeOriginale - 0.5, riduciSeClipNelMinutaggio: true}});
                ...
                if (datiAudioSuccessivo = TrovaSpezzoneSuccessivo(datiAudio, 1)) {CI_DisattivaAudioOriginaleAlPlay(datiAudioSuccessivo);}
            }
        }
...
function CI_DisattivaAudioOriginaleAlPlay(datiAudio) {
    datiAudio.alPlay.push({FunzioneAlPlay: CI_DisattivaAudioOriginale, latenzaEventoAlPlay: {secondi: 0, riduciSeClipNelMinutaggio: false}});
    console.log("Attivato evento alPlay per CI_DisattivaAudioOriginale nella clip", datiAudio);
}
...
function DeterminaVolumeVideoGuidaPerCI() {
    if (ColonnaInternazionaleAttivata) {
        ...
        if (PosizioneConAudioOriginale) {
            ...
            if (PosizioneSuccessiva = DatiCI[+DatiCI.indexOf(PosizioneConAudioOriginale) + 1]) {CI_DisattivaAudioOriginaleAlPlay(DatiAudioRegistrato_Registrazione[PosizioneSuccessiva.CI[0]][0]);}
        } else {
...
function StoppaClipAudio(datiAudio) {
    if (datiAudio.avviato) {
        ...
        datiAudio.alPlay.forEach((evento, N) => {clearTimeout(datiAudio.tmrEventoAlPlay[N]);});
        datiAudio.alPlay = [];
        console.log("Eliminati eventi alPlay per la clip", datiAudio);
