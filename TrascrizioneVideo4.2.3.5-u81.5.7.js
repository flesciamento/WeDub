...
/*		case "(": NodoAttuale.textContent = NodoAttuale.textContent.slice(0, PosizioneCursore) + ")" + NodoAttuale.textContent.slice(PosizioneCursore); ; return 2;
        ...
        case "Backspace":
            if (T.innerText.trim() == "") {
                AJAX("TrascrizioneVideo_EliminaBloccoTesto.php", "NumID=" + encodeURIComponent(T.dataset.numid) + "&N=" + encodeURIComponent(N), () => {const NumID = T.dataset.numid; CaricaCopione(() => {FunzioniCopione.AggiornaTestoGuida(); console.log("Eliminato blocco testo", NumID); ((div = document.getElementById('RifCopione' + (NumID - 1))) && (div.lastElementChild.querySelector("[contenteditable='true']").focus()) ); console.log(div)});}, "", "", true);
                return;
            } else {
