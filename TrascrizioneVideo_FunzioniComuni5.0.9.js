...
CreaElemento('style', 'styleTestoGuida', document.body.id,
    `.TestoGuida-evidenziato {background-color: rgba(200, 255, 200); border-radius: 10px;}
     #ManigliaSposta_TestoGuida {width: fit-content; margin: 0px auto; cursor: move; font-size: 1.5vh; z-index: 1;}
     #LatoSx_TestoGuida {position: absolute; top: 0px;    left: 0px; height: 100%; width: 3vh; cursor: col-resize;}
     #LatoDx_TestoGuida {position: absolute; top: 0px;   right: 0px; height: 100%; width: 3vh; cursor: col-resize;}
     #Tetto_TestoGuida  {position: absolute; top: 0px;    left: 0px; height: 3vh; width: 100%; cursor: row-resize;}
     #Base_TestoGuida   {position: absolute; bottom: 0px; left: 0px; height: 3vh; width: 100%; cursor: row-resize;}
     #TestoGuida {position: relative; top: 1.5vh; height: 100%; overflow-y: scroll;}`
);
const ContenitoreTestoGuida = CreaElemento('div', 'ContenitoreTestoGuida', 'ContenitoreVideoGuida'); ContenitoreTestoGuida.iStyle({position: "fixed", top: "calc(" + ContenitoreVideoGuida.offsetHeight + "px - 20%)", left: "30%", right: "30%", bottom: "10%", overflow: "hidden", padding: "5px", backgroundColor: "rgba(255, 255, 255, 0.8)", borderRadius: "10pt", fontSize: "1.5vh", lineHeight: "2vh", textAlign: "left", textShadow: "1px 1px 1px aliceblue", opacity: 0, visibility: "hidden", transition: "opacity 1s", userSelect: "none"});
...
function RidimensionaTestoGuida(e) {
    ...
    ContenitoreTestoGuida.iStyle({[ContenitoreTestoGuida.dataset.styledamodificare]: Math.abs(ContenitoreTestoGuida.dataset.misuradipartenza - Coordinata) + "px"})
}
