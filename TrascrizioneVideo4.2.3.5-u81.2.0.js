...
/*		case "(": NodoAttuale.textContent = NodoAttuale.textContent.slice(0, PosizioneCursore) + ")" + NodoAttuale.textContent.slice(PosizioneCursore); ; return 2;
        ...
        case "ArrowUp":   if ((PosizioneCursore < 2) && (campoprecedente = TrovaCampoTestoVicino(ElementoAttuale, -1))) {await pausa(100); campoprecedente.focus(); console.log(campoprecedente);} return;
        case "ArrowDown": ((PosizioneCursore > NodoAttuale.textContent.length - 5) && (camposuccessivo = TrovaCampoTestoVicino(ElementoAttuale, 1)) && (camposuccessivo.focus())); return;
...
function ModificaPersonaggio(T, Personaggio) {
    ...
    SalvaCopione(divContenitore, true, Array.from(divContenitore.children).indexOf(spanBattuta), Array.from(spanBattuta.children).indexOf(T.nextElementSibling.nextElementSibling));
}
