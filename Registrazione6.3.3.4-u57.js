...
function GeneraBufferCI(datiAudio, buffer, FunzioneAlTermine = () => {}) {
    ...
    if ((datiAudio_SpezzonePrecedente = TrovaSpezzoneVicino(datiAudio, -1)) && (VideoGuidaMinutaggioCorrente() < datiAudio.minutaggiodefault)) {
        async function AggiungiPezzoMix() {
            if (!datiAudio_SpezzonePrecedente.buffer) {console.log("AggiungiPezzoMix()", "minutaggio corrente:", new MinutiESecondi(VideoGuidaMinutaggioCorrente()), "minutaggio spezzoneCI:", datiAudio.minutaggiodefault, "Attesa buffer spezzone", datiAudio_SpezzonePrecedente.numspezzoneCI); await pausa(500); AggiungiPezzoMix(); return;} // Attende che abbia caricato il buffer dello spezzone precedente prima di continuare.
    ...
    function TerminaBufferCI() {
        if (TrovaSpezzoneVicino(datiAudio, 1)) {
            /* Fade-out ultimo pezzo (se ci sono altri spezzoni dopo) */
