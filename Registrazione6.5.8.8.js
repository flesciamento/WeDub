		...
		function (Dati) {
			...
			for (I = 0; I < totDati; I++) {
				...
				if (NumAudio >= totAudio) {
                    ...
                    if (document.getElementById(ID_Opzioni) || (Dati[I].DurataRegistrazione == 0) || (ModalitaStreaming)) {continue;}
...
function AJAXSalvaAudioAscoltato(datiAudio) {
    AJAX('SalvaRegistrazioneAscoltata.php', "N=" + datiAudio.NumeroUnivoco + "&A=" + (datiAudio.daVisualizzare? "1" : "0"), "", "", "", true);

    datiAudio.daVisualizzare = !datiAudio.daVisualizzare;
...
function CreaFinestraOpzioniClip(RiferimentoRegistrazione) {
    ...
    var NuovoAudioTrattato = false, NuoveClipCreate = [];
    ...
    const divcontenitorebody = CreaElemento('div', ID_Opzioni + 'ContenitoreOpzioniBody', ID_Opzioni); divcontenitorebody.className = "panel-body";
        var Tabella = CreaElemento('table', ID_Opzioni + 'TabellaOpzioni', divcontenitorebody.id), tr, td; Tabella.className = "TabellaOpzioni"; Tabella.style.width = "100%";
        ...
        if (SonoCreatoreProgetto) {
            const labelDaAscoltare = CreaElemento('label', ID_Opzioni + 'labelPulDaAscoltare', divcontenitorebody.id); labelDaAscoltare.className = "btn btn-default btn-xs"; labelDaAscoltare.iStyle({position: "absolute", marginLeft: "2px"}); labelDaAscoltare.setAttribute('title', strDaAscoltare_title);
                  const pulDaAscoltare = CreaElemento('input', ID_Opzioni + 'checkPulDaAscoltare', labelDaAscoltare.id); pulDaAscoltare.setAttribute('type', 'checkbox'); pulDaAscoltare.checked = datiAudio.daVisualizzare;
                        pulDaAscoltare.dataset.RiferimentoRegistrazione = RiferimentoRegistrazione;
                        pulDaAscoltare.onclick = (e) => {AJAXSalvaAudioAscoltato(DatiAudioRegistrato[e.currentTarget.dataset.RiferimentoRegistrazione]);};
