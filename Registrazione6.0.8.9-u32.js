...
function DividiClip_puntatorerilasciatoFuori(e) {
    const ELT = ELTCliccato, datiAudio = DatiAudioRegistrato[ELT.dataset.RiferimentoRegistrazione], MinutaggioPrimoClick = MinutaggioCliccatoNellaClip(+ELT.dataset.posizionePuntatoreInizioTaglio), MinutaggioRilascio = MinutaggioCliccatoNellaClip(e.clientX); /* ((e.clientX > +ELT.dataset.posizionePuntatoreInizioTaglio) ? +datiAudio.taglioFinale + 1 : 0); */
...
function ElaboraDivisioneClipRichiestaConPuntatore(Numero, MinutaggioPrimoClick, MinutaggioRilascio) {
    ...
    setTimeout(() => {/* DisabilitaELT_tranne(Numero, false); */ divVetro.style.display = "none";}, 500);
    CambiaTool(toolStandard);
}
                    ...
                    switch(StrumentoMouse) {
                        ...
                        case toolDividiClip:
                            ...
                            if ((datiAudio.taglioIniziale < MinutaggioSelezionatoClip) && (MinutaggioSelezionatoClip < datiAudio.taglioFinale)) {
                                ...
                                divVetro.style.display = "inline"; console.log(divVetro);
...
function DisabilitaELT_tranne(Numero, Disabilita) {
    const totAudio = DatiAudioRegistrato.length, StyleDisplay = (Disabilita? "none" : ""); //StylePointerEvents = (Disabilita? "none" : "auto"), StyleOpacity = (1 * !Disabilita), StyleTransition = (Disabilita? "all 0.5s" : ""), StyleVisibility = (Disabilita? "hidden" : "");
    for (let I = 0; I < totAudio; I++) {
        if (I == Numero) {continue;}
        document.getElementById('ELTReg' + I).iStyle({display: StyleDisplay});
    }
}
