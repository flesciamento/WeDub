...
async function Posizionati(MinutaggioNuovo, RiabilitaLaSchermata = true) {
    ...
    function RiattivaVideoGuida() {
        if ((Math.round(VideoGuidaMinutaggioCorrente())) != (Math.round(MinutaggioNuovo))) {setTimeout(RiattivaVideoGuida, 1000); return;}
    ...
    function RiabilitaLaSchermataSeRichiesto() {
        if (RiabilitaLaSchermata && (Righello.dataset.DisattivaClick == "no")) {DisabilitaComandiPerRiprodurre(false);}
    }

    function DisabilitaComandiPerRiprodurre(Disabilita) {
        pulPlay.disabled = Disabilita; PlayPausaCliccandoSulVideo(!Disabilita); DisabilitaElementiRegistrazione(Disabilita);
    }
    ...
    setTimeout(RiattivaVideoGuida, 500);
}
