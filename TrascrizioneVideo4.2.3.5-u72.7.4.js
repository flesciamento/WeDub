...
async function ScriviPersonaggio(T, Personaggio) {
    ...
    if (Personaggio) {
        span.innerHTML = `<b style='background-color: lightgreen;'>${Personaggio}</b><span>: </span><span>${NodoCursore.textContent}</span>`;
        /* span.textContent = `${Personaggio}: ${NodoCursore.textContent.trim()}`; 
           PosizionaCursore(span.childNodes[0], +Personaggio.length + 2); */
        PosizionaCursore(span.childNodes[2].childNodes[0], 0);
