...
function GestisciImmissioneTesto(e) {
    ...
    if (Object.keys(TastiIndicazioni).includes(e.key)) {
        ...
        if (e.key == "F1") {
            ...
            const StringaConsiderata = T.textContent.slice(0, T.selectionStart);
        ...
        T.textContent = T.textContent.slice(0, PosizioneCursore) + ((PosizioneCursore > 0) ? " " : "") + TestoIndicato.slice(0, TestoIndicato.indexOf('|')) + " " + T.textContent.slice(PosizioneCursore); T.selectionStart = PosizioneCursore + TestoIndicato.length + 1; T.selectionEnd = T.selectionStart;
        SalvaCopione(T);
        return;
    }
...
/*		case "[": T.textContent = T.textContent.slice(0, PosizioneCursore) + VisualizzaMinutaggioAttuale() + "] " + T.textContent.slice(PosizioneCursore); T.selectionEnd = PosizioneCursore; setTimeout(() => {T.selectionStart = T.selectionEnd = T.textContent.length;}, 100); return 2;
        ...
        case "ArrowDown": ((PosizioneCursore > (T.textContent.length - 5)) && (textareasuccessiva = TrovaTextareaVicina(T, 1)) && !(textareasuccessiva.selectionStart = textareasuccessiva.selectionEnd = 0) && (textareasuccessiva.focus())); break;
		case "Enter":
	...
	AJAX("TrascrizioneVideo_CaricaCopione.php", "N=" + encodeURIComponent(N),
        async function (Dati) {
                ...
                for (let I = 0; I < totBlocchiDiTesto; I++) {
                    ...
                    T.selectionStart = T.textContent.length; T.selectionEnd = T.selectionStart;
