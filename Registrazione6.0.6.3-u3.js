    ...
    const Marcatore = CreaElemento('div', 'Marcatore' + IDMarcatore, 'Traccia' + DatiDoppiatori[ID_UtenteMarcatore].numeroTraccia); Marcatore.className = 'Marcatore'; Marcatore.dataset.id = IDMarcatore;
          Marcatore.style.left = (Minutaggio / totDurataVideoGuida * 100) + "%";
            ...
            const Casella = CreaElemento('input', 'input' + Marcatore.id, Marcatore.id); Casella.value = Stringa; Casella.readOnly = !Modificabile; Casella.style.pointerEvents = (Modificabile? "auto" : "none");
                  Casella.onchange = (e) => {const C = e.currentTarget; if (C.value.trim() == "") {C.value = C.dataset.valoreprec;} Casella.size = Casella.value.length - 1; C.dataset.valoreprec = C.value;};
                  Casella.onblur = (e) => {
