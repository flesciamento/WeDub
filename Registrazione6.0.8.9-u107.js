...
function VisualizzazioneGraficaTaglioClip(Numero, OndaCompleta = VisualizzazioneGraficaTaglioClip.OndaSonoraCompleta) {
    ...
    if (StratoColore) {
        ...
        OndaSonora.style.clipPath = (OndaCompleta? "none" : "inset(0% " + StratoColore.style.right + " 0% " + StratoColore.style.left + ")");
    }
}
...
function DisabilitaELT_tranne(ELT, Disabilita) {
    const StratoColoreAltriELT = document.querySelectorAll(".StratoColoreELT:not(#" + ELT.id + "StratoColore)"), totStratoColoreAltriELT = StratoColoreAltriELT.length, stylePointerEvents = (Disabilita? "none" : ""), classELT = (Disabilita? "ELT transizione-morbida-no nohover" : "ELT transizione-morbida-si");
    
    for (let I = 0; I < totStratoColoreAltriELT; I++) {
        StratoColoreAltriELT[I].style.pointerEvents = stylePointerEvents;
        StratoColoreAltriELT[I].parentNode.parentNode.className = classELT;
    }
}
