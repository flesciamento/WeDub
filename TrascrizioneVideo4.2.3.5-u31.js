...
async function ScriviPersonaggio(T, Personaggio) {
    var NuovaPosizioneCursore = 0, NodoCursore = UltimaPosizioneCursore.nodo;
	
    VisualizzaBoxSceltaPersonaggio(false);
		
    if (Personaggio) {
        NodoCursore.textContent = NodoCursore.textContent.slice(0, UltimaPosizioneCursore.p) + Personaggio + ": " + NodoCursore.textContent.slice(UltimaPosizioneCursore.p);
        NuovaPosizioneCursore = (+UltimaPosizioneCursore.p) + (+Personaggio.length) + 2;
    }
