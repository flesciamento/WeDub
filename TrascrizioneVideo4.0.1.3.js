...
/* 		case "Control": if (e.location == 2) {RiprendiDaPocoPrima(6);} */
		...
		case "Enter":
            ...
            const Minutaggio = (VideoGuidaMinutaggioCorrente() | 0) - 2;
            var nuovatextarea;
            if (nuovatextarea = document.getElementById('t' + Minutaggio)) {nuovatextarea.focus(); return;}
...
function CreaNuovaTextarea(Minutaggio) {
    const NumRigo = (++CreaNuovaTextarea.num), MinutaggioMassimo = new MinutiESecondi(totDurataVideoGuida);
    const tr = CreaElemento('tr', 'tr' + NumRigo, "ContenitoreTesto");
            td = CreaElemento('td', 'tdInfoMinutaggio' + NumRigo, tr.id); td.style.width = "350px";
                    M = CreaElemento('input', 'MinutaggioMinuti' + NumRigo, td.id); M.setAttribute("type", "number"); M.className = "SelettoreMinutaggioMinuti";
                    ...
                    S = CreaElemento('input', 'MinutaggioSecondi' + NumRigo, td.id); S.setAttribute("type", "number"); S.className = "SelettoreMinutaggioSecondi";
                    S.setAttribute("min", "0"); S.setAttribute("max", "59.999"); S.setAttribute("step", "1");
                    S.value = MinutaggioAttuale.Secondi;
                    S.dataset.RiferimentoRegistrazione = RiferimentoRegistrazione;
                    /* S.addEventListener("change", ...); */

                    CreaElemento('br', 'br' + NumRigo, td.id);
                    CreaElemento('a', 'pulPlay' + NumRigo, td.id, "<span class='fa fa-play-o'></span> Play").className = 'btn btn-default btn-xs';
