...
function AggiornaRappresentazioneColonnaInternazionale(AggiornaSeModificato = false) {
    ...
    TracciaCI.iStyle({backgroundImage: `linear-gradient(90deg, ${colorePorzioneTagliata} 0%, ${colorePorzioneTagliata} ${PercentualeMinutaggio(InizioVideoGuida)} ${strSfondo.slice(0, -2)})`, backgroundSize: "100% 100%"});
...
function CreazioneClipPrimoCaricamento(DatiClipAudio) {
    const totClipAudio = DatiClipAudio.length, PresenteColonnaInternazionale = DatiCI.find(el => el.CI), CondizionePulsanteSwitchColonnaInternazionale = (PresenteColonnaInternazionale || SonoCreatoreProgetto);
