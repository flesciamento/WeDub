...
for (var NomeTasto in TastiIndicazioni) {
	...
	const divTasto = CreaElemento('div', 'div' + NomeTasto, 'infoTasti', "<a class='btn btn-default btn-xs' style='pointer-events: none;' data-tasto='" + NomeTasto + "'><b>" + NomeTasto + "</b></a> " + Testo); divTasto.setAttribute('title', Descrizione); divTasto.dataset.tasto = NomeTasto; divTasto.className = 'btn btn-default';
          divTasto.onclick = (e) => {if (UltimaTextareaUtilizzata && TrovaCampoTestoVicino(UltimaTextareaUtilizzata, 0)) {UltimaTextareaUtilizzata.focus(); const tasto = e.currentTarget.dataset.tasto; setTimeout(() => {GestisciImmissioneTesto({currentTarget: UltimaTextareaUtilizzata, key: tasto});}, 100);}};
}
...
async function GestisciImmissioneTesto(e) {
    ...
    if (Object.keys(TastiIndicazioni).includes(e.key)) {
        ...
        NodoAttuale.textContent = NodoAttuale.textContent.slice(0, PosizioneCursore) + ((PosizioneCursore > 0) ? " " : "") + TestoIndicato.slice(0, TestoIndicato.indexOf('|')) + " " + NodoAttuale.textContent.slice(PosizioneCursore); PosizionaCursore(NodoAttuale, (+PosizioneCursore) + (+TestoIndicato.length) + 1);
        SalvaCopione(T);
        return;
    }
...
function PartenzaProgramma() {
	...
	VideoGuidaPause();
    setTimeout(() => {AttivaVetro(false);}, 500);
}
