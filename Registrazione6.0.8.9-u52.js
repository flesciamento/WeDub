...
function DividiClip_puntatorerilasciato(e) {
    const ELT = ELTCliccato, MinutaggioPrimoClick = MinutaggioCliccatoNellaClip(+ELT.dataset.posizionePuntatoreInizioTaglio), MinutaggioRilascio = MinutaggioCliccatoNellaClip(e.clientX || e.touches[0].clientX), TrascinatoVersoDestra = MinutaggioPrimoClick < MinutaggioRilascio;
    ...
    document.body.removeEventListener('mouseup', DividiClip_puntatorerilasciato); document.body.removeEventListener('touchend', DividiClip_puntatorerilasciato); ELT.removeEventListener('mousemove', AnteprimaSelezioneClip.Visualizza); ELT.removeEventListener('touchmove', AnteprimaSelezioneClip.Visualizza);
    CambiaTool(toolStandard);
    DisabilitaSchermata();

    /*** Se taglio senza trascinamento, si limita a dividere la clip in due senza chiedere niente ***/
    if ((secondiInizioTaglio | 0) == (secondiFineTaglio | 0)) {DividiClip(ELT.dataset.RiferimentoRegistrazione, secondiInizioTaglio, secondiFineTaglio); return;}
    /************************************************************************************************/
...
function CreaElementoLineaTemporale(ID, DoveInserirlo, PartenzaRegistrazione, LunghezzaRegistrazione, Stringa, RiferimentoRegistrazione, Posizione, Altezza, imgOndaSonora, DaVisualizzare) {
	...
	if ( ( (DatiAudioRegistrato[RiferimentoRegistrazione].ID_Utente == ID_Utente) && (SolaVisione == false) ) || (SonoCreatoreProgetto)) {
        ...
        function GestioneEventoELTCliccato (e) {
            ...
            const ELT = e.currentTarget, NumeroAudio = +ELT.dataset.RiferimentoRegistrazione, datiAudio = DatiAudioRegistrato[NumeroAudio], X = e.clientX || e.touches[0].clientX;
                ...
                switch(StrumentoMouse) {
                    ...
                    case toolDividiClip:
                        ...
                        if ((datiAudio.taglioIniziale < MinutaggioSelezionatoClip) && (MinutaggioSelezionatoClip < datiAudio.taglioFinale)) {
                            ...
                            ELT.addEventListener('mousemove', AnteprimaSelezioneClip.Visualizza);       ELT.addEventListener('touchmove', AnteprimaSelezioneClip.Visualizza);
                            document.body.addEventListener('mouseup', DividiClip_puntatorerilasciato);  document.body.addEventListener('touchend', DividiClip_puntatorerilasciato);
                        }
                    break;
        ...
        ELT.addEventListener('mousedown', GestioneEventoELTCliccato); ELT.addEventListener('touchstart', GestioneEventoELTCliccato);
...
function AnteprimaSelezioneClip(ELT, ClientX) {
    ...
    var divCartello = false;

    if (SistemaAttualeAndroid) {divCartello = CreaElemento('div', 'divCartello' + div.id, document.body.id, strCliccaQuiPerConfermareSelezione); divCartello.className = 'alert alert-info'; divCartello.iStyle({position: "fixed", zIndex: 1000000000, top: "10px", left: "40%"});}

    AnteprimaSelezioneClip.Visualizza = (e) => {
        const DistanzaMouse = ((+(e.clientX || e.touches[0].clientX) + window.scrollX) - ContenitoreRighello.offsetLeft - e.currentTarget.offsetLeft) - x;
        div.iStyle({width: Math.abs(DistanzaMouse) + "px", transform: ((DistanzaMouse > 0) ? "" : "translate(-100%, 0)")});
        e.preventDefault();
    };

    AnteprimaSelezioneClip.Elimina = () => {EliminaElemento(div); EliminaElemento(divCartello);};
}
