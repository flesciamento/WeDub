...
function VisualizzazioneGraficaTaglioClip(Numero, CropOnda = true) {
...
function CreaElementoLineaTemporale(ID, DoveInserirlo, PartenzaRegistrazione, LunghezzaRegistrazione, Stringa, RiferimentoRegistrazione, Posizione, Altezza, imgOndaSonora, DaVisualizzare) {
	...
	if ( ( (DatiAudioRegistrato[RiferimentoRegistrazione].ID_Utente == ID_Utente) && (SolaVisione == false) ) || (SonoCreatoreProgetto)) {
        ...
        ELT.addEventListener('mouseleave', (e) => {clearTimeout(tmrRiposizionamentoAutomaticoELT); VisualizzaOndaSonoraClipCompleta(e.currentTarget, document.getElementById(ID_Opzioni));});
...
function AnteprimaSelezioneClip(ELTConsiderato, ClientX, FunzioneTouchEnd = () => {}) {
    ...
    AnteprimaSelezioneClip.Elimina = () => {EliminaElemento(div); clearTimeout(AnteprimaSelezioneClip.tmr);};
}
...
function CreaFinestraOpzioniClip(RiferimentoRegistrazione) {
    ...
    setTimeout(() => {divContenitoreOpzioni.style.display = "inline"; divContenitoreOpzioni.onmouseenter = DisattivaSpostamentoManualeClip; VisualizzaOndaSonoraClipCompleta(ELT, true); window.addEventListener('keydown', ScorciatoieTastieraFinestraOpzioni);}, 200);
...
function OpzioniClip(Numero, Apri, SalvaAllaChiusura) {
    ...
    if (Apri) {
        ...
        setTimeout(() => {VisualizzaOndaSonoraClipCompleta(ELT, false);}, 500);
        RiabilitaSchermata();
