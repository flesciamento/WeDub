        ...
        case "Backspace":
            if (T.innerText.trim() == "") {
                ...
                if ((e.key == "Backspace") && (PosizioneCursore < 2)) {
                    ...
                    if ((NumeroSpanBattuta <= 2) && (NumeroSpan == 2)) {SalvaCopione(T, true); await pausa(100); ( (div = document.getElementById('RifCopione' + (T.dataset.numid - 1))) && (div.lastElementChild.querySelector("[contenteditable='true']").focus()) ); return;}
                    
                    /* if (!EraPresentePersonaggio && !brPrecedente) {tagNomePersonaggio.innerHTML = "&nbsp;";}  */
                    
                    EliminaElemento(brPrecedente);

                    /* Salva il copione, aggiorna il testo e si posiziona nella casella da editare */
                    SalvaCopione(T, true, NumeroSpanBattuta - (2 * EraPresentePersonaggio), (EraPresentePersonaggio? spanBattuta.previousElementSibling.children.length - 1 : NumeroSpan - 3));
                    return;
                }
            }
            break;
