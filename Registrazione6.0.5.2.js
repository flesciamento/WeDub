...
function Posizionati(MinutaggioNuovo, RiabilitaLaSchermata = true) {
    if (VideoGuidaMinutaggioCorrente() == MinutaggioNuovo) {return;}
    DisabilitaSchermata(true);
    Posizionati.RiabilitaLaSchermata = RiabilitaLaSchermata;
    setTimeout(() => {PosizionaCursore(MinutaggioNuovo);}, 100);
    Posizionati.stavoRiproducendo = (RiproduzioneInCorso? "si" : "no");
...
Posizionati.stavoRiproducendo = false;
Posizionati.RiabilitaLaSchermata = true;

function RiattivaVideoGuida() {
    VideoGuidaRimuoviEventoAlTermineCaricamento(RiattivaVideoGuida);
    setTimeout(() => {
        if (Posizionati.stavoRiproducendo == "si") {
            PlayVideoGuida();
        } else {
            if (Posizionati.RiabilitaLaSchermata) {RiabilitaSchermata(true);}
        }
    }, 100);
}
