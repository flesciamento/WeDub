                        ...
                        pulRiduciRumore.onclick = async (e) => {
                            ...
                            SalvaModificheClipAudio(Numero, async () => {
                                ...
                                function VerificaTermineProcesso() {
                                    CaricaAudio(0, {Registrazione: "FileProvvisori/" + ID_Utente + bloburl.slice(bloburl.lastIndexOf('/') + 1) + "." + formatoQualitaAlta}, (Contenuto) => {audioContext.decodeAudioData(Contenuto).then((buffer) => {da.buffer = buffer; CreaClipAudio(da.numero); console.log("Creato nuovo audio"); pulsante.abilita(true);}).catch((err) => {console.log("Errore nuovo audio", err);})});
                                }
