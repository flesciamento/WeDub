	...
	AJAX("TrascrizioneVideo_CaricaCopione.php", "N=" + encodeURIComponent(N),
        async function (Dati) {
                ...
                if (totBlocchiDiTesto == 0) {
                    ...
                    CreaElemento('div', 'ContenitoreCaricaSottotitoli', 'tdContenitoreTesto', `<fieldset><legend>${strCaricaSottotitoli}</legend><form id='frmFileSottotitoli' action="TrascrizioneCopione_CaricaSottotitoli.php" method="post" enctype='multipart/form-data'></form></fieldset>`);
...
AltreFunzioniPlayVideoGuida = () => {RiproduzioneInCorso = true; MettiFocusTextareaCorrispondente(); if (InizioVideo || FineVideo) {VerificaRiproduzioneSegmento(InizioVideo, FineVideo);}};
