...
/*		case "[": T.innerHTML = T.innerHTML.slice(0, PosizioneCursore) + VisualizzaMinutaggioAttuale() + "] " + T.innerHTML.slice(PosizioneCursore); T.selectionEnd = PosizioneCursore; setTimeout(() => {T.selectionStart = T.selectionEnd = T.innerHTML.length;}, 100); return 2;
        ...
        case "ArrowDown": ((NumeroNodoCursore(InfoCursore.T, NodoCursore) == InfoCursore.T.childNodes.length) && (textareasuccessiva = TrovaTextareaVicina(T, 1)) && (PosizionaCursore(textareasuccessiva.childNodes[0], 0)) && (textareasuccessiva.focus())); break;
		case "Enter":
