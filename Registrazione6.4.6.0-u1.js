...
function DeterminaVolumeVideoGuidaPerCI() {
    if (ColonnaInternazionaleAttivata) {if (PosizioneAttualeDatiCI().CI) {CI_DisattivaAudioOriginale();} else {CI_AttivaAudioOriginale(false);}}
}
...
function AggiornaRappresentazioneColonnaInternazionale() {
    const strDatiCI = JSON.stringify(DatiCI);
    if (strDatiCI == AggiornaRappresentazioneColonnaInternazionale.strDatiCIPrec) {return;}

    const divCIStato = document.getElementById('divCI_Stato');
    const totDatiCI = DatiCI.length, TermineVideo = (((FineVideo > InizioVideo) && (FineVideo < totDurataVideoGuida)) ? FineVideo : totDurataVideoGuida);
    
    function PercentualeMinutaggio(Minutaggio) {return Math.floor(Minutaggio / totDurataVideoGuida * 100).toString() + "%, ";}

    divCIStato.innerHTML = "";
    
    var strSfondo = "", percPartenza = PercentualeMinutaggio(InizioVideo), MenuADiscesa = [];
    for (let I = 0; I < totDatiCI; I++) {
        const BloccoCI = DatiCI[I], colore = (BloccoCI.CI ? "rgba(0, 150, 0, 0.5) " : `rgba(255, 255, 255, ${BloccoCI.VolumeVideoGuida}) `), percTermine = PercentualeMinutaggio((DatiCI[+I + 1] || {Partenza: TermineVideo}).Partenza);
        strSfondo += colore + percPartenza + colore + percTermine;

        MenuADiscesa.push({DoveInserirlo: 'divCI_Stato', ID_Menu: 'pulMenuCI' + I, stiliContenitore: {position: "absolute", left: percPartenza.slice(0, -2), width: `calc(${percTermine.slice(0, -2)} - ${percPartenza.slice(0, -2)})`, margin: 0}, stiliMenu: {textAlign: "center", width: "100%", color: ((BloccoCI.CI || BloccoCI.VolumeVideoGuida == 0) ? "white" : "black"), fontWeight: (BloccoCI.CI ? "bold" : ""), backgroundColor: colore, whiteSpace: "break-spaces", wordWrap: "anywhere"},
            Elementi: (BloccoCI.CI
                        ? [{dato: I + "CI", stringa: "<?=s_GP_opzCI_CICaricata?>", predefinito: true, nascosto: true}, {dato: I + "MCI", stringa: "<span class='fa fa-edit'></span> <?=s_GP_opzCI_CIModifica?>"}]
    ...
    divCIStato.style.backgroundImage = `linear-gradient(90deg, ${colorePorzioneTagliata} 0%, ${colorePorzioneTagliata} ${PercentualeMinutaggio(InizioVideo)} ${strSfondo} ${colorePorzioneTagliata} ${PercentualeMinutaggio(TermineVideo)} ${colorePorzioneTagliata} 100%)`;
...
function ApriFinestraCI_e_monitora(e) {
    ...
    ApriFinestraCI_e_monitora.tmr = setInterval(() => {AJAX("AggiornaCIPerGestioneProgetto.php", "N=<?=$N?>", (Dati) => {DatiCI = Dati.DatiCI; AggiornaRappresentazioneColonnaInternazionale();}, "", "", true);}, 4000);
}
...
function CreazioneClipPrimoCaricamento(DatiClipAudio) {
    const totClipAudio = DatiClipAudio.length, PresenteColonnaInternazionale = DatiCI.find(el => el.CI);
