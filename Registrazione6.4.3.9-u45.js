    ...
    function TerminaBufferCI() {
        ...
        if (TrovaSpezzoneVicino(datiAudio, 1)) {
            ...
            if (ClipAudioTerminaPrimaDellaFineDelVideo(datiAudio)) {
                ...
                const LunghezzaBuffer = datiAudio.buffer.length;
                const lunghezzabytemixCIeOriginale = MixCIeOriginale.FadeOut * SampleRate;
                for(c = 0; c < numCanali; c++) {
                    bufferCanale = datiAudio.buffer.getChannelData(c);
...
function CI_AttivaAudioOriginalePocoPrimaDelTermine(datiAudio) {
    datiAudio.alPlay.push({FunzioneAlPlay: CI_AttivaAudioOriginale, latenzaEventoAlPlay: {daltermine: -MixCIeOriginale.AnticipoOriginale, riduciSeClipNelMinutaggio: true}});
}
...
function FadeInVolumeAudioOriginale(Volume) {
    const VolumeFinale = Volume * GuadagnoPrincipale[AudioBufferColonnaInternazionale[0].numero].gain.value;
    var I = 0, Incremento = Math.round(VolumeFinale / 20);
