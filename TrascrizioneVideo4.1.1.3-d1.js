...
function MettiFocusTextareaCorrispondente() {
    const MinutaggioCorrente = VideoGuidaMinutaggioCorrente(), textarea = document.querySelectorAll("textarea[data-eliminata='0']"), totTextarea = textarea.length;
    for (let I = 0; I < totTextarea; I++) {
        ...
        console.log(T.dataset.minutaggio, textareasuccessiva.dataset.minutaggio, MinutaggioCorrente);
        if ((T.dataset.minutaggio <= MinutaggioCorrente) && (MinutaggioCorrente < textareasuccessiva.dataset.minutaggio)) {T.focus(); T.selectionStart = T.selectionEnd = T.value.length; return;}
    }
...
AltreFunzioniPlayVideoGuida = FunzioniCopione.AttivaTestoGuida;
...
document.getElementById('ContenitoreVideoGuida').addEventListener('click', MettiFocusTextareaCorrispondente);
