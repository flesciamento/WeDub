    ...
    AJAX("ElencoCandidatiRuoliDaAssegnare.php", "N=" + encodeURIComponent(N), 
        function (Dati) {
            ...
            for (var I = 0; I < totCandidati; I++) {
                ...
                if (Dati[I].daAscoltare == 1) {CreaElemento('div', 'indicatoreCandidatoRuoliDaAssegnare' + I, pulElenco.id, strNuoveClip).iStyle({position: "absolute", top: "10px", left: "15px", color: "white", backgroundColor: "red", padding: "2px", borderRadius: "20%", fontWeight: "bold", transform: "rotate(-45deg)"});}
...
function VerificaNuoveClipAltriCandidati() {
    AJAX("ElencoCandidatiRuoliDaAssegnare.php", "N=" + encodeURIComponent(N),
        function (Dati) {
            ...
            for (var I = 0; I < totCandidati; I++) {
                contatoreCandidatiNuoveClip += (Dati[I].ID != RuoliDaAssegnare_CandidatoSelezionato) && (Dati[I].daAscoltare == 1);
...
function SalvaModificheClipAudio(Numero, FunzioneAlTermine = () => {}) {
    const datiAudio = DatiAudioRegistrato[Numero], AutoreCommentoClip = (SonoCreatoreProgetto? "CreatoreProgetto" : "Doppiatore"), ModificatoDaAscoltare = (document.getElementById(ID_Opzioni + 'checkPulDaAscoltare')?.dataset?.cliccato == "si");
		...
		function (Dati) {
			...
			for (I = 0; I < totDati; I++) {
				...
				if (NumAudio >= totAudio) {
                    ...
                    /* Effetti */
                    datiAudio.effetti = DatiAggiornati.Effetti; datiAudio.intensitaeffetti = DatiAggiornati.IntensitaEffetti; VisualizzaEffettiAudio(NumAudio); // Gli effetti audio verranno applicati solo alla creazione del buffer (v. CreaClipAudio)
                    ...
                    /* Ascoltato */
                    if (SonoCreatoreProgetto && (datiAudio.daAscoltare != DatiAggiornati.Visualizzato)) {VisualizzaModificaAudioAscoltato(datiAudio);}
				}
            }
...
function ContrassegnaClipDaAscoltare(datiAudio) {
    if (datiAudio.daAscoltare) {
        document.getElementById('ELTReg' + datiAudio.numero + 'Contenuto').style.border = "3px dashed darkgreen";
        datiAudio.alPlay = [{FunzioneAlPlay: AJAXSalvaAudioAscoltato, latenzaEventoAlPlay: {secondi: 2, riduciSeClipNelMinutaggio: false}}];
    };
}

function AJAXSalvaAudioAscoltato(datiAudio) {
    AJAX('SalvaRegistrazioneAscoltata.php', "N=" + datiAudio.NumeroUnivoco + "&A=1", "", "", "", true);
...
function SelezionaESpostaELT(NumeroAudio, X = 0) {
    ...
    [document.getElementById(ID_Opzioni + 'TabellaOpzioniRigaVolume'), document.getElementById(ID_Opzioni + 'divOpzioneEffetti')].forEach((opzionemodificamultipla) => {
...
function CreaFinestraOpzioniClip(RiferimentoRegistrazione) {
    ...
    var NuovoAudioTrattato = false, NuoveClipCreate = [], tr, td;
    ...
    const divcontenitorebody = CreaElemento('div', ID_Opzioni + 'ContenitoreOpzioniBody', ID_Opzioni); divcontenitorebody.className = "panel-body";
        ...
        if (SonoCreatoreProgetto) {
            const labelDaAscoltare = CreaElemento('label', ID_Opzioni + 'labelPulDaAscoltare', divcontenitorebody.id); labelDaAscoltare.className = "btn btn-default btn-xs"; labelDaAscoltare.iStyle({position: "absolute", top: "50px", right: "2px"}); labelDaAscoltare.setAttribute('title', strDaAscoltare_title);
        ...
        /******************************/
                                                    ...
                                                    /* Minutaggio */
        const tabellaminutaggio = CreaElemento('table', ID_Opzioni + 'TabellaMinutaggio', divcontenitorebody.id); tabellaminutaggio.iStyle({float: "left", width: "250px"});
            tr = CreaElemento('tr', ID_Opzioni + 'TabellaOpzioniRiga1', tabellaminutaggio.id);
                td = CreaElemento('td', ID_Opzioni + 'tdOpzionePosizioneLabel', tr.id, strPosizione); td.iStyle({fontFamily: "Verdana", fontSize: "12px", verticalAlign: "bottom", paddingBottom: "14px"});
                        ...
                        tr = CreaElemento('tr', ID_Opzioni + 'SottoTabellaRiga2', SottoTabella.id);
                            ...
                            td = CreaElemento('td', ID_Opzioni + 'tdOpzionePosizioneSecondi', tr.id);
                                ...
                                S.addEventListener("change", function () {SpostaMinutaggioRegistrazione(this.dataset.RiferimentoRegistrazione);});
                                        ...
                                        /* Effetti */
        div = CreaElemento('div', ID_Opzioni + 'divOpzioneEffetti', divcontenitorebody.id); div.iStyle({textAlign: "center", float: "left"});
        ...
        CreaElemento('div', ID_Opzioni + 'divSeparatore', divcontenitorebody.id).style.clear = "both";
               
        /*** Tabella sliders ***/
        var Tabella = CreaElemento('table', ID_Opzioni + 'TabellaOpzioni', divcontenitorebody.id); Tabella.className = "TabellaOpzioni"; Tabella.style.width = "100%";
                    ...
                    const divTrattamentoAudio = CreaElemento('div', ID_Opzioni + 'divTrattamentoAudio', divOpzioniVarie.id); divTrattamentoAudio.className = "btn"; divTrattamentoAudio.iStyle({cursor: "auto", width: "fit-content"});
                        const fieldsetTrattamentoAudio = CreaElemento('fieldset', divTrattamentoAudio.id + "fieldset", divTrattamentoAudio.id);
                            ...
                            pulSpezzaBattute.onclick = async (e) => {
                                ...
                                if (NumeroAttacchi > 1) {
                                    da.taglioIniziale = attacchi[0];
                                    da.taglioFinale = attacchi[1];
                                    VisualizzazioneGraficaTaglioClip(Numero, false);
                                    VisualizzaEffettiAudio(Numero);
                                    AggiornaRiproduzioneClip(Numero);
                                    if (NumeroAttacchi > 2) {
                                        ...
                                        for(A = 2; A < NumeroAttacchi; A += 2) {
                                            ...
                                            if ((SecondiStacco - SecondiAttacco) > SpezzaBattute.SecondiTaglioMinimo) {
                                                ...
                                                if (NumeroAttacchi == 3) {Messaggio(strSpezzaBattute_battutatagliata, "OK"); Riattiva();} // Se ha trovato solo due battute e l'ultima è troppo corta, conclude il processo.
                                            }
                                        }
                                    } else {
    ...
    div = CreaElemento('div', ID_Opzioni + 'PulsantiFinali', ID_Opzioni); div.className = "panel-footer";
        Tabella = CreaElemento('table', ID_Opzioni + 'TabellaPulsantiFinali', divcontenitorebody.id); Tabella.className = "TabellaOpzioni"; Tabella.style.width = "100%";
