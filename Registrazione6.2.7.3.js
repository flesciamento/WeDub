...
function AttivaProgramma() {
    ...
    if (!ModalitaStreaming) {
        ...
        setTimeout(() => {document.getElementById('LogoWEDUB').iStyle({top: 0, right: 0, marginRight: 0});}, 1000);
...
async function Posizionati(MinutaggioNuovo, RiabilitaTuttaLaSchermata = false) {
...
async function TermineCaricamentoClip() {
    if (!CaricamentoInizialeTerminato) {
        ...
        if (ModalitaStreaming) {
            ...
            DisattivaMessaggiAttesa();
            AttivaInterfaccia();
            document.getElementById('LogoWEDUB').iStyle({top: 0, right: 0, opacity: 0.5, marginRight: 0});
...
window.setTimeout(CaricamentoVideo, 2000); tmrCaricamento = window.setInterval(CaricamentoVideo, 10000); window.setTimeout(() => {const imgLogoWEDUB = document.getElementById('imgLogoWEDUB'); imgLogoWEDUB.src = "assets/img/" + LogoPrincipale; imgLogoWEDUB.parentNode.style.marginRight = (150 + (larghezzaLogo / 2)) + "px"; imgLogoWEDUB.setAttribute('width', larghezzaLogo)}, secondiCambioLogo);
