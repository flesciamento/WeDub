...
/*		case "[": T.innerHTML = T.innerHTML.slice(0, PosizioneCursore) + VisualizzaMinutaggioAttuale() + "] " + T.innerHTML.slice(PosizioneCursore); T.selectionEnd = PosizioneCursore; setTimeout(() => {T.selectionStart = T.selectionEnd = T.innerHTML.length;}, 100); return 2;
    ...
    async function AttivaSceltaPersonaggio() {
        await pausa(100);
        UltimaPosizioneCursore = PosizioneDelCursore(); console.log("UltimaPosizioneCursore:", UltimaPosizioneCursore);
...
async function ScriviPersonaggio(T, Personaggio) {
    ...
    PosizionaCursore(UltimaPosizioneCursore.nodo, UltimaPosizioneCursore.p);

    var InfoCursore = PosizioneDelCursore(); console.log("ScriviPersonaggio", InfoCursore); // Aggiorna le info sul cursore
    var NuovaPosizioneCursore = InfoCursore.p, NodoCursore = InfoCursore.nodo;
		
    if (Personaggio) {
        NodoCursore.textContent = Personaggio + ": " + NodoCursore.textContent.slice(InfoCursore.p);
        NuovaPosizioneCursore += (+Personaggio.length) + 2;
    }

    SalvaCopione(T);

    InfoCursore = PosizioneDelCursore(); console.log("ScriviPersonaggio", InfoCursore);
    PosizionaCursore(InfoCursore.nodo, NuovaPosizioneCursore);
...
function SalvaCopione(Textarea) {
    ...
    PosizionaCursore(c.T.childNodes[NumNodo], c.p); // Rimette il cursore dov'era. Ritrova il nodo attraverso il numero.
