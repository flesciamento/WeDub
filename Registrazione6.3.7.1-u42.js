                    ...
                    td = CreaElemento('td', ID_Opzioni + 'tdOpzioneRiduciRumore', tr.id);
                        ...
                        pulRiduciRumore.onclick = async (e) => {
                            ...
                            function VerificaTermineProcesso() {
                                AJAX("CaricaBlobTemp.php", "N=" + encodeURIComponent(da.NumeroUnivoco), (Dati) => {if (Dati.urlBlob != "") {CaricaAudio(0, {Registrazione: Dati.urlBlob}, (Contenuto) => {audioContext.decodeAudioData(Contenuto).then((buffer) => {da.buffer = buffer; CreaClipAudio(da.numero); console.log("Creato nuovo buffer"); pulsante.abilita(true);})});} else {setTimeout(VerificaTermineProcesso, 1000);}});
                            }
