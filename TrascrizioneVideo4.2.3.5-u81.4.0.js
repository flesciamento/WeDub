        ...
        case "Backspace":
            if (T.innerText.trim() == "") {
                ...
                if ((e.key == "Backspace") && (PosizioneCursore < 2)) {
                    ...
                    if (NumeroSpanBattuta == 2) {SalvaCopione(T, true); GestisciImmissioneTesto({currentTarget: ElementoAttuale, key: "ArrowUp"}); return;}
                    
                    console.log(spanBattuta, EraPresentePersonaggio, spanBattuta.previousElementSibling, tagNomePersonaggio.previousElementSibling);
                    /* Elimina il <br> */
                    if (EraPresentePersonaggio) {EliminaElemento(spanBattuta.previousElementSibling);} else {EliminaElemento(tagNomePersonaggio.previousElementSibling);}
                    /* Salva il copione, aggiorna il testo e si posiziona nella casella da editare */
                    SalvaCopione(T, true, NumeroSpanBattuta - (2 * EraPresentePersonaggio), (EraPresentePersonaggio? spanBattuta.previousElementSibling.children.length - 1 : NumeroChildren(spanBattuta, ElementoAttuale) - 3));
                    return;
                }
            }
            break;
