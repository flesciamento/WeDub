...
function CaricaCopione(FunzioneAlTermine = () => {}) {
	...
	AJAX("TrascrizioneVideo_CaricaCopione.php", "N=" + encodeURIComponent(N),
        async function (Dati) {
                ...
                setTimeout(() => {Messaggio(strCaricamentoInCorso); BarraCaricamento.style.opacity = 1; ContornoBarraCaricamento.style.opacity = 1;}, 300);

                var textareaCreate = 0, tmr = setInterval(() => {VisualizzaProgressoBarraCaricamento(BarraCaricamento, textareaCreate / totBlocchiDiTesto);}, 1500);
...
function PartenzaProgramma() {
    ...
    document.getElementById('pannelloContenitoreInfo').append(divAJAX); divAJAX.iStyle({position: "static", width: ""}); VisualizzaProgressoBarraCaricamento(BarraCaricamento, 1); setTimeout(() => {BarraCaricamento.style.opacity = 0; Messaggio(strCopioneCaricato);}, 1000);
...
function MettiFocusTextareaCorrispondente() {
    setTimeout(() => {
        ...
        for (let I = 0; I < totTextarea; I++) {
            ...
            if ((T.dataset.minutaggio <= MinutaggioCorrente) && (MinutaggioCorrente < textareasuccessiva.dataset.minutaggio) && (UltimaTextareaUtilizzata != T)) {T.focus(); T.selectionStart = T.selectionEnd = T.value.length; break;}
        }
...
function AdattaAltezzeTextarea() {
    const textarea = document.getElementsByTagName('textarea'), totTextarea = textarea.length;
