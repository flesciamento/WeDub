...
function GeneraBufferCI(datiAudio, buffer, FunzioneAlTermine = () => {}) {
    ...
    const b = buffer, SampleRate = b.sampleRate, numCanali = b.numberOfChannels, lunghezzabytetempomix = SampleRate * tempomixCIPrec;
    ...
    function TerminaBufferCI() {
        ...
        if (TrovaSpezzoneVicino(datiAudio, 1)) {
            ...
            datiAudio.alPlay = () => {if (ColonnaInternazionaleAttivata && RiproduzioneInCorso && (VideoGuidaMinutaggioCorrente() < (totDurataVideoGuida - 1))) {SwitchColonnaInternazionale(false);}};
...
function startRecording() {
    ...
    if (!ModalitaUltraLightAttiva) {
        ...
        c.iStyle({position: "absolute", top: "0%", height: "100%", left: Cursore.offsetLeft + "px", border: "2px dotted blue"});
...
function EffettuaAutoTaglioIniziale(arraybufferaudio, sampleRate) {
    taglioiniziale = arraybufferaudio.indexOf(arraybufferaudio.find((a) => {return a > AutoTaglioIniziale.SogliaDB;})) - (AutoTaglioIniziale.Tolleranza * sampleRate);
    EffettuatoAutoTaglioIniziale = (taglioiniziale > 0);
    taglioiniziale *= EffettuatoAutoTaglioIniziale;
    arraybufferaudio = arraybufferaudio.slice(taglioiniziale);
    MinutaggioUltimaRegistrazione += (taglioiniziale / sampleRate);
    return arraybufferaudio;
}
