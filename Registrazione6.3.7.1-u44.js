                ...
                tr = CreaElemento('tr', ID_Opzioni + 'TabellaOpzioniRigaVarie', Tabella.id);
                    ...
                    td = CreaElemento('td', ID_Opzioni + 'tdOpzioneRiduciRumore', tr.id);
                        ...
                        pulRiduciRumore.onclick = async (e) => {
                            ...
                            const doppiomediaprofilorumore = mediaprofilorumore * 2;
                            for (let I = 0; I < lunghezzaBuffer; I++) {
                                const sogliasuperata = (Math.abs(b[I]) > doppiomediaprofilorumore);
                            ...
                            ApriFinestra({currentTarget: {dataset: {larghezza: "800px", altezza: "600px", link: "prova_ffmpeg_nuovo.php?N=" + encodeURIComponent(da.NumeroUnivoco) + "&Percorso=" + encodeURIComponent(bloburl)}}});

                            /* function VerificaTermineProcesso() {
                                AJAX("CaricaBlobTemp.php", "N=" + encodeURIComponent(da.NumeroUnivoco), (Dati) => {if (Dati.urlBlob != "") {CaricaAudio(0, {Registrazione: Dati.urlBlob}, (Contenuto) => {audioContext.decodeAudioData(Contenuto).then((buffer) => {da.buffer = buffer; CreaClipAudio(da.numero); console.log("Creato nuovo audio"); pulsante.abilita(true);})});} else {setTimeout(VerificaTermineProcesso, 1000);}}, "", "", true);
                            }
