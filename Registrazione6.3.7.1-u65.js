...
function AttivaAggiornamentoClip() {
    ...
    if (!ModalitaStreaming && !ModalitaLightAttiva && (ELTDaModificare.length == 0)) {tmrAggiornaClip = window.setInterval(AggiornaClip, 30000);}
}
                ...
                tr = CreaElemento('tr', ID_Opzioni + 'TabellaOpzioniRigaVarie', Tabella.id);
                    ...
                    const tdTrattamentoAudio = CreaElemento('td', ID_Opzioni + 'tdTrattamentoAudio', tr.id);
                        ...
                        pulRiduciRumore.onclick = async (e) => {
                            ...
                            DisabilitaSchermata();
                            const pulsante = e.currentTarget, Numero = pulsante.dataset.RiferimentoRegistrazione, da = DatiAudioRegistrato[Numero], b = da.buffer.getChannelData(0), lunghezzaBuffer = b.length, SampleRate = da.buffer.sampleRate, LimiteSecondiSogliaRumore = SampleRate / DecimiSecondiSogliaRumore, bufferoriginale = audioContext.createBuffer(1, lunghezzaBuffer, SampleRate);
                            StoppaAutomaticamenteAscoltoInSolo(da);
                            bufferoriginale.copyToChannel(b, 0);
                            console.log("bufferoriginale", bufferoriginale, "b", b);
                                        ...
                                        CaricaAudio(0, {Registrazione: Dati.PercorsoCompleto}, 'arraybuffer', 
                                            (Contenuto) => {
                                                audioContext.decodeAudioData(Contenuto).then((buffernuovo) => {
                                                    const buffertrattato = buffernuovo;
