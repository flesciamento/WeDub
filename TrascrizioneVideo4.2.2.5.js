...
function CreaNuovaTextarea(NumID, Minutaggio) {
    ...
    const Rigo = CreaElemento('div', 'RigoBloccoTesto' + NumRigo, "ContenitoreTesto");
            im = CreaElemento('div', 'divInfoMinutaggio' + NumRigo, Rigo.id); im.className = "colonna-minutaggio";
                    ...
                    M = CreaElemento('span', 'MinutaggioMinuti' + NumRigo, im.id, MinutaggioAttuale.Minuti || "0"); M.style.textAlign = "right";
            
                    CreaElemento('span', 'spandivisore' + NumRigo, im.id, ":");

                    S = CreaElemento('span', 'MinutaggioSecondi' + NumRigo, im.id, VisualizzaSecondi(MinutaggioAttuale.Secondi));
                    
                    fs = CreaElemento('span', 'freccias' + NumRigo, im.id); fs.className = "btn btn-default btn-xs fa fa-arrow-up";   fs.iStyle({fontSize: "50%", float: "right"}); fs.dataset.rifRigo = NumRigo; fs.dataset.differenza = 1;  fs.dataset.numid = NumID; fs.onclick = AggiornaMinutaggio;
                    fg = CreaElemento('span', 'frecciag' + NumRigo, im.id); fg.className = "btn btn-default btn-xs fa fa-arrow-down"; fg.iStyle({fontSize: "50%", float: "right"}); fg.dataset.rifRigo = NumRigo; fg.dataset.differenza = -1; fg.dataset.numid = NumID; fg.onclick = AggiornaMinutaggio;

            divTesto = CreaElemento('div', 'divTesto' + NumRigo, Rigo.id);
                /** Textarea **/
                const NuovaTextarea = CreaElemento('textarea', 't' + NumRigo, divTesto.id);
