    ...
    const NuoviElementi = CreaNuoviElementi([
        ...
        PannelloOpzioni,
            ['div', {id: divBody_id, className: "panel-body text-center"}],,
                ['div', {innerHTML: strDescriviPersonaggiDoppiatiDa + "<b>" + DatiDoppiatoreCandidato.nomecandidato + "</b>&nbsp;"}, {margin: "10px 20px 40px 20px"}],, ['input', {id: inputRuoliCandidato_id}, {}, {}, {type: "text", name: "RuoloCandidato", size: 30, value: strRuoliDaAssegnare.replace(/\n/g, ' ')}],
...
function VerificaModificaCopione(divContenitore) {
    ...
    if (!ContenitoreCopione.document.getElementById(id_pulRipristina) && (AcquisisciTestoCopione(divContenitore) != TrovaDatiCopioneID(divContenitore.dataset.numid).testo)) {
        const NuoviPulsanti = [divContenitore], stilipulsanti = {margin: "10px 5px", padding: "0 5px", lineHeight: 2};
        if (SonoCreatoreProgetto) {NuoviPulsanti.push(['a', {id: id_pulSalva, className: "alert btn-warning", innerHTML: "<span class='fa fa-save'></span>", onclick: SalvaCopioneModificato}, stilipulsanti, {idcontenitore: divContenitore.id}]);}
        NuoviPulsanti.push(['a', {id: id_pulRipristina, className: "alert btn-warning", innerHTML: "<span class='fa fa-undo'></span>", onclick: RipristinaCopioneModificato}, stilipulsanti, {idcontenitore: divContenitore.id}]);
...
function SalvaCopioneModificato(e) {
    ...
    if (ContenitoreCopione != window) {window.FunzioniCopione.RiformattaBloccoTesto(TrovaElementoCopioneModificato(e, window), Testo);}
}
...
function DividiClip_puntatorerilasciato(e) {
    ...
    const styleImmagini = "style='float: left; height: 40px; border-radius: 20%;'", classePulsanti = 'btn btn-default btn-lg', selezioneparteiniziale = (secondiInizioTaglio <= (+datiAudio.taglioIniziale) + (+taglioclip_diffmin)), selezionepartefinale = (secondiFineTaglio >= (datiAudio.taglioFinale - taglioclip_diffmin));
    let Opzioni = [
        document.body,
    ...
    if ((secondiInizioTaglio > taglioclip_diffmin) && (secondiFineTaglio < (datiAudio.Durata - taglioclip_diffmin))) {
            ...
            if (DoppiatoreConsiderato) {
                function TrovaClipEPosizionati() {
                    if (DatiAudioUtente = DatiAudioRegistrato_Utente[DoppiatoreConsiderato]) {
                        ...
                        if (PrimaClipDaAscoltare) {PosizioneInizialeCursore = (+PrimaClipDaAscoltare.MinutaggioRegistrazione) + (+PrimaClipDaAscoltare.taglioIniziale) - 2; slideZoom.value = 1000; CambiaZoom(); window.scroll({top: document.getElementById('Traccia' + DatiDoppiatori[DoppiatoreConsiderato].numeroTraccia).offsetTop - 90});}
                    }
...
function VolumeAttualeCI() {
    return (AudioBufferColonnaInternazionale.length ? GuadagnoPrincipale[AudioBufferColonnaInternazionale[0].numero].gain.value : 1) * AttivaDisattivaCI.attivata;
}
...
function RiposizionamentoAutomaticoClipSovrapposte(ELTConsiderato) {
    ...
    if (PrimaRiga.length > 0) {
        PrimaRiga.sort((a, b) => ( a.Registrazione.localeCompare(b.Registrazione) || ((b.taglioFinale - b.taglioIniziale) - (a.taglioFinale - a.taglioIniziale)) ));
        let R = 0;
...
function VideoGuidaPronto() {
    ...
    if (!ModalitaStreaming) {
        ...
        CreaNuoviElementi([ManigliaSposta_TestoGuida, ['span', {id: "pulSwitchCopioneEditabile", className: "btn btn-default fa fa-edit", title: str_copione_ModalitaEditAttivata_title, onmousedown: SwitchCopioneEditabile}, {marginLeft: "10px"}]]);
        /* Aggiunta del tasto che permette di aprire il copione in una finestra separata */
        if (!SistemaAttualeAndroid) {CreaNuoviElementi([ManigliaSposta_TestoGuida, ['span', {id: "pulCopioneInAltraFinestra", className: "btn btn-default fa fa-sign-out", title: str_copione_ApriCopioneInAltraFinestra_title, onmousedown: ApriCopioneInAltraFinestra}, {marginLeft: "10px"}]]);}
        /**********************************************/
    }
