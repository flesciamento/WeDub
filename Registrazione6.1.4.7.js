...
async function Posizionati(MinutaggioNuovo, RiabilitaLaSchermata = true) {
    ...
    Posizionati.tmr = setTimeout(() => {Posizionati(MinutaggioNuovo, RiabilitaLaSchermata);}, 15000);
}
...
function EscludiRipristinaTraccia(IDUtente) {
    ...
    if (TracceEscluse.includes(IDUtente)) {
        ...
        if (RiproduzioneInCorso) {
            if (DatiAudioRegistrato_Utente[IDUtente]) {DatiAudioRegistrato_Utente[IDUtente].forEach((datiAudio) => {StoppaClipAudio(datiAudio);});}
        }
    }
}
...
function CreaFinestraOpzioniClip(RiferimentoRegistrazione) {
    ...
    setTimeout(() => {
        ...
        if ((divContenitoreOpzioni.offsetLeft <= ELT_PosizioneX) && (ELT_PosizioneX <= (+divContenitoreOpzioni.offsetLeft) + (+divContenitoreOpzioni.offsetWidth)) && (ELT_PosizioneY <= (+divContenitoreOpzioni.offsetTop) + (+divContenitoreOpzioni.offsetHeight))) {divContenitoreOpzioni.iStyle({right: "10%", top: "0px"});}
        /*************************************************************/
