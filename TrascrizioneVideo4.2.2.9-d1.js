...
function AdattaAltezzaTextarea(T) {
    ...
    document.getElementById('divInfoMinutaggio' + T.dataset.rifRigo).style.height = T.style.height;
}
...
function AdattaAltezzeTextarea_slow() {
    clearTimeout(AdattaAltezzeTextarea_slow.tmr);
    AdattaAltezzeTextarea_slow.tmr = setTimeout(AdattaAltezzeTextarea, 1000);
}
AdattaAltezzeTextarea_slow.tmr = false;
...
async function AdattaAltezzeTextarea() {
    ...
    for (let I = 0; I < totTextarea; I++) {
        const T = textarea[I];
        if (TextareaVisibileASchermo(T)) {AdattaAltezzaTextarea(T); console.log(T, "Adattata altezza")} else {continue;}
        await pausa(10);
    }
}

function TextareaVisibileASchermo(T) {
    const AltezzaDivContenitore = divContenitoreTesto.offsetHeight, PosizioneY = T.getBoundingClientRect().y - AltezzaDivContenitore;
    return (PosizioneY > 0) && ((AltezzaDivContenitore - PosizioneY) > 0);
}

divContenitoreTesto.onscroll = AdattaAltezzeTextarea_slow;
