...
var MinutaggiRighello = {
	Disegna: async function () {
        const StepRighello = parseInt(50 * totDurataVideoGuida / divMinutaggiRighello.clientWidth) + 1; /* console.log("StepRighello", this.StepRighello, Righello.clientWidth, totDurataVideoGuida); */
        const LunghezzaRighello = totDurataVideoGuida - StepRighello;
		const Minutaggio = new MinutiESecondi(totDurataVideoGuida);
		...
		for (var I = InizioVideoGuida; I < LunghezzaRighello; I += StepRighello) {
			Minutaggio = new MinutiESecondi(I);
            ...
            ((I % 50) && await pausa(100));
		}
    },
    
    Cancella: function () {
        for (var I = 1; I <= this.NumeroMinutaggi; I++) {divMinutaggiRighello.textContent = "";}
    },
...
function AttivaProgramma() {
    ...
    slideMinutaggioAttuale.setAttribute('max', totDurataVideoGuida); slideZoom.setAttribute('max', totDurataVideoGuida * 5);
...
function RiposizionamentoAutomaticoELTRiferitoAllaRegistrazione(datiAudio) {
    setTimeout(() => {RiposizionamentoAutomaticoELT_NumClipAudio(datiAudio.numero);}, 1000);
}
...
function RiposizionamentoAutomaticoClipSovrapposte(ELTConsiderato) {
    if (ELTDaSpostare == false) {
        ...
        DatiAudioRegistrato_Utente[datiAudioConsiderato.ID_Utente].forEach((datiAudio) => {
            ...
            ( (ELT.style.display != "none") && (ELT.style.visibility != "hidden") && (((+datiAudio.MinutaggioRegistrazione) + (+datiAudio.taglioIniziale)) < TermineClipConsiderata) && (MinutaggioPartenzaClipConsiderata < ((+datiAudio.MinutaggioRegistrazione) + (+datiAudio.taglioFinale))) && (ELTDaRiordinare[ELT.id] = true) && (NuoviELTDaRiordinare.push(ELT)) );
        });
