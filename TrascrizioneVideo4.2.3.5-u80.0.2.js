...
function GestisciImmissioneTesto(e) {
    ...
    function MandaACapoEAttivaSceltaPersonaggio() {
        ...
        const s = document.createElement('span'); /* s.contentEditable = true; s.onkeydown = GestisciImmissioneTesto; */
        br.insertAdjacentElement('afterend', s);
        const s2 = document.createElement('span'); s2.textContent = RigaCompleta.slice(PosizioneCursore) || " ";
        s.append(s2);
...
function SalvaCopione(Textarea, AggiornaTesto = false) {
    ...
    if (AggiornaTesto) {
        ...
        document.getElementById(ID_ElementoContenitore).children[NumeroChildren].querySelector("[contenteditable='true']").focus();
    }
