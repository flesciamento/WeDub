...
const toolStandard = 0, toolDividiClip = 1, toolEscludiClip = 2, toolMarcatore = 3, puntatoreTool = ["pointer", "url(images/CursoreMouseLineaTemporale_toolDividiClip.png) 8 0, auto", "url(images/CursoreMouseLineaTemporale_toolEscludiClip.png) 8 0, auto, pointer"];
...
function CambiaTool(Tool) {
    ...
    divContenutoTracce.onmousemove = ((Tool == toolDividiClip) ? AnteprimaCursore : AnteprimaCursoreSeVuoto);
...
Righello.onclick = AggiungiMarcatore;
...
function PosizionatiAlMinutoCliccato(e) {
	if ((Righello.dataset.DisattivaClick == "no") && (e.which == 1) && (e.clientY < window.innerHeight - 50)) {
...
function AggiungiMarcatore(e) {
    ...
    DisegnaMarcatore(IDUtente, MinutaggioMarcatore, strMarcatoreDescrDefault).select();
}

function DisegnaMarcatore(ID_Utente, Minutaggio, Stringa) {
    const Marcatore = CreaElemento('div', 'Marcatore' + (++DisegnaMarcatore.Num), 'Traccia' + DatiDoppiatori[ID_Utente].numeroTraccia); Marcatore.className = 'Marcatore'; Marcatore.dataset.id = 1;
          Marcatore.onclick = (e) => {
            if (e.target.tagName != 'INPUT') {
                if (confirm(strVuoiEliminareMarcatore)) {EliminaElemento(e.currentTarget);}
            }
          };

          const Casella = CreaElemento('input', 'input' + Marcatore.id, Marcatore.id); Casella.value = Stringa;
    InserimentoInProporzioneNellaLineaTemporale(Marcatore, Minutaggio, 1);

    return Casella;
}
DisegnaMarcatore.Num = 0;
...
function CreaElementoLineaTemporale(ID, DoveInserirlo, PartenzaRegistrazione, LunghezzaRegistrazione, Stringa, RiferimentoRegistrazione, Posizione, Altezza, imgOndaSonora, DaVisualizzare) {
	...
	if ( ( (DatiAudioRegistrato[RiferimentoRegistrazione].ID_Utente == ID_Utente) && (SolaVisione == false) ) || (SonoCreatoreProgetto)) {
        ...
        ELT.addEventListener('mousedown',
            function(e) {
                if ((e.which != 1) || (StrumentoMouse == toolMarcatore)) {return;}
