...
function VisualizzaClipAudio(Numero, Attivo) {
    ...
    document.getElementById(id_ELT + "Contenuto").style.opacity = (Attivo? 1 : 0.3);
}
...
async function ApriCestinoTraccia(e) {
    ...
    if (AudioDaRipristinare.length) {
        if (Cestino.dataset.ripristinati == "si") {
            const totAudio = AudioDaRipristinare.length;
            for (let I = 0; I < totAudio; I++) {
                CestinaClipRimosse(AudioDaRipristinare[I]);
                await pausa(30);
            }
...
function VisualizzaClipRimosse(datiAudio) {
    ...
    clearTimeout(VisualizzaClipRimosse.tmr);
    VisualizzaClipRimosse.tmr = setTimeout(() => {RiposizionamentoAutomaticoELT_NumClipAudio(Numero);}, 1000);
}
VisualizzaClipRimosse.tmr = false;
