...
const CanaleRegistrazione = document.getElementById('CanaleRegistrazione');
const QualitaRegistrazione = document.getElementById('QualitaRegistrazione');
...
function AdattaLunghezzaLivelloMic() {
    ...
    livelloMic.style.backgroundSize = lunghezzaLivelloMic + "px " + lunghezzaLivelloMic + "px";
}
...
function AdattaNuovaAltezzaTraccePerc() {
...
function AttivaProgramma() {
        ...
        setTimeout(() => {
            ...
            if (RuoliDaAssegnare_NumeroTraccia !== false) {
                ...
                if (NumeroTotaleTracce == 1) {
                    ...
                    if (!SonoCreatoreProgetto && !SessioneOspite) {AltezzaTracce = window.innerHeight - NomiTracce.children[0].offsetHeight - 200; VariaAltezzaTracce();}
                }
            }
    ...
    setTimeout(() => {
        const vol = ((ModalitaStreaming) || (Provino && SessioneOspite) ? 0 : 1);
...
function SelezionaCandidatoRuoliDaAssegnare(ID) {
    ...
    AJAX("DatiUtente.php", "ID=" + encodeURIComponent(ID),
        function (Dati) {
            ...
            AdattaAltezzaTracciaRuoliDaAssegnare();
            if (!SonoCreatoreProgetto) {AltezzaTracce = window.innerHeight - document.getElementById('divContenutoNomeTraccia' + RuoliDaAssegnare_NumeroTraccia).offsetHeight - 100; VariaAltezzaTracce();} // Se l'utente è il visitatore candidato, ingrandisce il video
...
function ProcessaAudio_Monitora(e) {
   ...
   const valore = instant || (lentezzaLivelloMic * ProcessaAudio_Monitora.valoreprec);
...
function AggiornaParametriRegistrazione() {
    if (registrazione) {
        registrazione.port.postMessage({modalita: "Monitoraggio", numCanale: CanaleRegistrazione.dataset.valore, bufferSize: Number(dimensioneBuffer)});
    }
}
...
function GeneraBufferCI(datiAudio, buffer, FunzioneAlTermine = () => {}) {
    ...
    function TerminaBufferCI() {
        ...
        if (TrovaSpezzoneVicino(datiAudio, 1)) {
            ...
            datiAudio.bufferdalegare = appendBuffer([datiAudio.buffer], LunghezzaBuffer - lunghezzabytetempomix);
            ...
            if (Math.floor((+datiAudio.MinutaggioRegistrazione) + (+datiAudio.Durata)) < Math.floor(totDurataVideoGuida - 1)) {
...
function SalvaModificheClipAudio(Numero, FunzioneAlTermine = () => {}) {
	...
	AJAX("AggiornaRegistrazione-unstable.php", Parametri, FunzioneAlTermine, strAggiornamento, strSalvataggioCompletato, true);
}
...
function AssegnaElementoMenu(e) {
    const ID_Menu = e.currentTarget.dataset.idmenu;
    document.getElementById(ID_Menu).dataset.valore = e.currentTarget.dataset.valore;
    document.getElementById("label" + ID_Menu).textContent = e.currentTarget.textContent;
    ApriChiudiMenu(ID_Menu, false);
}
...
function VideoGuidaPronto() {
    ...
    if (totMenu = MenuDaAggiungere.length) {
        for (let I = 0; I < totMenu; I++) {
            const M = MenuDaAggiungere[I];
            const div = CreaElemento('div', M.ID_Contenitore, M.DoveInserirlo); div.className = 'btn-group'; div.iStyle(M.stili);
                    const a = CreaElemento('a', 'a' + M.ID_Contenitore, M.ID_Contenitore, `<span id='label${M.ID_Menu}'></span>&nbsp;<span class='caret'></span>`); a.dataset.toggle = 'dropdown'; a.dataset.idmenu = M.ID_Menu; a.className = 'btn btn-default btn-xs'; a.onclick = (e) => {ApriChiudiMenu(e.currentTarget.dataset.idmenu, true);}
                    CreaElemento('ul', M.ID_Menu, M.ID_Contenitore);
                        const totElementi = M.Elementi.length;
                        for (let NumElemento = 0; NumElemento < totElementi; NumElemento++) {
                            const li = CreaElemento('li', M.ID_Menu + 'li' + NumElemento, M.ID_Menu, M.Elementi[NumElemento].stringa); li.className = 'btn'; li.dataset.idmenu = M.ID_Menu; li.dataset.valore = M.Elementi[NumElemento].dato; li.onclick = (e) => {AssegnaElementoMenu(e); M.FunzioneAlClick();};
                            if (M.Elementi[NumElemento].predefinito) {AssegnaElementoMenu({currentTarget: li});}
                        }
        }
    }
