...
function AttivaProgramma() {
    ...
    if (!ModalitaStreaming) {
        ...
        const divContenitoreSlideMinutaggio = document.getElementById('divContenitoreSlideMinutaggio'); divContenitoreSlideMinutaggio.append(ControlliInSovrimpressione); ContenitoreVideoGuida.append(divContenitoreSlideMinutaggio);
...
function PosizioneAttualeDatiCI(tolleranza = 0) {
    const MinutaggioCorrente = VideoGuidaMinutaggioCorrente();
    return DatiCI.find((el) => {return (((el.Partenza - tolleranza) <= MinutaggioCorrente) && (MinutaggioCorrente <= (DatiCI[+DatiCI.indexOf(el) + 1] || {Partenza: totDurataVideoGuida}).Partenza))});
}
...
function EscludiRipristinaTraccia(IDUtente, EscludiRipristina = "") {
    ...
    function CambiaIconaVolume(Icona) {
        if (pulEscludiRipristinaTraccia) {pulEscludiRipristinaTraccia.className = "btn btn-default btn-xs fa fa-volume-" + Icona;}
    }

    if (TracceEscluse.includes(IDUtente)) {
        /* Riattiva la traccia */
        if (EscludiRipristina == "Escludi") {CambiaIconaVolume("off"); return;}
        ...
        CambiaIconaVolume("up");

    } else {
        /* Esclude la traccia */
        if (EscludiRipristina == "Ripristina") {CambiaIconaVolume("up"); return;}
...
function CreazioneClipPrimoCaricamento(DatiClipAudio) {
    ...
    if (CondizionePulsanteSwitchColonnaInternazionale) {SwitchColonnaInternazionale(); setTimeout(() => {pulSwitchColonnaInternazionale.style.display = "";}, 500); if (pulEscludiTracciaCI = document.getElementById('EscludiRipristinaTracciaCI')) {pulEscludiTracciaCI.onclick = pulSwitchColonnaInternazionale.onclick;}}
...
function GestioneEventoELTCliccato(e) {
    ...
    const NumeroAudio = +e.currentTarget.dataset.RiferimentoRegistrazione, ELT = document.getElementById('ELTReg' + NumeroAudio), X = e.clientX || e.touches[0].clientX;
    ...
    if ((!StoRegistrando) && ( (Righello.dataset.DisattivaClick == "no") || ((StrumentoMouse == toolStandard) && ELTDaSpostare)) ) { // Attiva la finestra delle opzioni, il trascinamento col mouse o la selezione multipla se il righello è attivo oppure se era stata selezionata almeno una clip
        switch(StrumentoMouse) {
            case toolStandard:    SelezionaESpostaELT(ELT, NumeroAudio, X); break;
            case toolDividiClip:  DividiClipELT(ELT, NumeroAudio, X); break;
            case toolEscludiClip: EscludiRipristinaClip(NumeroAudio); break;
        }
    }
}

function SelezionaESpostaELT(ELT, NumeroAudio, X) {
    ELTCliccato = ELT;
...
function DividiClipELT(ELT, NumeroAudio, X) {
    ELTCliccato = ELT;
...
function SelezionaTutteLeClipDiUnUtente(e) {
    ...
    if (!StoRegistrando && DatiAudioRegistrato_Utente[ID_Utente]) {
        DatiAudioRegistrato_Utente[ID_Utente].forEach((da) => {
            ...
            if (ELT && !ELTDaModificare.includes(ELT)) {SelezionaESpostaELT(ELT, da.numero, 0);}
        });
    }
}
