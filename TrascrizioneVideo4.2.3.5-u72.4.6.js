...
/*		case "[": T.innerHTML = T.innerHTML.slice(0, PosizioneCursore) + VisualizzaMinutaggioAttuale() + "] " + T.innerHTML.slice(PosizioneCursore); T.selectionEnd = PosizioneCursore; setTimeout(() => {T.selectionStart = T.selectionEnd = T.innerHTML.length;}, 100); return 2;
		...
		case "Enter":
            ...
            function MandaACapoEAttivaSceltaPersonaggio() {
                ...
                NodoBattuta.after(RigaCompleta.slice(PosizioneCursore));
                /* NodoBattuta.after(document.createElement('br'));
                T.childNodes[NumNodoBattuta + 1].after("\n" + RigaCompleta.slice(PosizioneCursore));*/
                PosizionaCursore(T.childNodes[+NumNodoBattuta + 1], 0);
                MandaA_AttivaSceltaPersonaggio();
            }
...
/*     if (Personaggio) { */
        NodoCursore.textContent = "\n" + Personaggio + ": " + NodoCursore.textContent;
...
function SalvaCopione(Textarea, AggiornaTesto = false) {
    ...
    if (AggiornaTesto) {FunzioniCopione.Visualizza();}
