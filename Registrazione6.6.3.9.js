...
function AggiungiDoppiatoreCandidatoNelCast(e) {
    ...
    function AttivaOpzioneTracciaRuoliDaAssegnare(e) {
        ...
        var divPulsantiFinali = CreaElemento('div', PannelloOpzioni.id + 'PulsantiFinali', PannelloOpzioni.id); divPulsantiFinali.className = 'panel-footer'; divPulsantiFinali.style.height = "60px";
            const pulAnnulla = CreaElemento('a', PannelloOpzioni.id + 'Annulla', td.id, "<span class='fa fa-times'></span> " + strAnnullalemodifiche); pulAnnulla.className = "btn btn-default"; pulAnnulla.iStyle({position: "absolute", left: "15%"});
                  pulAnnulla.onclick = AnnullaAggiungiDoppiatore;
                            
            const pulSalva = CreaElemento('a', PannelloOpzioni.id + 'Salva', td.id, "<span class='fa fa-arrow-right'></span> " + strProcedi); pulSalva.className = "btn btn-success"; pulSalva.iStyle({position: "absolute", right: "15%"});
...
function ContrassegnaClipDaAscoltare(datiAudio) {
    if (datiAudio.daAscoltare) {
        ...
        if (datiAudio.buffer) {
            const SampleRate = audioContext.sampleRate, s = datiAudio.buffer.getChannelData(0).slice(datiAudio.taglioIniziale * SampleRate, datiAudio.taglioFinale * SampleRate);
            const secondifineultimabattuta = s.findLastIndex(v => v > (TrattamentoClip.AutoTaglioIniziale.SogliaDB / GuadagnoPrincipale[datiAudio.numero].gain.value)) / SampleRate;
