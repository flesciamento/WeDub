...
function AttivaRegistrazione_TimeUpdate() {
    ...
    const opzAscoltaClipDuranteRegistrazione = checkAscoltaClipDuranteRegistrazione.checked || ColonnaInternazionaleAttivata;
...
function VerificaFunzioneAlTerminePrecaricamento() {
    if (FunzioneAlTerminePrecaricamento) {
        ...
        document.getElementById('dbg').innerHTML += "Clip precaricate: " + (+ContatoreClipPrecaricate + 1) + ", Clip da precaricare: " + totClipDaPrecaricare + "<br />";
...
function GeneraBufferCI(datiAudio, buffer) {
    ...
    dbg.innerHTML += "Caricamento buffer colonna " + datiAudio.numspezzoneCI + "<br />";
    ...
    if (SpezzoneSuccessivo = SpezzoniAudioCI[+datiAudio.numspezzoneCI + 1]) {
        dbg.innerHTML += "Caricamento buffer successivo colonna " + datiAudio.numspezzoneCI + "<br />";
        CaricaAudio(0, {Registrazione: SpezzoneSuccessivo}, 'arraybuffer',
            (Contenuto) => {
                audioContext.decodeAudioData(Contenuto).then(
                    (bufferSucc) => {
                        dbg.innerHTML += "Buffer successivo colonna " + datiAudio.numspezzoneCI + " caricato.<br />";
                        ...
                        dbg.innerHTML += "VerificaFunzioneAlTerminePrecaricamento() colonna " + datiAudio.numspezzoneCI + "<br />";
                        VerificaFunzioneAlTerminePrecaricamento();
...
function PlayVideoGuida() {
    dbg = document.getElementById('dbg'); dbg.innerHTML = "";
    ...
    setTimeout(() => {
        if (RiproduzioneInCorso == false) {
            ...
            if (StoRegistrando == false) {
                dbg.innerHTML += "VerificaClipPrecaricate()<br />";
                if (VerificaClipPrecaricate(SecondiPrecaricamentoAlPlay) == false) {return;}
                dbg.innerHTML += "Superato VerificaClipPrecaricate()<br />";
                ElaboraClipDaRiprodurre();
                setTimeout(() => {
                    if (ModalitaStreaming) {
                ...
                if ((checkAscoltaClipDuranteRegistrazione.checked) || (opzFunzionalitaPreRegistrazione.value == "preroll") || ColonnaInternazionaleAttivata) {
                    if ((!checkAscoltaClipDuranteRegistrazione.checked) && (opzFunzionalitaPreRegistrazione.value != "preroll") && (ColonnaInternazionaleAttivata)) {DisattivaClipNonCI(true);} // Se è stata selezionata solo la colonna internazionale, vengono temporaneamente disattivate tutte le altre clip per fare in modo che carichi e riproduca soltanto le clip che riguardano la CI
	...
	AJAX("CaricaRegistrazione.php", "NumProgetto=" + encodeURIComponent(N) + "&NumProvino=" + encodeURIComponent(P) + "&SoloMieRegistrazioni=" + (ModalitaLightAttiva? "1" : "0") + "&IDCandidatoRuoliDaAssegnare=" + encodeURIComponent(RuoliDaAssegnare_CandidatoSelezionato),
		function (Dati) {
            ...
            setTimeout(() => {
                ...
                if (RiproduzioneInCorso && (!StoRegistrando || checkAscoltaClipDuranteRegistrazione.checked)) {VerificaClipPrecaricate(SecondiPrecaricamentoAlPlay);} // Verifica se deve precaricare e quindi riprodurre le clip appena caricate
                CheckUtentiAttivi();
            }, 250);
		}, "", "", true
	);
