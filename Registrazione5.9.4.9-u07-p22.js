...
function CaricaBufferAudio(Numero, FunzioneAlTermine = () => {}, SoloBuffer = false) {
    ...
    CaricaAudio(Numero, datiAudio, 'arraybuffer', (ClipAudio) => {
        audioContext.decodeAudioData(ClipAudio).then((buffer) => {
            ...
            if (DatiAudioRegistrato[Numero].numspezzoneCI) {GeneraBufferCI(DatiAudioRegistrato[Numero], buffer); return;}
            /**********************************************************************************************************************************/
...
function GeneraBufferCI(datiAudio, buffer) {
    ...
    console.log(SpezzoniAudioCI, SpezzoniAudioCI[+datiAudio.numspezzoneCI + 1]);

    if (SpezzoneSuccessivo = SpezzoniAudioCI[+datiAudio.numspezzoneCI + 1]) {
        console.log(datiAudio, SpezzoneSuccessivo);
...
function CaricaColonnaInternazionale(Dati) {
    ...
    Dati = {Registrazione: "ColonneAudio/263_1661301815_CIspezzone008.flac,ColonneAudio/263_1661301829_CIspezzone009.flac", volume: Dati.volume};
