...
function GestisciIntensitaEffetto(Numero, Effetto) {
    const Intensita = DatiAudioRegistrato[Numero].intensitaeffetti;

    switch (Effetto) {
        case "radio": FiltroBandaAR[Numero].frequency.value = 1500 / 0.5 * Intensita; break;
        case "ovattato": FiltroBandaAR[Numero].frequency.value = 800 / 0.5 * Intensita; break;
...
function CreaFinestraOpzioniClip(RiferimentoRegistrazione) {
    ...
    div = CreaElemento('div', ID_Opzioni + 'ContenitoreOpzioniBody', ID_Opzioni); div.className = "panel-body";
        ...
        function AttivaDisattivaEffetto(e) {
            ...
            const pulsantiEffetti = document.getElementsByName('pulEffetto'), totPulsantiEffetti = pulsantiEffetti.length, classe_default = "btn btn-default", id_slideEffetto = ID_Opzioni + "slideEffetto";
        
            /* Resetta graficamente gli effetti */
            for (var I = 0; (I < totPulsantiEffetti) && (pulsantiEffetti[I].className = classe_default); I++);
            EliminaElemento(id_slideEffetto);
            ...
            if (datiAudio.effetti != Effetto) {
                ...
                const slideEffetto = CreaElemento('input', id_slideEffetto, pulEffetto.id); slideEffetto.setAttribute("type", "range"); slideEffetto.setAttribute("min", 0.1); slideEffetto.setAttribute("max", 1); slideEffetto.setAttribute("step", 0.1);
                      slideEffetto.value = 0.5; slideEffetto.dataset.effetto = Effetto;
                      slideEffetto.onchange = (e) => {ELTDaModificare.forEach((clipELT) => {const Numero = clipELT.dataset.RiferimentoRegistrazione; DatiAudioRegistrato[Numero].intensitaeffetti = e.currentTarget.value; GestisciIntensitaEffetto(Numero, e.currentTarget.dataset.effetto);})};
                      slideEffetto.dispatchEvent(ev_cambiamento);
            }
