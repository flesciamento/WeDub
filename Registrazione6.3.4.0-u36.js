    ...
    function TerminaBufferCI() {
        ...
        if (TrovaSpezzoneVicino(datiAudio, 1)) {
            ...
            if (((+datiAudio.MinutaggioRegistrazione) + (+datiAudio.Durata)) < totDurataVideoGuida) {
                ...
                datiAudio.latenzaEventoAlPlay = {secondi: datiAudio.Durata - tempomixCIeOriginale, riduciSeClipNelMinutaggio: true};
            }
        }
...
async function PassaGradualmenteAllOriginale() {
    if (ColonnaInternazionaleAttivata && RiproduzioneInCorso) {
        PassaGradualmenteAllOriginale.t = 0;
        PassaGradualmenteAllOriginale.tmr = setInterval(() => {
            PassaGradualmenteAllOriginale.t += 0.001;
            VideoGuidaImpostaVolume(CambiaVolumeVideoGuida.volume / tempomixCIeOriginale * PassaGradualmenteAllOriginale.t);
            if (PassaGradualmenteAllOriginale.t >= tempomixCIeOriginale) {clearInterval(PassaGradualmenteAllOriginale.tmr);}
        }, 1);
...
PassaGradualmenteAllOriginale.t = 0; PassaGradualmenteAllOriginale.tmr = false;
...
function audiobufferToWav(buffer, sampleRate, opt) {
    ...
    return encodeWAV(EffettuaAutoTaglioIniziale(buffer, sampleRate), format, sampleRate, numChannels, bitDepth);
}

function EffettuaAutoTaglioIniziale(buffer, sampleRate) {
    if (opzTagliaSilenzi.checked) {
        taglioiniziale = buffer.indexOf(buffer.find((a) => {return a > AutoTaglioIniziale.SogliaDB;})) - (AutoTaglioIniziale.Tolleranza * sampleRate);
...
function CreaRegistrazione_wav() {
    ...
    var view = audiobufferToWav(buffer, sampleRate, {float32: true});
...
function DividiClip(Numero, secondiInizioTaglio, secondiFineTaglio, MessaggioAlTermine = strClipDivisa) {
