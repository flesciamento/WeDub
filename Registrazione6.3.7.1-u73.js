               ...
               CreaSlide('TaglioFinale',      strTaglioFinale,   taglioclip_diffmin, LunghezzaRegistrazione, 0.01, taglioclip_diffmin, LunghezzaRegistrazione, 0.01, strsecondi, ev_CambiaTaglioFinaleClip,    ev_CambiaTaglioFinaleClip,   datiAudio.taglioFinale,   FunzioneOnFocusPerTagliInizialiEFinali);
                ...
                tr = CreaElemento('tr', ID_Opzioni + 'TabellaOpzioniRigaVarie', Tabella.id);
                    ...
                    const tdTrattamentoAudio = CreaElemento('td', ID_Opzioni + 'tdTrattamentoAudio', tr.id);
                        const pulRiduciRumore = CreaElemento('div', ID_Opzioni + 'pulRiduciRumore', tdTrattamentoAudio.id, "Riduzione rumore (test)"); pulRiduciRumore.className = "btn btn-default"; pulRiduciRumore.dataset.RiferimentoRegistrazione = RiferimentoRegistrazione;
                        pulRiduciRumore.onclick = async (e) => {
                            divVetro.iStyle({display: "inline", opacity: 0});
                            const pulsante = e.currentTarget, Numero = pulsante.dataset.RiferimentoRegistrazione, da = DatiAudioRegistrato[Numero], b = da.buffer.getChannelData(0), lunghezzaBuffer = b.length, SampleRate = da.buffer.sampleRate, LimiteSecondiSogliaRumore = SampleRate / RiduzioneRumore.FrazioneSecondoSogliaRumore                            
                            StoppaAutomaticamenteAscoltoInSolo(da);
                    ...
                    const pulAnnulla = CreaElemento('a', ID_Opzioni + 'Annulla', td.id, "<span class='fa fa-times'></span> " + strAnnullalemodifiche); pulAnnulla.className = "btn btn-default";
                          ...
                          pulAnnulla.addEventListener('click',
                            function (e) {
                                ...
                                /* Buffer */
                                if (bufferoriginale) {DatiAudioRegistrato[Numero].buffer = bufferoriginale;}
                ...
                td = CreaElemento('td', ID_Opzioni + 'CasellaSalva', tr.id); td.className = "text-right";
                    const pulSalva = CreaElemento('a', ID_Opzioni + 'Salva', td.id, "<span class='fa fa-check'></span> " + strSalva); pulSalva.className = "btn btn-success";
                          ...
                          pulSalva.addEventListener('click', function (e) {
                                ...
                                if ((AudioTrattato = document.getElementById(ID_Opzioni + "inputAudioTrattato")) && (AudioTrattato.checked) && (NuovoAudioTrattato)) {
                                    const da = DatiAudioRegistrato[RiferimentoRegistrazione];
                                    da.Rimosso = true;

                                    const NuovaRegistrazione = NuovoAudioTrattato.slice();
                                    AJAX("TrattamentoAudio_ClonaClip.php", CreaParametri({N: da.NumeroUnivoco, Registrazione: new File([NuovaRegistrazione])}), () => {OpzioniClip(RiferimentoRegistrazione, false, true);}, strSalvataggioInCorso, "", true, true);

                                    /* MinutaggioUltimaRegistrazione = da.MinutaggioRegistrazione;
                                    ffmpeg_TotaleProcessi = 2; ffmpeg_FunzioneAlTermineProcessi = SalvaNuovaRegistrazione;
                                    ffmpeg_Processi = ["-i clip1 output." + formatoQualitaAlta, "-i clip1 -lavfi \"showwavespic=s=3000x200:colors=blue\" onda.gif"];
                                    initWorker();
                                    const ConversioneWav = audiobufferToWav(da.buffer, da.buffer.sampleRate, {float32: true, notrattamento: true});
                                    sampleAudioData = new Uint8Array(ConversioneWav);
                                    runCommand(ffmpeg_Processi[0]);
                                    setTimeout(() => {runCommand(ffmpeg_Processi[1]);}, 1000);
                                    Messaggio(strSalvataggioInCorso); ComparsaBarraCaricamento();
                                    BC.style.transition = "all 10s"; VisualizzaProgressoBarraCaricamento(BC, 0.5); */
