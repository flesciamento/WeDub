        ...
        (Dati) => {
            ...
            CreaListaMenu(DatiPersonaggi, inputSceltaPersonaggio,
                ...
                (PersonaggioDaCancellare) => {
                    if (confirm(strConfermaCancellaNomePersonaggio)) {AJAX("TrascrizioneVideo_EliminaPersonaggioDaLista.php", "N=" + encodeURIComponent(N) + "&NomePersonaggio=" + encodeURIComponent(PersonaggioDaCancellare), () => {AttivaSceltaPersonaggio(T, FunzioneAllaSceltaPersonaggio, PersonaggioDaModificare);}, "", "", true);}
                }
            );
...
function AggiornaMinutaggio(e) {
    ...
    const LimiteMinimo = +(textareaprecedente.dataset.minutaggio || -2) + 2, LimiteMinimoRispettato = (LimiteMinimo < MinutaggioNuovo), LimiteMassimo = +(textareasuccessiva.dataset.minutaggio || totDurataVideoGuida) - 1, LimiteMassimoRispettato = (MinutaggioNuovo < LimiteMassimo);
