...
function audiobufferToWav(buffer, sampleRate, opt) {
    ...
    return encodeWAV(EffettuaAutoTaglioInizialeEFadeOut(buffer, sampleRate), format, sampleRate, numChannels, bitDepth);
}

function EffettuaAutoTaglioInizialeEFadeOut(buffer, sampleRate) {
    ...
    const lunghezzafadeout = tempofadeout * sampleRate, LunghezzaBuffer = buffer.length;
    for(s = lunghezzafadeout; s >= 0; s--) {
        bufferCanale[LunghezzaBuffer - s - 1] *= s / lunghezzafadeout;
    }
