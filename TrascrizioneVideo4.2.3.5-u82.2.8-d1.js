            ...
            if (T.innerText.trim() == "") {
                ...
                if ((e.key == "Backspace") && (PosizioneCursore <= /\S/.exec(NodoAttuale.textContent + ".").index)) {
                    ...
                    if (PresentePersonaggio) {
                        ModificaPersonaggio(tagNomePersonaggio, '');
                        return;
                    } else {
...
async function AttivaSceltaPersonaggio(T, FunzioneAllaSceltaPersonaggio, PersonaggioDaModificare = false) {
    ...
    AJAX("TrascrizioneVideo_CaricaListaPersonaggi.php", "N=" + encodeURIComponent(N), 
        (Dati) => {
            ...
            CreaListaMenu(DatiPersonaggi, inputSceltaPersonaggio,
                ...
                () => {
                    ...
                    if (NuovoPersonaggio) {
                        ...
                        if (PersonaggioDaModificare) {FunzioneAllaSceltaPersonaggio(T, PersonaggioDaModificare); return;} // Se non è stato scelto nulla, il personaggio non viene modificato.
                    }
            ...
            pulEliminaPersonaggio.onclick = (PersonaggioDaModificare? () => {console.log('Elimina personaggio', T, T.innerText); FunzioneAllaSceltaPersonaggio(T, '');} : "");
        }, "", "", true
    );
}
...
function ModificaPersonaggio(span, Personaggio) {
    const spanBattuta = span.parentElement, divContenitore = spanBattuta.parentElement, NumeroSpanBattuta = NumeroChildren(divContenitore, spanBattuta);
    VisualizzaBoxSceltaPersonaggio(false);
    AutoRiproduzioneVideoAttiva = true;

    if (Personaggio) {
        const NonEraPresentePersonaggio = (span.textContent.trim() == "");
        span.textContent = Personaggio;
        span.nextElementSibling.textContent = ": ";
        SalvaCopione(divContenitore, true, +NumeroSpanBattuta + (2 * NonEraPresentePersonaggio));

    } else {
        if (confirm(strVuoiEliminarePersonaggio)) {span.nextElementSibling.textContent = ""; span.innerHTML = "&nbsp;"; SalvaCopione(divContenitore, true, +NumeroSpanBattuta - 2, +spanBattuta.previousElementSibling.previousElementSibling.children.length + 3);} else {return;}
    }
}
