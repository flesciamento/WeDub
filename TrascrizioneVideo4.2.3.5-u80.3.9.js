...
async function ScriviPersonaggio(T, Personaggio) {
    ...
    for (let I = NumNodoSuccessivo; I < totNodi; I++) {
        ...
        RigaCompleta += NodoSuccessivo.textContent || "<br>";
        EliminaElemento(NodoSuccessivo);
    }

    const TestoPrimaInvio = RigaCompleta.slice(0, PosizioneCursore), TestoDopoInvio = "&nbsp;" + RigaCompleta.slice(PosizioneCursore).trim(); // lo spazio iniziale serve sia per evitare che, se il testo mandato a capo contiene i due punti, potrebbe essere scambiato per il nome di un personaggio, sia perché il testo dopo l'invio potrebbe essere vuoto e quindi serve almeno un elemento per creare il nodo di testo.
    ...
    if (Personaggio) {    
        ...
        br.insertAdjacentHTML('afterend', TestoDopoInvio);
...
function ModificaPersonaggio(T, Personaggio) {
    ...
    SalvaCopione(divContenitore, true);
}
...
function SalvaCopione(divContenitore, AggiornaTesto = false) {
    /* var Testo = "";
