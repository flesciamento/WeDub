...
function AttivaProgramma() {
    ...
    if (!ModalitaStreaming) {
        ...
        setTimeout(() => {
            ...
            if (RuoliDaAssegnare_NumeroTraccia !== false) {VariaAltezzaTraccia(RuoliDaAssegnare_NumeroTraccia, (+document.getElementById('divContenutoNomeTraccia' + RuoliDaAssegnare_NumeroTraccia).offsetHeight) + 15 + (65 * SonoCreatoreProgetto)); SpostaInBassoTracceSuccessive();}
            if (!SonoCreatoreProgetto) {AltezzaTracce = ((window.innerHeight) - (window.innerHeight * 0.4)) - NomiTracce.children[0].offsetHeight - 100; VariaAltezzaTracce();}
...
function SelezionaCandidatoRuoliDaAssegnare(ID) {
    ...
    AJAX("DatiUtente.php", "ID=" + encodeURIComponent(ID),
        function (Dati) {
            ...
            setTimeout(() => {console.log("sistemazione traccia", document.getElementById('divContenutoNomeTraccia' + RuoliDaAssegnare_NumeroTraccia).offsetHeight); VariaAltezzaTraccia(RuoliDaAssegnare_NumeroTraccia, (+document.getElementById('divContenutoNomeTraccia' + RuoliDaAssegnare_NumeroTraccia).offsetHeight) + 100 + (55 * SonoCreatoreProgetto)); SpostaInBassoTracceSuccessive();}, 100);
