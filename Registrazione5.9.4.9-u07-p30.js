...
function AttivaProgramma() {
    ...
    setTimeout(() => {
        ...
        VideoGuidaPause(); ImpostaStatoPlay(false); VideoGuidaPosizionati(0); slideMinutaggioAttuale.value = 0; VideoGuidaImpostaVolume(vol); slideVolumeVideoGuida.value = vol;
...
function startRecording() {
    ...
    if ((!ColonnaInternazionaleAttivata) && (slideVolumeVideoGuida.value > 0.8)) {slideVolumeVideoGuida.value = 0.2; CambiaVolumeVideoGuida();}
...
function ContoAllaRovescia() {
	...
	if (ContoAllaRovescia.Conteggio == 0) {
        ...
        if (opzFunzionalitaPreRegistrazione.value == "preroll") {
            if (!checkAscoltaClipDuranteRegistrazione.checked) {StoppaTutteLeRegistrazioni(true);}
    ...
    setTimeout(() => {
        if (RiproduzioneInCorso == false) {
            ...
            if (StoRegistrando == false) {
                ...
                if ((ColonnaInternazionaleAttivata) && (VerificaSpezzoniColonnaInternazionaleCaricati() == false)) {return;}
...
function VerificaSpezzoniColonnaInternazionaleCaricati() {
    const totSpezzoniCI = AudioBufferColonnaInternazionale.length, DalMinutaggio = VideoGuidaMinutaggioCorrente();
    for (let I = 0; I < totSpezzoniCI; I++) {
        const NumeroDatiAudioRegistrato = AudioBufferColonnaInternazionale[I].numero;
        if (ClipDaPrecaricare(NumeroDatiAudioRegistrato, DalMinutaggio, totDurataVideoGuida)) {
            DisabilitaSchermata(true); StopVideoGuida(); CaricaBufferAudio(NumeroDatiAudioRegistrato, PlayVideoGuida);
            return false;
        }
    }
...
function CreazioneClipPrimoCaricamento(Dati) {
    ...
    NumeroTotaleAudioCaricamentoIniziale = totClipAudio + (+ModalitaStreaming);
    ...
    if (ModalitaStreaming) {
        ...
        PrecaricaClip(0, SecondiPrecaricamentoAlPlay, () => {AggiornaCaricamentoClip(true);}, {FunzioneAlTermineSingoloPrecaricamento: () => {AggiornaCaricamentoClip(true);}, SoloBuffer: true});
...
function SwitchColonnaInternazionale(CI) {
    ...
    slideVolumeVideoGuida.value = (CI ? GuadagnoPrincipale[AudioBufferColonnaInternazionale[0].numero].gain.value : 1);
...
function CambiaVolumeCI() {
    AudioBufferColonnaInternazionale.forEach((datiAudioCI) => {
        GuadagnoPrincipale[datiAudioCI.numero].gain.value = Number(slideVolumeVideoGuida.value);
    })
}
...
function CaricaColonnaInternazionale(Dati) {
    ...
    SpezzoniAudioCI.forEach((SpezzoneAudio, NumSpezzone) => {
        CreaNuovaClipAudio({Registrazione: SpezzoneAudio, N: "CI" + NumSpezzone, ID_Utente: 'CI', MinutaggioRegistrazione: 75 * NumSpezzone, TaglioIniziale: 0, TaglioFinale: 75, DurataRegistrazione: 75, Guadagno: Dati.volume}, (datiAudio) => {datiAudio.numspezzoneCI = NumSpezzone; AudioBufferColonnaInternazionale.push(datiAudio);}, false);
    });
