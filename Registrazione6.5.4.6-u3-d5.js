...
function AggiornaClip(FunzioneAlTermine = false, FunzioneNuovaClip = RiposizionamentoAutomaticoELTRiferitoAllaRegistrazione) {
	AJAX("CaricaRegistrazione.php", "NumProgetto=" + encodeURIComponent(N) + "&NumProvino=" + encodeURIComponent(P) + "&SoloMieRegistrazioni=" + (ModalitaLightAttiva? "1" : "0") + "&IDCandidatoRuoliDaAssegnare=" + encodeURIComponent(RuoliDaAssegnare_CandidatoSelezionato),
		function (Dati) {
            if (document.getElementById(ID_Opzioni) && !FunzioneAlTermine) {return;} // Non aggiorna le clip con la finestra delle opzioni aperta, a meno che non è stata richiesta una funzione al termine
                ...
                tr = CreaElemento('tr', ID_Opzioni + 'TabellaOpzioniRigaVarie', Tabella.id);
                    ...
                    const tdTrattamentoAudio = CreaElemento('td', ID_Opzioni + 'tdTrattamentoAudio', tr.id);
                        const fieldsetTrattamentoAudio = CreaElemento('fieldset', tdTrattamentoAudio.id + "fieldset", tdTrattamentoAudio.id);
                            ...
                            CreaElemento('span', ID_Opzioni + 'spaziotrattamentoaudio', tdTrattamentoAudio.id, ' ');

                            const pulSpezzaBattute = CreaElemento('div', ID_Opzioni + 'pulSpezzaBattute', tdTrattamentoAudio.id, 'prova'); pulSpezzaBattute.className = "btn btn-default"; pulSpezzaBattute.dataset.RiferimentoRegistrazione = RiferimentoRegistrazione;
                            pulSpezzaBattute.onclick = async (e) => {
                                ...
                                if (NumeroAttacchi > 2) {
                                    ...
                                    for(A = 2; A < NumeroAttacchi; A += 2) {
                                        DuplicaClip(Numero, (ClipNuova) => {ClipNuova.taglioIniziale = attacchi[A]; ClipNuova.taglioFinale = attacchi[+A + 1]; console.log(A, ClipNuova.taglioIniziale, ClipNuova.taglioFinale, attacchi); SelezionaESpostaELT(ClipNuova.numero); console.log("ELTDaModificare", ELTDaModificare);});
                                    }
                                    divVetro.style.display = "none";
                                }
                            }
                    ...
                    const pulAnnulla = CreaElemento('a', ID_Opzioni + 'Annulla', td.id, "<span class='fa fa-times'></span> " + strAnnullalemodifiche); pulAnnulla.className = "btn btn-default";
                          ...
                          pulAnnulla.addEventListener('click',
                            function (e) {
                                ...
                                /** Ricarica le proprietà delle eventuali altre clip modificate, elimina quelle create e poi chiude la finestra **/
                                ...
                                if (totNuoveClipCreate == 0) {
                                    ...
                                    NuoveClipCreate.forEach((da) => {da.Rimosso = true; SalvaModificheClipAudio(da.numero, () => {if (++NuoveClipCestinate == totNuoveClipCreate) {OpzioniClip(Numero, false, false);}});});
                                }
                            }
                        );
