...
var FunzioniCopione = {
    ...
    Visualizza: function () {
        ...
        const FunzioniNomePersonaggio = ((FunzioniCopione.CopioneEditabile == 2) ? " style='cursor: pointer;' onclick='FunzioniCopione.FunzioneModificaPersonaggio(event)'" : ""), TestoEditabile = ((FunzioniCopione.CopioneEditabile > 0) ? " contenteditable='true' onkeydown='FunzioniCopione.FunzioneModificaBattuta(event);'" : "");

        /* Scrive e formatta il testo */
        var Testo = "";
        for (let I = 0; I < totBT; I++) {
            Testo += "<div id='RifCopione" + DatiCopione[I].NumID + "' data-numid='" + DatiCopione[I].NumID + "' data-minutaggio='" + DatiCopione[I].minutaggio + "' " + FunzioneOnClickBattuta + "><br>" + DatiCopione[I].testo + "</div>";
        }
        TestoGuida.innerHTML = "<span><span>" + Testo.replace(/\n/g, '<br>').replace(new RegExp('<br>([' + PatternRegexNomePersonaggio + ']+): ', 'g'), `</span></span><br style='line-height: 5px;'><span data-battuta='1' name=\"Battuta_$1\"><b ${FunzioniNomePersonaggio}>$1</b>:<span> </span><span ${TestoEditabile}>`) + "</span></span><br style='line-height: 4vh;' />";
        /******************************/
    ...
    /** @type {Boolean} indica se si può ridimensionare la finestra del copione **/
