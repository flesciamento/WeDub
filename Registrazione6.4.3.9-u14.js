...
function ScaricaMemoria(AncheSuccessivi = false) {
    ...
    for (let I = 0; I < totAudio; I++) {
        ...
        if (datiAudio.buffer && (((+datiAudio.MinutaggioRegistrazione) + (+datiAudio.taglioFinale) < MinutaggioVideo) || (AncheSuccessivi && ((+datiAudio.MinutaggioRegistrazione) > MinutaggioVideo)))) {
            datiAudio.audio = null; datiAudio.buffer = null; datiAudio.bufferdalegare = null; datiAudio.RichiestoCaricamentoBuffer = false; datiAudio.alPlay = [];
...
function StoppaClipAudio(datiAudio) {
    if (datiAudio.avviato) {
        ...
        console.log("Eliminati timer eventi alPlay per la clip", datiAudio);
