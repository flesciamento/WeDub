...
function GestisciImmissioneTesto(e) {
    ...
    if (Object.keys(TastiIndicazioni).includes(e.key)) {
        ...
        if (e.key == "F1") {
            ...
            const StringaConsiderata = T.textContent.slice(0, PosizioneCursore);
        ...
        T.textContent = T.textContent.slice(0, PosizioneCursore) + ((PosizioneCursore > 0) ? " " : "") + TestoIndicato.slice(0, TestoIndicato.indexOf('|')) + " " + T.textContent.slice(PosizioneCursore); PosizioneDelCursore(T, PosizioneCursore) + TestoIndicato.length + 1;
        SalvaCopione(T);
        return;
    }
	...
	switch (e.key) {
		case "(": T.textContent = T.textContent.slice(0, PosizioneCursore) + ")" + T.textContent.slice(PosizioneCursore); T.selectionEnd = PosizioneCursore; return 2;
/*		case "[": T.textContent = T.textContent.slice(0, PosizioneCursore) + VisualizzaMinutaggioAttuale() + "] " + T.textContent.slice(PosizioneCursore); T.selectionEnd = PosizioneCursore; setTimeout(() => {T.selectionStart = T.selectionEnd = T.textContent.length;}, 100); return 2;
        ...
        case "ArrowDown": ((PosizioneCursore > (T.textContent.length - 5)) && (textareasuccessiva = TrovaTextareaVicina(T, 1)) && (PosizionaCursore(textareasuccessiva, 0)) && (textareasuccessiva.focus())); break;
		case "Enter":
            ...
            if (textareasuccessiva = TrovaTextareaVicina(T, 1)) {
                if (Minutaggio < textareasuccessiva.dataset.minutaggio) {
                    ...
                    nuovatextarea.textContent = "<br>" + nuovatextarea.textContent;
...
async function ScriviPersonaggio(T, Personaggio) {
	const PosizioneCursore = PosizioneDelCursore();
    var NuovaPosizioneCursore = PosizioneCursore;
	
    VisualizzaBoxSceltaPersonaggio(false);
		
    if (Personaggio) {
        T.textContent = T.textContent.slice(0, PosizioneCursore) + Personaggio + ": " + T.textContent.slice(PosizioneCursore);
...
function SalvaCopione(Textarea) {
     AdattaAltezzaTextarea_MemorizzaTesto({currentTarget: Textarea}); AggiornaTestoGuida_slow();
	AJAX("TrascrizioneVideo_SalvaCopione.php", "NumID=" + encodeURIComponent(Textarea.dataset.numid) + "&N=" + encodeURIComponent(N) + "&Testo=" + encodeURIComponent(Textarea.textContent) + "&Minutaggio=" + encodeURIComponent(Textarea.dataset.minutaggio), "", "", strModificheSalvate, true);
}
...
async function EliminaBloccoTestoVuoto(e) {
    ...
    if (['Delete', 'Backspace'].includes(e.key) && (T.textContent == "") && (T.dataset.rifRigo > 1)) {
...
function MemorizzaTestoTextarea(T) {
    TrovaDatiCopioneID(T.dataset.numid).testo = T.textContent;
}
