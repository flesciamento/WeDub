...
const SecondiPrecaricamentoAlPlay = (ModalitaStreaming? 60 : 50), SecondiPrecaricamentoAlPlayPrimaDiRegistrare = 120;
...
function AttivaProgramma() {
    ...
    setTimeout(() => {
        ...
        if ((FineVideoGuida - InizioVideoGuida) < InizioVideoGuida) {slideZoom.value = 1000; setTimeout(CambiaZoom, 1000);}
    }, 200);
    ...
    AJAX("ElencoCandidatiRuoliDaAssegnare.php", "N=" + encodeURIComponent(N),
        function (Dati) {
            ...
            for (var I = 0; I < totCandidati; I++) {
                ...
                DatiDoppiatori[Dati[I].ID] = {nome: Dati[I].Nome, numeroTraccia: RuoliDaAssegnare_NumeroTraccia}; // Aggiorna l'elenco dei doppiatori per determinare se sono online e per altre funzionalità
            }
...
async function Posizionati(MinutaggioNuovo, RiabilitaLaSchermata = true) {
    ...
    async function RiattivaVideoGuida() {
        ...
        await pausa(500);
        if ((Math.round(VideoGuidaMinutaggioCorrente())) != (Math.round(MinutaggioNuovo))) {setTimeout(() => {Posizionati(+MinutaggioNuovo + 0.01, RiabilitaLaSchermata || (Righello.dataset.DisattivaClick == "no"));}, 1000); return;} // Se si incanta prova a riposizionarsi leggermente più avanti, riabilita la schermata se previsto o se era stata riabilitata in precedenza.
        if (stavoRiproducendo) {
            PlayVideoGuida();
        } else {
            RiabilitaLaSchermataSeRichiesto();
        }
        if (FunzioniCopione.CopioneVisualizzato) {FunzioniCopione.RiattivaCopione(); FunzioniCopione.AggiornaTestoGuida();}
    }
...
function DividiClip_puntatorerilasciato(e) {
    ...
    VisualizzazioneGraficaTaglioClip.OndaSonoraCompleta = false;
    if (e.touches) {CambiaTool(toolStandard);} // Onde evitare tocchi involontari, in caso di schermo touch torna sempre al tool standard.
    DisabilitaSchermata();
...
function CestinaClip(Numero, SenzaAnimazione = false) {
...
function CestinaClipRimosse(datiAudio, SenzaAnimazione = false) {
    if (datiAudio.Rimosso) {CestinaClip(datiAudio.numero, SenzaAnimazione);} // CestinaClip si occuperà anche di resettare il flag "ripristinati"
...
async function TermineCaricamentoClip() {
    ...
    setTimeout(() => {AzzeraBarraCaricamento(BC, ContornoBC, {loaded: 1, total: 1});}, 1000);
}
...
function GestioneEventoELTCliccato(e) {
    ...
    if ((Righello.dataset.DisattivaClick == "no") || ((ELTDaSpostare == ELT) && (StrumentoMouse == toolStandard))) { // Attiva il trascinamento col mouse se il righello è attivo oppure in ogni caso se l'ELT era stato in precedenza cliccato.
        ELTCliccato = ELT;
...
function RiposizionamentoAutomaticoClipSovrapposte(ELTConsiderato) {
    if (ELTDaSpostare == false) {
        ...
        DatiAudioRegistrato_Utente[datiAudioConsiderato.ID_Utente].forEach((datiAudio) => {
            ...
            ( (ELT) && (ELT.style.display != "none") && (ELT.style.visibility != "hidden") && (((+datiAudio.MinutaggioRegistrazione) + (+datiAudio.taglioIniziale)) < TermineClipConsiderata) && (MinutaggioPartenzaClipConsiderata < ((+datiAudio.MinutaggioRegistrazione) + (+datiAudio.taglioFinale))) && (ELTDaRiordinare[ELT.id] = true) && (NuoviELTDaRiordinare.push(ELT)) );
        });
...
function OpzioniClip(Numero, Apri, SalvaAllaChiusura) {
    ...
    if (Apri) {
        ...
        DisabilitaSchermata();
        ELT.className = "ELT transizione-morbida-no";
        ELT.style.zIndex = 5000000;
        ELTContenuto.style.backgroundColor = "rgba(255, 255, 255, 0.8)";
        StratoColore.style.transition = "";
        StratoColore.ontouchstart = GestioneEventoELTCliccato;
        ...
        VisualizzaELTNormale(Numero);
        ELTDaSpostare = false;
        ELT.className = "ELT transizione-morbida-si"; ELT.style.zIndex = ""; ELTContenuto.style.backgroundColor = ""; StratoColore.ontouchstart = "";
...
var TocchiPrec_Y = [0, 0];
function GestisciTouchStartLineaTemporale(e) {
    if (e.touches.length == 2) {
        TocchiPrec_Y[0] = e.touches[0].clientY;
        TocchiPrec_Y[1] = e.touches[1].clientY;
    }
}

function GestisciTouchMoveLineaTemporale(e) {
    if (e.touches.length == 2) {
        GestisciTouchMoveLineaTemporale.contatore++;
        const toccobasso = ((e.touches[0].clientY > e.touches[1].clientY) ? 0 : 1);
        if (((+GestisciTouchMoveLineaTemporale.PosizioneYPrec - 20) < e.touches[toccobasso].clientY) && (e.touches[toccobasso].clientY < (+GestisciTouchMoveLineaTemporale.PosizioneYPrec + 20))) {return;}
        
        const zoomprecedente = slideZoom.value;
        slideZoom.value += 1 - (2 * (e.touches[toccobasso].clientY < GestisciTouchMoveLineaTemporale.PosizioneYPrec));
        Messaggio(GestisciTouchMoveLineaTemporale.contatore++ + ": " + zoomprecedente + " - " + slideZoom.value);
        if (zoomprecedente != slideZoom.value) {CambiaZoom();}
        GestisciTouchMoveLineaTemporale.PosizioneYPrec = e.touches[toccobasso].clientY;
    }
}
GestisciTouchMoveLineaTemporale.PosizioneYPrec = 0; GestisciTouchMoveLineaTemporale.contatore = 0;
/****************************************/
...
function VideoGuidaPronto() {
    ...
    if (!ModalitaStreaming) {
        ...
        AltreFunzioniVisualizzaCopioneWeDub = () => {if (RiproduzioneInCorso) {FunzioniCopione.AttivaTestoGuida();} else {FunzioniCopione.AggiornaTestoGuida();}};
