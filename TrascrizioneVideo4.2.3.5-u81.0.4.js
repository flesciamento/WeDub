    ...
    if (Object.keys(TastiIndicazioni).includes(e.key)) {
        ...
        if (e.key == "F1") {
            ...
            if ((InfoCursore.nodo.previousElementSibling?.tagName != "BR") && (ElementoAttuale.previousElementSibling?.previousElementSibling?.tagName == "B")) {
                /* Se il personaggio c'era già, simula il click sul nome per attivarne la modifica */
...
async function ScriviPersonaggio(T, Personaggio) {
    ...
    if (Personaggio) {
        ...
        SalvaCopione(T, true, NumeroSpanBattuta, +NumeroSpan + 3); // +3 perché salta il br, lo span contenente lo spazio e arriva allo span del testo.
    }
...
function ModificaPersonaggio(T, Personaggio) {
    const divContenitore = T.parentElement.parentElement;
    VisualizzaBoxSceltaPersonaggio(false);
    ...
    if (Personaggio) {
        ...
        if (confirm(strVuoiEliminarePersonaggio)) {EliminaElemento(T.nextElementSibling); EliminaElemento(T);} else {return;}
    }

    SalvaCopione(divContenitore, true, false);
}
...
function SalvaCopione(divContenitore, AggiornaTesto = false, NumeroSpanBattutaCursore = false, NumeroSpanCursore = 0) {
    ...
    if (AggiornaTesto) {
        if (NumeroSpanBattutaCursore) {
            ...
            let s = document.getElementById(ID_ElementoContenitore).children[NumeroSpanBattutaCursore], spanContenitoreTestoAggiornato;
