...
function RiposizionamentoAutomaticoClipSovrapposte(ELTConsiderato) {
    if (ELTDaSpostare == false) {
        ...
        DatiAudioRegistrato_Utente[datiAudioConsiderato.ID_Utente].forEach((datiAudio) => {
            ...
            ( (ELT) && (ELT.style.display != "none") && (ELT.style.visibility != "hidden") && (((+datiAudio.MinutaggioRegistrazione) + (+datiAudio.taglioIniziale) + (+tolleranza)) < TermineClipConsiderata) && (MinutaggioPartenzaClipConsiderata < ((+datiAudio.MinutaggioRegistrazione) + (+datiAudio.taglioFinale) - tolleranza)) && (ELTDaRiordinare[ELT.id] = true) && (NuoviELTDaRiordinare.push(ELT)) );
        });
        /* Se tra gli ELT sovrapposti ce n'è uno più lungo, rifà l'elaborazione considerando quest'ultimo */
        NuoviELTDaRiordinare.sort((a, b) => {return DatiAudioRegistrato[b.dataset.RiferimentoRegistrazione].Durata - DatiAudioRegistrato[a.dataset.RiferimentoRegistrazione].Durata;});
        if (NuoviELTDaRiordinare[0] != ELTConsiderato) {console.log("Rimanda", NuoviELTDaRiordinare[0], ELTConsiderato); RiposizionamentoAutomaticoClipSovrapposte(NuoviELTDaRiordinare[0]); return;}
        
        /* Effettua il riposizionamento */
        const totELTDaRiordinare = NuoviELTDaRiordinare.length;
        console.log(totELTDaRiordinare, NuoviELTDaRiordinare);
