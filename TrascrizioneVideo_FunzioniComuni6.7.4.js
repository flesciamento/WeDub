const PosizioneDefaultCopione = {top: "15%", left: "8%", right: "63%", bottom: "45%", fontSize: "14px", lineHeight: "14px"}, AcquisisciIdentificativoPiattaforma = () => ((typeof IdentificativoPiattaforma != 'undefined') ? IdentificativoPiattaforma : "");
var DatiCopione = [];
...
const ImgAttesaTestoGuida = CreaElemento('a', 'ImgAttesaTestoGuida', 'ContenitoreTestoGuida', "<span class='fa fa-spin fa-spinner'></span>");

const PatternRegexNomePersonaggio = "A-Za-z0-9àéèìòù\\.\\(\\) ", RegexNomePersonaggio_CaratteriNonAmmessi = new RegExp('[^' + PatternRegexNomePersonaggio + ']', 'g'), RegexNomePersonaggio_CaratteriAmmessi = new RegExp('[' + PatternRegexNomePersonaggio + ']', 'g'), min_maxCaratteriPersonaggio = "{2,25}";
...
var FunzioniCopione = {
    ...
    AggiornaTestoGuida: function (MinutaggioAttuale = VideoGuidaMinutaggioCorrente()) {
        ...
        ( (PosizioneCopione) && (SezioneTesto = document.getElementById('RifCopione' + PosizioneCopione.NumID)) && (SezioneTesto.className = FunzioniCopione.ClasseEvidenziaBattuta) && (FunzioniCopione.ScorrimentoAutomatico) && (ContenitoreTestoGuida.scrollTo({top: SezioneTesto.offsetTop - (ContenitoreTestoGuida.offsetHeight / 3.5), behavior: 'smooth'})) );
    ...
    Visualizza: function (Partenza = 0) {
        ...
        if (Partenza == 0) {
            TestoGuida.innerHTML = "<br style='line-height: 20px;' />" + FunzioniCopione.FormattaTesto(Testo) + "<br id='brTermineCopione' style='line-height: 4vh;' />";
        } else {
    ...
    FormattaTesto: function (Testo) {
        const FunzioniNomePersonaggio = ((FunzioniCopione.CopioneEditabile == 2) ? " style='display: inline-block;' class='PersonaggioCliccabile' onclick='FunzioniCopione.FunzioneModificaPersonaggio(event)'" : ""), TestoEditabile = "name='ContenutoEditabile' onpaste='FunzioniCopione.GestisciIncolla(event);'" + ((FunzioniCopione.CopioneEditabile > 0) ? " contenteditable='true' onkeydown='FunzioniCopione.FunzioneModificaBattuta(event);' onfocus='FunzioniCopione.FunzioneOnFocusBattuta(event);'" + ((FunzioniCopione.CopioneEditabile == 2) ? " style='display: inline-block; min-width: 200px;'" : "") : "");
        const strRegexPersonaggio = `[${PatternRegexNomePersonaggio}]${min_maxCaratteriPersonaggio}`;
        return Testo.replace(/\n/g, '<br>').replace(new RegExp('<br>\\s+|<br>(?!' + strRegexPersonaggio + ':)', 'g'), `</span><br><b ${FunzioniNomePersonaggio}>&nbsp;</b><span></span><span ${TestoEditabile}>`).replace(new RegExp('<br>(' + strRegexPersonaggio + '):', 'g'), `</span></span><br style='line-height: 5px;'><span data-battuta='1' name=\"Battuta_$1\"><b ${FunzioniNomePersonaggio}>$1</b><span>: </span><span ${TestoEditabile}>`);
    },
    ...
    SalvaPosizioneCopione: function () {
        if ((FunzioniCopione.CopioneEditabile < 2) && (typeof NonCaricarePosizioneCopione == 'undefined')) {
            AJAX("TrascrizioneVideo_SalvaPosizioneCopione.php", "I=" + encodeURIComponent(AcquisisciIdentificativoPiattaforma()) + "&top=" + encodeURIComponent(ContenitoreTestoGuida.style.top) + "&left=" + encodeURIComponent(ContenitoreTestoGuida.style.left) + "&right=" + encodeURIComponent(ContenitoreTestoGuida.style.right) + "&bottom=" + encodeURIComponent(ContenitoreTestoGuida.style.bottom) + "&fontSize=" + encodeURIComponent(ContenitoreTestoGuida.style.fontSize), "", "", "", true);
        }
    },
    /******************************************************/
    ...
    Eventi: function (Attiva) {
        const OpzSposta_TestoGuida = document.getElementById('OpzSposta_TestoGuida'), OpzRiduciTesto_TestoGuida = document.getElementById('OpzRiduciTesto_TestoGuida'), OpzAumTesto_TestoGuida = document.getElementById('OpzAumTesto_TestoGuida'), OpzFinestraAParte = document.getElementById('OpzVisualizzaAltraFinestra_TestoGuida'), OpzRicercaBattuta = document.getElementById('OpzRicercaBattuta_TestoGuida');
        ...
        OpzEvidenzia_TestoGuida.onmousedown = (e) => {e.stopPropagation();}; OpzEvidenzia_TestoGuida.onchange = (Attiva? FunzioniCopione.EvidenziaPersonaggio : '');
        OpzRicercaBattuta.onmousedown = (Attiva? FunzioniCopione.PosizionatiBattutaPersonaggio : '');
    ...
    EvidenziaPersonaggio: function (e) {
        ...
        /* Si posiziona alla prima battuta del personaggio più vicina se non siamo già in una sua battuta e se non siamo in riproduzione né in modalità modifica */
        if (!e.currentTarget.dataset.nonautoposizionare && !RiproduzioneInCorso && (FunzioniCopione.CopioneEditabile == 0) && !document.querySelector(`.${FunzioniCopione.ClasseEvidenziaBattuta} [name="Battuta_${PersonaggioDaEvidenziare}"]`)) {FunzioniCopione.PosizionatiBattutaPersonaggio(e);}
    },
    /***************************/

    /** Posizionati alla battuta successiva del personaggio considerato **/
    PosizionatiBattutaPersonaggio: function (e) {
        e.stopPropagation();
        ...
        if (PersonaggioDaEvidenziare) {
            ...
            FunzioniCopione.FunzionePosizionaVideo(battute[0].parentElement.dataset.minutaggio); // Se non ha trovato la battuta successiva rispetto al minutaggio corrente, si posiziona sulla prima battuta del personaggio.
        }
    },
    /*********************************************************************/
    ...
    VisualizzaSuAltraFinestra: function (e) {
        ...
        function CaricaCopione(e) {
            ...
            if (e.data.VersioneStampabileCaricata) {
                finestra.postMessage({Copione: TestoGuida.innerHTML}, window.location.origin);
                window.removeEventListener('message', CaricaCopione);
            }
        }
    ...
    GestisciIncolla: async function (e) {
        ...
        await pausa(100);
        spanEditabile.innerHTML   = spanEditabile.innerHTML.replace(/&amp;/g, "&").replace(/&.+?;/g, ' ');
        spanEditabile.textContent = spanEditabile.innerText.replace(/\\/g, '/').replace(/<|>/g, '');
    },
    /********************************/
...
function AcquisisciTestoCopione(Elemento) {
    return Elemento.innerText.replace(/^\n/, '').rtrim().replace(/\\/g, '/').replace(/<+/g, '«').replace(/>+/g, '»');
}
/*************/