...
function AggiungiMarcatore(e) {
    ...
    if ((ID_Utente == IDUtente) || (SonoCreatoreProgetto)) {
        AJAX('SalvaNuovoMarcatore.php', `NumProgetto=${N}&NumProvino=${P}&Utente=${encodeURIComponent(IDUtente)}&Minutaggio=${encodeURIComponent(MinutaggioMarcatore)}&Testo=${encodeURIComponent(strMarcatoreDescrDefault)}`,
            (Dati) => {
                CaricaMarcatori();
                document.getElementById('Marcatore' + Dati.IDMarcatore).select();
            }, "", "", true
        );
    }
...
function DividiClip_puntatorerilasciato(e) {
    const ELT = e.currentTarget, MinutaggioPrimoClick = MinutaggioCliccatoNellaClip(ELT.dataset.posizionePuntatoreInizioTaglio), MinutaggioRilascio = MinutaggioCliccatoNellaClip(e.clientX), TrascinatoVersoDestra = MinutaggioPrimoClick < MinutaggioRilascio;
