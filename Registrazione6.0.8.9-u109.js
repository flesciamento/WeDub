...
function EscludiRipristinaTraccia(IDUtente) {
    ...
    if (TracceEscluse.includes(IDUtente)) {
        ...
        if (RiproduzioneInCorso) {
            const MinutaggioVideo = VideoGuidaMinutaggioCorrente();
            DatiAudioRegistrato_Utente[IDUtente].forEach((datiAudio) => {VerificaClipDaRiprodurre(datiAudio.numero, MinutaggioVideo);});
            FunzioneRiproduzioneClip = RiproduciClipInSync;
        }
        ...
        if (RiproduzioneInCorso) {
            DatiAudioRegistrato_Utente[IDUtente].forEach((datiAudio) => {StoppaClipAudio(datiAudio);});
        }
    }
}
...
function AnteprimaSelezioneClip(ELTConsiderato, ClientX, FunzioneTouchEnd = () => {}) {
    ...
    ELT.iStyle({zIndex: 500000000, pointerEvents: "auto"}); DisabilitaELT_tranne(ELT, true); VisualizzazioneGraficaTaglioClip.OndaSonoraCompleta = true;
...
function RiposizionamentoAutomaticoClipSovrapposte2(ELTAttuale) {
...
function RiposizionamentoAutomaticoClipSovrapposte(ELTConsiderato) {
    if (ELTDaSpostare == false) {
        ...
        DatiAudioRegistrato_Utente[datiAudioConsiderato.ID_Utente].forEach((datiAudio) => {
            ...
            ( (ELT.style.display != "none") && (((+datiAudio.MinutaggioRegistrazione) + (+datiAudio.taglioIniziale)) < TermineClipConsiderata) && (MinutaggioPartenzaClipConsiderata < ((+datiAudio.MinutaggioRegistrazione) + (+datiAudio.taglioFinale))) && (ELTDaRiordinare[ELT.id] = true) && (NuoviELTDaRiordinare.push(ELT)) );
        });
