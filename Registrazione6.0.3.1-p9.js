...
function AttivaProgramma() {
    ...
    totDurataVideoGuida = ((FineVideoGuida > InizioVideoGuida) ? FineVideoGuida : VideoGuidaDurataTotale());
    ...
    setTimeout(() => {
        ...
        VideoGuidaPause(); ImpostaStatoPlay(false); ImpostaVolumeVideoGuida(vol);
        VideoGuidaPosizionati(InizioVideoGuida); PosizionaCursore(InizioVideoGuida); AggiornaMinutaggioVideo(InizioVideoGuida);
...
function PosizionatiAlMinutoCliccato(e) {
	if ((Righello.dataset.DisattivaClick == "no") && (e.which == 1)) {
		...
		var MinutaggioNuovo = (MN = (X - ContenitoreRighello.offsetLeft) * totDurataVideoGuida / divContenutoTracce.clientWidth) * (MN > InizioVideoGuida);
		Posizionati(MinutaggioNuovo || InizioVideoGuida);
	}
}
...
function AggiornaMinutaggioVideo(Minutaggio) {
    ...
    if ((RiproduzioneInCorso) && (!StoRegistrando)) {
        if (Minutaggio >= totDurataVideoGuida) {StopVideoGuida(); return;}
...
function Posizionati(MinutaggioNuovo) {
    if (VideoGuidaMinutaggioCorrente() == MinutaggioNuovo) {return;}
