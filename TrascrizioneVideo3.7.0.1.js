var T = document.getElementById('Testo'), N = document.getElementById('N'), SP = document.getElementById('SceltaPersonaggi'), TestoGuida = document.getElementById('TestoGuida');
var tmrSospendi;

const SecondiRitornoIndietro = 2, MillisecondiRiprendiVideo = 1500;

for (var NomeTasto in TastiIndicazioni) {
	CreaElemento('li', 'li' + NomeTasto, 'infoTasti', "<a class='btn btn-default'><b>" + NomeTasto + "</b></a> " + TastiIndicazioni[NomeTasto]);
}
...
function SospendiVideo(e) {
	...
	switch (e.key) {
		...
		case "[": Minutaggio = new MinutiESecondi(VideoGuidaMinutaggioCorrente());
				  T.value = T.value.slice(0, PosizioneCursore) + Minutaggio.Minuti + ":" + Minutaggio.Secondi.toString().padStart(2, "0") + "] " + T.value.slice(PosizioneCursore); setTimeout(RiprendiDaPocoPrima, 1000); return 2;
		case "Control": if (e.location == 2) {RiprendiDaPocoPrima(6);}
		case "AltGraph": clearTimeout(tmrSospendi); break;
		case "Enter": clearTimeout(tmrSospendi); VideoGuidaPause(); T.removeEventListener('keyup', SospendiVideo); SP.style.opacity = 1; SP.style.zIndex = 10; T.addEventListener('keyup', ScegliPersonaggio); break; // [INVIO]
	}
...
function AltriAutomatismiTastiera(e) {
	...
	if (Object.keys(TastiIndicazioni).includes(e.key)) {
		...
		if (e.key == "F1") {
			...
			T.value = `${T.value.slice(0, PosizioneCursore)}\nScena ${(++contScena)} [${Minutaggio.Minuti}:${Minutaggio.Secondi.toString().padStart(2, "0")}]\n()\n${T.value.slice(PosizioneCursore)}`;
...
function SalvaCopione() {
	SalvataggioCopione(N.value, T.value, VideoGuidaMinutaggioCorrente(),
		function (Dati) {
			Messaggio("<img src='images/disk.png'>");
		}
	);
}
...
function CaricaCopione() {
	...
	CaricamentoCopione(N.value, 
		function (Dati) {
				...
				document.getElementById('ContenitoreVideoGuida').dataset.PosizioneInizialeVideo = Dati.PosizioneVideo;
				VideoGuidaCaricaEAvvia(PercorsoFile);
				VideoGuidaImpostaEventoPrimoCaricamento(PartenzaProgramma);
			}, strCaricamentoInCorso, strCopioneCaricato
	);
}

function PartenzaProgramma() {
	VideoGuidaPosizionati(document.getElementById('ContenitoreVideoGuida').dataset.PosizioneInizialeVideo);
	document.getElementById('Attendere').style.display = "none"; AttivazioneSchermata(true);
	VideoGuidaRimuoviEventoAlTermineCaricamento(PartenzaProgramma);
}

function AttivazioneSchermata(Attiva) {
	T.disabled = !Attiva; if (Attiva) {T.focus();}
	document.getElementById('ContenitoreVideoGuida').style.opacity = 1 * Attiva;
	VideoGuidaPause();
}
