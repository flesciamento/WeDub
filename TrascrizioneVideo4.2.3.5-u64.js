...
function GestisciImmissioneTesto(e) {
	const T = e.currentTarget, InfoCursore = PosizioneDelCursore(), PosizioneCursore = InfoCursore.p, NodoCursore = InfoCursore.nodo;
    var nuovatextarea;
    ...
    if (Object.keys(TastiIndicazioni).includes(e.key)) {
        ...
        if (e.key == "F1") {
            ...
            PosizionaCursore(NodoCursore, 0);
            nuovatextarea = T;
        ...
        NodoCursore.textContent = NodoCursore.textContent.slice(0, PosizioneCursore) + ((PosizioneCursore > 0) ? " " : "") + TestoIndicato.slice(0, TestoIndicato.indexOf('|')) + " " + NodoCursore.textContent.slice(PosizioneCursore); PosizionaCursore(NodoCursore, PosizioneCursore) + TestoIndicato.length + 1;
        SalvaCopione(T);
        return;
    }
...
/*		case "[": T.innerHTML = T.innerHTML.slice(0, PosizioneCursore) + VisualizzaMinutaggioAttuale() + "] " + T.innerHTML.slice(PosizioneCursore); T.selectionEnd = PosizioneCursore; setTimeout(() => {T.selectionStart = T.selectionEnd = T.innerHTML.length;}, 100); return 2;
        ...
        case "ArrowUp":   ((NumeroNodoCursore(InfoCursore.T, NodoCursore) == 0) && (precedentetextarea = TrovaTextareaVicina(T, -1)) && (precedentetextarea.focus())); break;
        case "ArrowDown": ((NumeroNodoCursore(InfoCursore.T, NodoCursore) == (InfoCursore.T.childNodes.length - 1)) && (textareasuccessiva = TrovaTextareaVicina(T, 1)) && (PosizionaCursore(textareasuccessiva.childNodes[0], 0)) && (textareasuccessiva.focus())); break;
		case "Enter":
            ...
            function MandaACapoEAttivaSceltaPersonaggio() {
                nuovatextarea = T;
                RigaCompleta = NodoCursore.textContent;
                const NumNodoCursore = NumeroNodoCursore(T, NodoCursore);
                NodoCursore.textContent = RigaCompleta.slice(0, PosizioneCursore);
                NodoCursore.after(document.createElement('br'));
                T.childNodes[NumNodoCursore + 1].after(RigaCompleta.slice(PosizioneCursore));
                PosizionaCursore(T.childNodes[NumNodoCursore + 2], 0);
                AttivaSceltaPersonaggio();
            }
            ...
            if (textareasuccessiva = TrovaTextareaVicina(T, 1)) {
                if (Minutaggio < textareasuccessiva.dataset.minutaggio) {
                    ...
                    PosizionaCursore(nuovatextarea.childNodes[1], 0);
                    AttivaSceltaPersonaggio();
                }
            } else {
    ...
    async function AttivaSceltaPersonaggio() {
        await pausa(100);
...
async function ScriviPersonaggio(T, Personaggio) {
   ...
   // var InfoCursore = PosizioneDelCursore(); console.log("ScriviPersonaggio", InfoCursore);  Aggiorna le info sul cursore
    ...
    /* InfoCursore = PosizioneDelCursore(); console.log("ScriviPersonaggio2", InfoCursore); */
...
function SalvaCopione(Textarea) {
    ...
    const Testo = Textarea.innerHTML;
