...
function GestisciImmissioneTesto(e) {
    ...
    UltimaPosizioneCursore = PosizioneCursore;
...
function OttieniInfoSulCursore() {
    return document.getSelection();
}

function PosizioneDelCursore() {
    const InfoCursore = OttieniInfoSulCursore(), T = InfoCursore.anchorNode.parentElement, Nodi = T.childNodes, PosizioneCursoreNodoAttuale = InfoCursore.anchorOffset;
    const NumNodoAttuale = Array.from(Nodi).indexOf(InfoCursore.anchorNode);

    var NumCaratteri = 0;
    for (let I = 0; I < NumNodoAttuale; I++) {
        NumCaratteri = (+NumCaratteri) + (+Nodi[I].textContent.length);
    }

    return ((+NumCaratteri) + (+PosizioneCursoreNodoAttuale));
}
