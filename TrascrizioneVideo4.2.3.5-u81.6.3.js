...
/*		case "(": NodoAttuale.textContent = NodoAttuale.textContent.slice(0, PosizioneCursore) + ")" + NodoAttuale.textContent.slice(PosizioneCursore); ; return 2;
        ...
        case "Backspace":
            if (T.innerText.trim() == "") {
                AJAX("TrascrizioneVideo_EliminaBloccoTesto.php", "NumID=" + encodeURIComponent(T.dataset.numid) + "&N=" + encodeURIComponent(N), () => {const divPrecedente_id = T.previousElementSibling?.id; CaricaCopione(() => {FunzioniCopione.AggiornaTestoGuida(); ( (div = document.getElementById(divPrecedente_id)) && (div.lastElementChild.querySelector("[contenteditable='true']").focus()) );});}, "", "", true);
                return;
            } else {
                ...
                if ((e.key == "Backspace") && (PosizioneCursore <= /\S/.exec(NodoAttuale.textContent + ".").index)) {
                    ...
                    /* Se è la prima battuta del minutaggio, elimina l'eventuale personaggio mantenendo lo spazio iniziale */
                    if ((NumeroSpanBattuta <= 2) && (ElementoAttuale == spanBattuta.querySelector("[contenteditable='true']"))) {
                        tagNomePersonaggio.innerHTML = "&nbsp;";
                        if (EraPresentePersonaggio) {SalvaCopione(T, true, 0);}
                        return;
                    }
                    ...
                    SalvaCopione(T, true, NumeroSpanBattuta - (2 * EraPresentePersonaggio), (EraPresentePersonaggio? spanBattuta.previousElementSibling.children.length - 1 : NumeroSpan - 4));
                    return;
                }
            }
            break;
...
function ModificaPersonaggio(T, Personaggio) {
    const spanBattuta = T.parentElement, divContenitore = spanBattuta.parentElement, NumeroSpanBattuta = NumeroChildren(divContenitore, spanBattuta);
    VisualizzaBoxSceltaPersonaggio(false);
    AutoRiproduzioneVideoAttiva = true;

    if (Personaggio) {
        const NonEraPresentePersonaggio = (T.textContent.trim() == "");
        ...
        if (confirm(strVuoiEliminarePersonaggio)) {T.nextElementSibling.textContent = ""; T.innerHTML = "&nbsp;"; SalvaCopione(divContenitore, true, +NumeroSpanBattuta - 2, +spanBattuta.previousElementSibling.previousElementSibling.children.length + 3);} else {return;}
    }
}
...
function SalvaCopione(divContenitore, AggiornaTesto = false, NumeroSpanBattutaCursore = false, NumeroSpanCursore = 0) {
    const Testo = divContenitore.innerText.slice(1).rtrim().replace(/\\/g, '/'), NumID = divContenitore.dataset.numid; // slice(1) serve per eliminare il primo <br> di default
...
function AggiornaMinutaggio(e) {
    const frecciapremuta = e.currentTarget, sec = document.getElementById('SecondiMinutaggioRifCopione'), min = document.getElementById('MinutiMinutaggioRifCopione');
    ...
    const T = document.getElementById('RifCopione' + frecciapremuta.dataset.numid), textareaprecedente = T.previousElementSibling || ({dataset: {minutaggio: -2}}), textareasuccessiva = T.nextElementSibling || ({dataset: {minutaggio: totDurataVideoGuida}});
...
function focusTextarea(e) {
    const ElementoAttuale = e.currentTarget;
 /*   const T = e.currentTarget;
 ...
 */
    ...
    UltimaTextareaUtilizzata = ElementoAttuale.parentElement.parentElement;

    /* Visualizza i tool */
    const MinutaggioAttuale = new MinutiESecondi(UltimaTextareaUtilizzata.dataset.minutaggio);
    const freccias = document.getElementById('freccias'), frecciag = document.getElementById('frecciag');
    document.getElementById('MinutiMinutaggioRifCopione').innerText = MinutaggioAttuale.Minuti;
    document.getElementById('SecondiMinutaggioRifCopione').innerText = VisualizzaSecondi(MinutaggioAttuale.Secondi);
    freccias.dataset.numid = frecciag.dataset.numid = UltimaTextareaUtilizzata.dataset.numid;

    document.getElementById('TestoGuida').insertBefore(divInfoTasti, UltimaTextareaUtilizzata);
