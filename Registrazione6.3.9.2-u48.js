...
function VariaAltezzaTracce() {
    ...
    const NuovaPosizione = percAltezzaTracce, NuovaPosizioneContenitore = percAltezzaTracce + " + 52px)";
    ...
    if (UnitaDiMisura == "%") {const NuovaAltezzaTracciaPerc = "calc(100% - " + (ContenitoreLineaTemporale.offsetTop - 50) + "px)"; ContenitoreLineaTemporale.style.height = NuovaAltezzaTracciaPerc; document.getElementById('NomiTracce').style.height = NuovaAltezzaTracciaPerc;}
...
function AttivaProgramma() {
    ...
    if (!ModalitaStreaming) {
        ...
        setTimeout(() => {
            ...
            if (!SonoCreatoreProgetto && !SessioneOspite) {AltezzaTracce = window.innerHeight - NomiTracce.children[0].offsetHeight - 100; VariaAltezzaTracce();} // Ingrandisce lo spazio dedicato al video per i doppiatori
