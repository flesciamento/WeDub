...
function DisabilitaELT_tranne(Numero, Disabilita) {
    const totAudio = DatiAudioRegistrato.length, StyleDisplay = (Disabilita? "none" : ""), StyleHeight = (Disabilita? "0%" : "100%"); //StylePointerEvents = (Disabilita? "none" : "auto"), StyleOpacity = (1 * !Disabilita), StyleTransition = (Disabilita? "all 0.5s" : ""), StyleVisibility = (Disabilita? "hidden" : "");
    for (let I = 0; I < totAudio; I++) {
        if (I == Numero) {continue;}
        document.getElementById('ELTReg' + I).iStyle({display: StyleDisplay, height: StyleHeight});
    }
}
