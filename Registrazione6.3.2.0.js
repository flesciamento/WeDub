...
async function CestinaClip(Numero, SenzaAnimazione = false) {
    ...
    if (ELT) {
        if (SenzaAnimazione) {
            ELT.iStyle({visibility: "hidden", display: "none"});
        } else {
    ...
    if (totAudioDellaTraccia) {
        ...
        if (Cestino.dataset.ripristinati == "si") {
            for (let I = 0; I < totAudioDellaTraccia; I++) {
                if (CestinaClipRimosse(AudioDellaTraccia[I])) {await pausa(30);}
            }
...
function VisualizzaClipRimosse(datiAudio) {
    ...
    ELT.iStyle({visibility: "visible", display: ""});
...
function RiposizionamentoAutomaticoClipSovrapposte(ELTConsiderato) {
    if (ELTDaSpostare == false) {
        ...
        if ((ELTConsiderato.style.visibility == "hidden") || (ELTConsiderato.style.display == "none")) {ID_ELT_trovati.splice(ID_ELT_trovati.indexOf(ELTConsiderato.id), 1);} // Non tiene conto dell'elemento considerato se questo è stato cestinato.
