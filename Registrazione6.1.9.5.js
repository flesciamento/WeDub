...
function CambiaTool(Tool) {
    ...
    const M = document.getElementsByClassName('Marcatore'), totM = M.length, puntatoreEliminaMarcatore = ((Tool == toolMarcatore) ? "url(images/CursoreMouseLineaTemporale_toolMarcatore_elimina.png) 8 0, auto" : "");
    ...
    for (let I = 0; (I < totM); I++) {
        if (M[I].dataset.modificabile) {M[I].style.cursor = puntatoreEliminaMarcatore;}
    }
...
function DisabilitaSchermata(ImmagineAttesa) {
    ...
    if (FunzioniCopione.CopioneVisualizzato) {FunzioniCopione.DisattivaCopione();}
...
function RiabilitaSchermata(Aggiorna) {
	...
	EliminaImgAttesa();
    if (FunzioniCopione.CopioneVisualizzato) {FunzioniCopione.RiattivaCopione();}
