const SP = document.getElementById('SceltaPersonaggi'), inputSceltaPersonaggio = document.getElementById('inputNomePersonaggio'), TestoGuida = document.getElementById('TestoGuida'), divVetro = document.getElementById('Vetro');
var tmrSospendi, AutoRiproduzioneVideoAttiva = true;
...
for (var NomeTasto in TastiIndicazioni) {
	...
	CreaElemento('li', 'li' + NomeTasto, 'infoTasti', "<a class='btn btn-default' style='pointer-events: none;' data-tasto='" + NomeTasto + "'><b>" + NomeTasto + "</b></a> " + Testo).setAttribute('title', Descrizione);
}
...
/* 		case "Control": if (e.location == 2) {RiprendiDaPocoPrima(6);} */
		...
		case "Enter":
            ...
            function AttivaSceltaPersonaggio() {
                ...
                inputSceltaPersonaggio.value = ""; inputSceltaPersonaggio.disabled = true;
                ...
                AJAX("TrascrizioneVideo_CaricaListaPersonaggi.php", "N=" + encodeURIComponent(N), 
                    (Dati) => {
                        ...
                        CreaListaMenu(DatiPersonaggi, inputSceltaPersonaggio,
                            ...
                            () => {
                                const NuovoPersonaggio = inputSceltaPersonaggio.value.replace(/[^A-z0-9\.\(\) ]/g, '').trim();
...
function CreaNuovaTextarea(NumID, Minutaggio) {
    DatiCopione[NumID] = {minutaggio: Minutaggio, testo: ''};
    ...
    const tr = CreaElemento('tr', 'RigoBloccoTesto' + NumRigo, "ContenitoreTesto");
            td = CreaElemento('td', 'tdInfoMinutaggio' + NumRigo, tr.id); td.iStyle({width: "100px", backgroundColor: "white", textAlign: "center", verticalAlign: "top", fontSize: "10px"});
                    ...
                    function AggiornaMinutaggioTextarea(T, Minutaggio) {T.dataset.minutaggio = Minutaggio; DatiCopione[T.dataset.numid].minutaggio = Minutaggio;}
...
function AdattaAltezzaTextarea_MemorizzaTesto(e) {
    ...
    /* Memorizza il testo */
    DatiCopione[T.dataset.numid].testo = T.value;
}
...
window.onresize = () => {if (RidimensionaVideo) {setTimeout(() => {RidimensionaVideo();}, 500);}};
