...
const ManigliaSposta_TestoGuida = CreaElemento('div', 'ManigliaSposta_TestoGuida', 'ContenitoreTestoGuida', `<div style='width: 100%; text-align: center;'><div id='OpzSposta_TestoGuida' class="fa fa-navicon" style="cursor: move; position: absolute;"></div></div><div style='position: absolute; right: 0px;'><span class='fa fa-search'></span><select id='OpzEvidenzia_TestoGuida' style='font-size: 80%;'></select></div><span id='OpzRiduciTesto_TestoGuida' data-variazione='-1' style="font-size: 13px;" class="btn btn-default fa fa-font"><sup style="font-size: 5px;" class="fa fa-minus"></sup></span>&nbsp;<span id='OpzAumTesto_TestoGuida' data-variazione='1' class="btn btn-default fa fa-font"><sup style="font-size: 8px;" class="fa fa-plus"></sup></span>`), OpzEvidenzia_TestoGuida = document.getElementById('OpzEvidenzia_TestoGuida');
...
var FunzioniCopione = {
    ...
    Visualizza: function () {
        ...
        const FunzioniNomePersonaggio = ((FunzioniCopione.CopioneEditabile == 2) ? " style='cursor: pointer;' onclick='FunzioniCopione.FunzioneModificaPersonaggio(event)'" : ""), TestoEditabile = ((FunzioniCopione.CopioneEditabile > 0) ? " contenteditable='true' onkeydown='FunzioniCopione.FunzioneModificaBattuta(event);' onclick='FunzioniCopione.FunzioneOnClickBattuta(event);'" : "");

        /* Scrive e formatta il testo */
        var Testo = "";
        for (let I = 0; I < totBT; I++) {
            Testo += "<div id='RifCopione" + DatiCopione[I].NumID + "' data-numid='" + DatiCopione[I].NumID + "' data-minutaggio='" + DatiCopione[I].minutaggio + "' " + FunzioneOnClickBattuta + "><span><span" + TestoEditabile + "><br>" + DatiCopione[I].testo + "</span></span></div>";
        }
        TestoGuida.innerHTML = Testo.replace(/\n/g, '<br>').replace(new RegExp('<br>([' + PatternRegexNomePersonaggio + ']+):', 'g'), `</span></span><br style='line-height: 5px;'><span data-battuta='1' name=\"Battuta_$1\"><b ${FunzioniNomePersonaggio}>$1</b><span>: </span><span ${TestoEditabile}>`) + "<br style='line-height: 4vh;' />";
        /******************************/
    ...
    PosizionaVideo: function (e) {
        ...
        /* Rende cliccabile la battuta vuota se il copione è editabile */
        if ((FunzioniCopione.CopioneEditabile > 0) && (BloccoTestoCliccato.innerText.trim() == "")) {BloccoTestoCliccato.children[0].children[0].focus();}
    ...
    ModificaDimensioniTesto: function (e) {
        ...
        if (FunzioniCopione.CopioneEditabile < 2) {FunzioniCopione.SalvaPosizioneCopione();} // Non salva le impostazioni se stiamo modificando l'intero copione.
    },
