...
function GestisciImmissioneTesto(e) {
	const NodoAttuale = e.currentTarget, T = NodoAttuale.parentElement.parentElement, InfoCursore = PosizioneDelCursore(), PosizioneCursore = InfoCursore.p;
    var nuovatextarea;
    ...
    TestoPrec = NodoAttuale.textContent.trim();
...
/*		case "[": T.innerHTML = T.innerHTML.slice(0, PosizioneCursore) + VisualizzaMinutaggioAttuale() + "] " + T.innerHTML.slice(PosizioneCursore); T.selectionEnd = PosizioneCursore; setTimeout(() => {T.selectionStart = T.selectionEnd = T.innerHTML.length;}, 100); return 2;
        ...
        case "ArrowUp":   ((NumeroNodo(InfoCursore.T, NodoAttuale) == 0) && (precedentetextarea = TrovaTextareaVicina(T, -1)) && (precedentetextarea.focus())); break;
        case "ArrowDown": ((NumeroNodo(InfoCursore.T, NodoAttuale) == (InfoCursore.T.childNodes.length - 1)) && (textareasuccessiva = TrovaTextareaVicina(T, 1)) && (PosizionaCursore(textareasuccessiva.childNodes[0], 0)) && (textareasuccessiva.focus())); break;
		case "Enter":
            ...
            function MandaACapoEAttivaSceltaPersonaggio() {
                ...
                const NodoBattuta = NodoAttuale.parentElement, NumNodoBattuta = NumeroNodo(T, NodoBattuta);
    ...
    setTimeout(() => {if (TestoPrec != NodoAttuale.textContent.trim()) {RiprendiVideoAutomaticamente(T);}}, 100);
}
