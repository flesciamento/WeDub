...
function CaricaMarcatori() {
    ...
    AJAX('CaricaMarcatori.php', 'NumProgetto=' + N,
        (Dati) => {
            ...
            /* Disegna solo i nuovi marcatori */
            for (let I = 0; I < totMarcatori; I++) {
                const M = Dati[I];
                if (!document.getElementById('Marcatore' + M.NumMarcatore)) {DisegnaMarcatore(M.ID_UtenteMarcatore, M.MinutaggioMarcatore, M.TestoMarcatore, M.NumMarcatore);}
                CaricaMarcatori.Marcatori[M.NumMarcatore] = true;
            }

            /* Elimina i marcatori non più presenti */
            CaricaMarcatori.MarcatoriPrec.forEach((I, IDMarcatore) => {console.log('Marcatore', IDMarcatore); if (!CaricaMarcatori.Marcatori[IDMarcatore]) {EliminaElemento('Marcatore' + IDMarcatore);}});
            console.log(CaricaMarcatori.Marcatori);
        }, "", "", true
    );
}
    ...
    const Marcatore = CreaElemento('div', 'Marcatore' + IDMarcatore, 'Traccia' + DatiDoppiatori[ID_UtenteMarcatore].numeroTraccia); Marcatore.dataset.id = IDMarcatore; Marcatore.className = 'Marcatore'; Marcatore.style.cursor = (Modificabile? "url(images/CursoreMouseLineaTemporale_toolMarcatore_elimina.png) 8 0, auto" : "");
          Marcatore.style.left = (Minutaggio / totDurataVideoGuida * 100) + "%";
            ...
            const Casella = CreaElemento('input', 'input' + Marcatore.id, Marcatore.id); Casella.value = Stringa; Casella.readOnly = !Modificabile; Casella.style.pointerEvents = (Modificabile? "auto" : "none");
                  ...
                  Casella.onchange = (e) => {const C = e.currentTarget; C.value = C.value.trim(); if (C.value == "") {C.value = C.dataset.valoreprec;} C.size = C.value.length - 1; C.dataset.valoreprec = C.value; C.parentNode.setAttribute('title', C.value);};
                  Casella.onblur = (e) => {
