...
function DisegnaMarcatore(ID_UtenteMarcatore, Minutaggio, Stringa, IDMarcatore) {
    ...
    const Marcatore = CreaElemento('div', 'Marcatore' + IDMarcatore, 'Traccia' + DatiDoppiatori[ID_UtenteMarcatore].numeroTraccia); Marcatore.dataset.id = IDMarcatore; Marcatore.className = 'Marcatore'; Marcatore.style.cursor = (Modificabile? "url(images/CursoreMouseLineaTemporale_toolMarcatore_elimina.png) 8 0, auto" : "");
          Marcatore.style.left = (Minutaggio / totDurataVideoGuida * 100) + "%";
            ...
            const Casella = CreaElemento('input', 'input' + Marcatore.id, Marcatore.id); Casella.value = Stringa; Casella.readOnly = !Modificabile; Casella.style.pointerEvents = (Modificabile? "auto" : "none");
                  Casella.onfocus  = (e) => {e.currentTarget.select();};
                  Casella.onchange = (e) => {
                    const C = e.currentTarget;
                    C.value = C.value.trim();
                    if (C.value == "") {C.value = C.dataset.valoreprec;}
                    C.size = C.value.length - 1; C.dataset.valoreprec = C.value; C.parentNode.setAttribute('title', C.value);
                    CompattaMarcatori();
                  };
                  Casella.onblur   = (e) => {
...
function CompattaMarcatori() {
    ...
    for(let I = 0; I < totM; I++) {
        const divMarcatore = divM[I], traccia = +divMarcatore.parentNode.id.replace('Traccia', '');
        M[traccia] = (M[traccia] || []);
        M[traccia].push({ID: divMarcatore.id, x: divMarcatore.offsetLeft, larghezza: divMarcatore.offsetWidth});
    }

    M.forEach((Mtraccia) => {
        Mtraccia.sort((a, b) => {return a.x - b.x;});
        const totMtraccia = Mtraccia.length;
        for(let I = 0; I < totMtraccia - 1; I++) {
            const Marcatore = Mtraccia[I], MarcatoreSucc = Mtraccia[I + 1], divMarcatore = document.getElementById(Marcatore.ID);
            const ComprimiMarcatore = (((+Marcatore.x) + (+Marcatore.larghezza)) > (+MarcatoreSucc.x));
            divMarcatore.children[0].className = (ComprimiMarcatore? "Marcatore-verticale" : "");
            //divMarcatore.style.width = (ComprimiMarcatore? "10px" : ""); se si fa così non lo decomprime mai più.
        }
    });
}
/*********************/
