...
const VolumeVideoGuida = document.getElementById('VolumeVideoGuida');
...
const SecondiPrecaricamentoAlPlay = (ModalitaStreaming? 60 : 120), SecondiPrecaricamentoAlPlayPrimaDiRegistrare = 200;

const ID_Opzioni = 'OpzioniClip';

const formatoQualitaAlta = "flac", formatoQualitaMedia = "ogg";

const toolStandard = 0, toolDividiClip = 1, toolEscludiClip = 2, puntatoreTool = ["pointer", "url(images/CursoreMouseLineaTemporale_toolDividiClip.png) 8 0, auto", "url(images/CursoreMouseLineaTemporale_toolEscludiClip.png) 8 0, auto"];
...
var ColonnaInternazionale = false, ColonnaInternazionaleAttivata = false, bufferCI = [];
...
var ELTDaSpostare = false, ELTDaRiordinare = [];
...
var MinutaggiRighello = {
	...
	Disegna: function () {
		...
		for (var I = 0; I < LunghezzaRighello; I += this.StepRighello) {
...
function CambiaZoom() {
    ...
    tmrRidisegnaRighello = setTimeout(() => {MinutaggiRighello.Ridisegna();}, 1000);
}
...
function AttivaProgramma() {
    ...
    totDurataVideoGuida = VideoGuidaDurataTotale();
    ...
    if (!ModalitaStreaming) {
        ...
        const pulSchermoInteroVetro = CreaElemento('div', 'pulSchermoInteroSuVetro', divVetro.id, "<span class='fa fa-desktop'></span> " + strVisualizzaASchermoIntero);
              ...
              pulSchermoInteroVetro.onclick = function (e) {document.getElementById('pulSchermoIntero').click(); EliminaElemento(e.currentTarget);};
    }
    ...
    setTimeout(() => {
        ...
        VideoGuidaPause(); ImpostaStatoPlay(false); VideoGuidaPosizionati(0); slideMinutaggioAttuale.value = 0; VideoGuidaImpostaVolume(vol); VolumeVideoGuida.value = vol;
        document.getElementById('VideoGuida').style.pointerEvents = "none"; PlayPausaCliccandoSulVideo(true);
...
function CambiaTool(Tool) {
    ...
    divContenutoTracce.onmousemove = ((Tool == 1) ? AnteprimaCursore : AnteprimaCursoreSeVuoto);
...
function AttivaColonnaInternazionale_TimeUpdate() {
    playColonnaInternazionaleInSync();
    VideoGuidaRimuoviFunzioneAlTimeUpdate(AttivaColonnaInternazionale_TimeUpdate);
    VideoGuidaFunzioneAlTimeUpdate(FunzioneNormaleAlTimeUpdate);
    FunzioneNormaleAlTimeUpdate();
}
...
function AttivaRegistrazione_TimeUpdate() {
    ...
    const opzAscoltaClipDuranteRegistrazione = checkAscoltaClipDuranteRegistrazione.checked;
    FunzioneNormaleAlTimeUpdate = (ModalitaUltraLightAttiva? (opzAscoltaClipDuranteRegistrazione? AggiornaTimeline : AggiornaCursore) : (opzAscoltaClipDuranteRegistrazione? AggiornaCursoreConAnteprimaOndaSonoraEAscoltoClip_Analizzatore : AggiornaCursoreConAnteprimaOndaSonora_Analizzatore));
...
function slide_SpostatiAlMinutaggioSelezionato() {
    Posizionati(slideMinutaggioAttuale.value);
}
...
function PosizionatiAlMinutoCliccato(e) {
	if ((Righello.dataset.DisattivaClick == "no") && (e.which == 1)) {
        if (e.clientY > window.innerHeight - 50) {return;}
		var MN = 0;
		var X = e.clientX + window.scrollX;
		var MinutaggioNuovo = (MN = (X - ContenitoreRighello.offsetLeft) * totDurataVideoGuida / divContenutoTracce.clientWidth) * (MN > 0);
		Posizionati(MinutaggioNuovo);
	}
}
...
function AggiornaMinutaggioVideo(Minutaggio) {
    ...
    if (!StoRegistrando) {
        ...
        if ((differenza > 0.1) && (RiproduzioneInCorso)) {
...
function Posizionati(MinutaggioNuovo) {
    ...
    ContenitoreVideoGuida.dataset.stavoRiproducendo = (RiproduzioneInCorso? "si" : "no");
...
function RiattivaVideoGuida() {
    ...
    setTimeout(() => {
        if (ContenitoreVideoGuida.dataset.stavoRiproducendo == "si") {PlayVideoGuida();} else {RiabilitaSchermata(true);}
    }, 300);
}
...
function PrecaricaClip(DalMinutaggio, FinoAlMinutaggio, FunzioneAlTermine = false, AltreOpzioni = {FunzioneAlTermineSingoloPrecaricamento: () => {}, SoloBuffer: false}) {
    ...
    if (!FinoAlMinutaggio) {FinoAlMinutaggio = totDurataVideoGuida;}
    ...
    if ((totClipDaPrecaricare == 0) && (FunzioneAlTerminePrecaricamento)) {FunzioneAlTerminePrecaricamento();} // Lancia subito la funzione al termine del precaricamento se prevista
}

function ClipDaPrecaricare(Numero, DalMinutaggio, FinoAlMinutaggio) {
    const datiAudio = DatiAudioRegistrato[Numero];
    return ((!datiAudio.buffer) && (!datiAudio.Rimosso) && (datiAudio.MinutaggioRegistrazione < FinoAlMinutaggio) && (DalMinutaggio < ((+datiAudio.MinutaggioRegistrazione) + (+datiAudio.Durata))));
}

function CaricaBufferAudio(Numero, FunzioneAlTermine = () => {}, SoloBuffer = false) {
    const datiAudio = DatiAudioRegistrato[Numero];
    if (datiAudio.CaricamentoBufferRichiesto) {return;}

    datiAudio.CaricamentoBufferRichiesto = true;
    CaricaAudio(Numero, datiAudio, 'arraybuffer', (ClipAudio) => {
        audioContext.decodeAudioData(ClipAudio).then((buffer) => {
            ...
            if (FunzioneAlTerminePrecaricamento) {
                ...
                setTimeout(PrecaricaClipSuccessiva, 100 + (1500 * SistemaAttualeAndroid));
            }
...
function PrecaricaClipSuccessiva() {
    if (RiproduzioneInCorso || ModalitaStreaming) {
...
function RiabilitaSchermata(Aggiorna) {
    ...
    pulMessaggioVocale.abilita(MessaggiIstantaneiAttivi);
...
function DisabilitaElementiGenerali(Disabilita) {
    pulPlay.disabled = Disabilita; MinutaggioMinuti.readOnly = Disabilita; MinutaggioSecondi.readOnly = Disabilita; opzFunzionalitaPreRegistrazione.disabled = Disabilita; inputCountdownRegistrazione.disabled = Disabilita; slideMinutaggioAttuale.disabled = Disabilita;
...
function RegistraMessaggioVocale() {
    ...
    RegistraMessaggioVocale.tmr = setTimeout(MandaMessaggioVocale, 60000);
}
...
function SalvaMessaggioVocale(Contenuto) {
    ...
    RiattivaInterfacciaDopoRegistrazione(); pulMessaggioVocale.abilita(false);
...
function startRecording() {
    ...
    if ((!ColonnaInternazionaleAttivata) && (VolumeVideoGuida.value > 0.8)) {VolumeVideoGuida.value = 0.2; CambiaVolumeVideoGuida();}
...
function AvviaContoAllaRovesciaRegistrazione() {
    EliminaImgAttesa();
    if (checkAscoltaClipDuranteRegistrazione.checked) {
        const MinutaggioVideo = VideoGuidaMinutaggioCorrente();
        DisabilitaSchermata(true); PrecaricaClip(MinutaggioVideo, MinutaggioVideo + SecondiPrecaricamentoAlPlayPrimaDiRegistrare, DisabilitaSchermata);
    }
...
function AvviaVideoGuidaPreroll() {
    ...
    EliminaImgAttesa();
    VideoGuidaRimuoviEventoAlPlay(PlayVideoGuida); VideoGuidaImpostaEventoAlPlay(AvviaContoAllaRovesciaAlPlay);
    setTimeout(PlayVideoGuida, 800);
}

function AvviaContoAllaRovesciaAlPlay() {
    VideoGuidaRimuoviEventoAlPlay(AvviaContoAllaRovesciaAlPlay); VideoGuidaImpostaEventoAlPlay(PlayVideoGuida);
    ContoAllaRovescia();
    tmrContoAllaRovescia = window.setInterval(ContoAllaRovescia, 1000);
}
...
function ContoAllaRovescia() {
	...
	if (ContoAllaRovescia.Conteggio == 0) {
        ...
        if (opzFunzionalitaPreRegistrazione.value == "preroll") {
            if (!checkAscoltaClipDuranteRegistrazione.checked) {StoppaTutteLeRegistrazioni();}
...
function FermaRegistrazione() {
    ...
    try {regMediaRecorder.stop();} catch (err) {}
    ...
    /* Ferma il video e rimette la funzione al time update di default */
	StopVideoGuida();
    VideoGuidaRimuoviFunzioneAlTimeUpdate(FunzioneNormaleAlTimeUpdate);
...
/* function playRegistrazione(Numero, PartenzaRegistrazione, TermineRegistrazione) {
    const Audio = AudioVideoRegistrato[Numero], datiAudio = DatiAudioRegistrato[Numero], t = new Date().getTime();
    Audio.currentTime = PartenzaRegistrazione;
	Audio.play().then(function () {const t1 = new Date().getTime(); Audio.currentTime += ((t1 - t) / 1000);});

    if (datiAudio.taglioFinale < datiAudio.Durata) {datiAudio.tmrTerminaAudio = setTimeout(function () {Audio.pause();}, TermineRegistrazione * 1000);}
} */
...
function StoppaClipAudio(datiAudio) {
    if (datiAudio.avviato) {
        try {datiAudio.audio.stop();} catch(err) {}
...
function PlayVideoGuida() {
    if (VideoGuidaMinutaggioCorrente() >= totDurataVideoGuida) {RiproduzioneInCorso = true; Posizionati(0); return;}
    ...
    setTimeout(() => {
        if (RiproduzioneInCorso == false) {
            ...
            if (StoRegistrando == false) {
                ...
                setTimeout(() => {
                    if (ModalitaStreaming) {
                        setTimeout(() => {VideoGuidaImpostaVolume(0); ComandiPlayer.style.opacity = 0;}, 2000);
                        FunzioneVisualizzazioneTitoli = VisualizzaTitoliInSync;
                        intervalloControllaClipPrecaricate = setInterval(() => {VerificaClipPrecaricate(SecondiPrecaricamentoAlPlay, false);}, SecondiPrecaricamentoAlPlay * 1000);
                    }
                    RiabilitaSchermata();
                    VideoGuidaPlay();
                }, 100);
                ...
                if ((checkAscoltaClipDuranteRegistrazione.checked) || (opzFunzionalitaPreRegistrazione.value == "preroll")) {
            ...
            if (ColonnaInternazionaleAttiva()) {setTimeout(AttivaColonnaInternazionaleInSync, 100 + (500 * SistemaAttualeAndroid));}
        }
    }, 100);
}
...
function StopVideoGuida() {
	if (RiproduzioneInCorso) {
        ...
        VideoGuidaPause();
        if (ColonnaInternazionale) {ColonnaInternazionale.pause();}
...
function ColonnaInternazionaleAttiva() {
    return (ColonnaInternazionale && (ModalitaStreaming || ColonnaInternazionaleAttivata));
}
...
function RiprendiRiproduzione() {
    if (ColonnaInternazionaleAttiva()) {AttivaColonnaInternazionaleInSync();}
    ElaboraClipDaRiprodurre();
    if (ModalitaStreaming) {FunzioneVisualizzazioneTitoli = VisualizzaTitoliInSync;}
}

function StoppaTutteLeRegistrazioni() {
    DatiAudioRegistrato.forEach(StoppaClipAudio);
    ClipDaRiprodurre = [];
    if (ModalitaStreaming) {InterrompiTitoli();}
}

function AttivaColonnaInternazionaleInSync() {
    VideoGuidaRimuoviFunzioneAlTimeUpdate(FunzioneNormaleAlTimeUpdate);
    VideoGuidaFunzioneAlTimeUpdate(AttivaColonnaInternazionale_TimeUpdate);
}

function playColonnaInternazionaleInSync() {
    const VolumePrec = ColonnaInternazionale.volume; ColonnaInternazionale.volume = 0;
    ColonnaInternazionale.currentTime = VideoGuidaMinutaggioCorrente() + LatenzaAudio + (0.2 * SistemaAttualeAndroid);
    const t = new Date().getTime();
    ColonnaInternazionale.play().then(() => {ColonnaInternazionale.currentTime += (new Date().getTime() - t) / 1000; ColonnaInternazionale.onseeked = () => {setTimeout(() => {ColonnaInternazionale.volume = VolumePrec; ColonnaInternazionale.onseeked = "";}, 10);}});
}

function VerificaEventualeColonnaInternazionalePrecaricata() {
    if (ColonnaInternazionale) {
        const MinutaggioVideo = VideoGuidaMinutaggioCorrente();
        if (ColonnaInternazionale.duration > MinutaggioVideo) { // Verifica se la colonna internazionale non termina prima del minutaggio attuale.
            if (Math.round(ColonnaInternazionale.currentTime) != Math.round(MinutaggioVideo)) { // Verifica se la colonna internazionale era già in posizione, altrimenti la posiziona sul minutaggio attuale.
                DisabilitaSchermata(true);
                PosizionaColonnaInternazionale(MinutaggioVideo, PlayVideoGuida);
                return false;
            }
        }
    }
...
function PosizionaColonnaInternazionale(Minutaggio, FunzioneAlCaricamento) {
    ColonnaInternazionale.currentTime = Minutaggio;
    ColonnaInternazionale.onseeked = () => {
        ColonnaInternazionale.oncanplaythrough = () => {
            FunzioneAlCaricamento();
            ColonnaInternazionale.oncanplaythrough = "";
        }
        ColonnaInternazionale.onseeked = "";
    }
...
function VerificaClipPrecaricate(SecondiMinimiClipPrecaricate, PrecaricamentoLeggero = true) {
    ...
    if (PrecaricamentoLeggero) {setTimeout(() => {PrecaricaClip(MinutaggioClipPrecaricate);}, 100 + (5000 * SistemaAttualeAndroid));}
...
function VerificaClipDaRiprodurre(I, MinutaggioVideo) {
    ...
    ( (datiAudio.buffer) && (!datiAudio.Rimosso) && (!datiAudio.Escluso) && (!TracceEscluse.includes(datiAudio.ID_Utente)) && (MinutaggioVideo < ((+datiAudio.MinutaggioRegistrazione) + (+datiAudio.taglioFinale))) && (ClipDaRiprodurre.push(I) && CreaClipAudio(I)) );
}
...
function ev_CambiaTaglioInizialeClip(e) {
    ...
    const Numero = e.currentTarget.dataset.RiferimentoRegistrazione, IDELT_Opzioni = "OpzioniClipTabellaOpzioniRigaTaglio", datiAudio = DatiAudioRegistrato[Numero], sogliamassima = (datiAudio.taglioFinale - 1), condizione = (TaglioIniziale < sogliamassima);
    ...
    slideTaglioFinale.min = TaglioIniziale * 1 + 1; casellaTaglioFinale.min = slideTaglioFinale.min;
...
function ev_CambiaTaglioFinaleClip(e) {
    ...
    const Numero = e.currentTarget.dataset.RiferimentoRegistrazione, IDELT_Opzioni = "OpzioniClipTabellaOpzioniRigaTaglio", datiAudio = DatiAudioRegistrato[Numero], sogliaminima = Number(datiAudio.taglioIniziale) + 1, condizioneSogliaMinima = (TaglioFinale > sogliaminima), sogliamassima = Number(datiAudio.Durata), condizioneSogliaMassima = (TaglioFinale < sogliamassima);
    ...
    slideTaglioIniziale.max = TaglioFinale - 1; casellaTaglioIniziale.max = slideTaglioIniziale.max;
...
function DividiClip(Numero, secondiTaglio) {
    AJAX("DividiClip.php", "N=" + DatiAudioRegistrato[Numero].NumeroUnivoco + "&Taglio=" + secondiTaglio,
        function (Dati) {
            RiabilitaSchermata();
            AggiornaClip(() => {
                const ClipNuova = TrovaClip(Dati.N);
...
function EscludiRipristinaTraccia(IDUtente) {
    ...
    if (TracceEscluse.includes(IDUtente)) {
        ...
        pulEscludiRipristinaTraccia.className = "btn btn-default btn-xs fa fa-volume-up";

    } else {
        /* Esclude la traccia */
        TracceEscluse.push(IDUtente);
        pulEscludiRipristinaTraccia.className = "btn btn-danger btn-xs fa fa-volume-off";
        StoppaTutteLeRegistrazioni();
    }
...
function SalvaModificheClipAudio(Numero) {
...
function CreazioneClipPrimoCaricamento(Dati) {
    const totClipAudio = Dati.length;
    var I;

    NumeroTotaleAudioCaricamentoIniziale = totClipAudio + (+ModalitaStreaming) + ((LinkColonnaInternazionale != "") ? 1 : 0);

    if (LinkColonnaInternazionale != "") {CaricaColonnaInternazionale({Registrazione: LinkColonnaInternazionale, volume: 1});}
    ...
    if (ModalitaStreaming) {
        for (I = 0; I < totClipAudio; I++) {
            NumeroTotaleAudioCaricamentoIniziale += ClipDaPrecaricare(I, 0, SecondiPrecaricamentoAlPlay);
        }
        Messaggio(strCaricamentoPrimeClip);
        PrecaricaClip(0, SecondiPrecaricamentoAlPlay, () => {Messaggio(strAttendereFineCaricamento); AggiornaCaricamentoClip(true);}, {FunzioneAlTermineSingoloPrecaricamento: () => {AggiornaCaricamentoClip(true);}, SoloBuffer: true});
    ...
    if (!CaricamentoInizialeTerminato) {
        ...
        setTimeout(() => {
            if (ModalitaStreaming) {
                ...
                pulPlayIniziale.onclick = function (e) {CancMex(); AttivaScorciatoieDiTastiera_ModalitaStreaming(); pulPlayIniziale.onclick = ""; divVetro.style.display = "none"; PlayVideoGuida();};
                DisattivaMessaggiAttesa();
                AttivaInterfaccia();
	...
	AJAX("CaricaRegistrazione.php", "NumProgetto=" + encodeURIComponent(N) + "&NumProvino=" + encodeURIComponent(P) + "&SoloMieRegistrazioni=" + (ModalitaLightAttiva? "1" : "0") + "&IDCandidatoRuoliDaAssegnare=" + encodeURIComponent(RuoliDaAssegnare_CandidatoSelezionato),
		function (Dati) {
            ...
            setTimeout(() => {
                ...
                if (RiproduzioneInCorso) {VerificaClipPrecaricate(SecondiPrecaricamentoAlPlay);} // Verifica se deve precaricare e quindi riprodurre le clip appena caricate
                CheckUtentiAttivi();
            }, 250);
		}, "", "", true
	);
...
function SwitchColonnaInternazionale(CI) {
    ...
    VideoGuidaImpostaVolume((CI ? 0 : 1));
    lblVolumeVideoGuida.innerText = (CI ? strVolumeInternazionale : strVolumeFilmato);
    VolumeVideoGuida.value = (CI ? ColonnaInternazionale.volume : 1);
    VolumeVideoGuida.onchange = (CI ? CambiaVolumeCI : CambiaVolumeVideoGuida);
    ...
    if (RiproduzioneInCorso) {
        if (CI) {
            AttivaColonnaInternazionaleInSync();
        } else {
            ColonnaInternazionale.pause();
        }
    }
}
...
function CambiaVolumeCI() {
    ColonnaInternazionale.volume = Number(VolumeVideoGuida.value);
}
...
function CaricaColonnaInternazionale(Dati) {
    ...
    /* Arriverà così da database */
    Dati = {Registrazione: "ColonneAudio/260_1661250073_CIspezzone000.ogg,ColonneAudio/260_1661250073_CIspezzone001.ogg,ColonneAudio/260_1661250073_CIspezzone002.ogg,ColonneAudio/260_1661250073_CIspezzone003.ogg,ColonneAudio/260_1661250073_CIspezzone004.ogg,ColonneAudio/260_1661250073_CIspezzone005.ogg,ColonneAudio/260_1661250073_CIspezzone006.ogg,ColonneAudio/260_1661250073_CIspezzone007.ogg,ColonneAudio/260_1661250073_CIspezzone008.ogg,ColonneAudio/260_1661250073_CIspezzone009.ogg,ColonneAudio/260_1661250073_CIspezzone010.ogg,ColonneAudio/260_1661250073_CIspezzone011.ogg,ColonneAudio/260_1661250073_CIspezzone012.ogg,ColonneAudio/260_1661250073_CIspezzone013.ogg,ColonneAudio/260_1661250073_CIspezzone014.ogg,ColonneAudio/260_1661250073_CIspezzone015.ogg,ColonneAudio/260_1661250073_CIspezzone016.ogg,ColonneAudio/260_1661250073_CIspezzone017.ogg,ColonneAudio/260_1661250073_CIspezzone018.ogg", volume: 1};

    const SpezzoniAudioCI = Dati.Registrazione.split(','), totSpezzoniAudioCI = SpezzoniAudioCI.length;

    var NumSpezzone = 0;
    function CaricaSpezzoneSuccessivo() {
        CaricaAudio(0, {Registrazione: SpezzoniAudioCI[NumSpezzone]}, 'arraybuffer', (Contenuto) => {
            audioContext.decodeAudioData(Contenuto).then((buffer) => {
                console.log(SpezzoniAudioCI[NumSpezzone], buffer.getChannelData(0));
                bufferCI.push(buffer);
                if (++NumSpezzone < totSpezzoniAudioCI) {
                    CaricaSpezzoneSuccessivo();
                } else {
                    CreaNuovaClipAudio({Registrazione: "", NumeroUnivoco: "-1", ID_Utente: 'ColonnaInternazionale', MinutaggioRegistrazione: 0, TaglioIniziale: 0, DurataRegistrazione: 1, Guadagno: Dati.volume},
                        (datiAudio) => {
                            datiAudio.buffer = appendBuffer(bufferCI);
   ...
   /*  if (ModalitaStreaming) {
        CaricaAudio(0, Dati, 'blob', (Contenuto) => {
            ColonnaInternazionale = new Audio(URL.createObjectURL(Contenuto));
            ColonnaInternazionale.volume = Dati.volume;
            AggiornaCaricamentoClip(true);
        });

    } else {
        ColonnaInternazionale = new Audio(Dati.Registrazione);
        ColonnaInternazionale.volume = Dati.volume;
        setTimeout(() => {
            if (pulSwitchColonnaInternazionale) {
                SwitchColonnaInternazionale(false);
                setTimeout(() => {pulSwitchColonnaInternazionale.abilita(true);}, 500);
            }
            if (!CaricamentoInizialeTerminato) {AggiornaCaricamentoClip();}
        }, 2000);
    } */
...
function OttieniDurataAudio(Numero, Funzione, SecondaFunzione) {
    ...
    if ((datiAudio.Durata = datiAudio.durataMemorizzata) == 0) {
        CaricaBufferAudio(Numero, () => {
            datiAudio.audio = new AudioBufferSourceNode(audioContext, {buffer: datiAudio.buffer});

            /** Memorizza la durata **/
            datiAudio.Durata = datiAudio.audio.buffer.duration || 0;
	...
	if ( ( (DatiAudioRegistrato[RiferimentoRegistrazione].ID_Utente == ID_Utente) && (SolaVisione == false) ) || (SonoCreatoreProgetto)) {
        ...
        ELT.addEventListener('mousedown',
            function(e) {
                ...
                const ELTCliccato = e.currentTarget, NumeroAudio = ELTCliccato.dataset.RiferimentoRegistrazione;
                ...
                if ((Righello.dataset.DisattivaClick == "no") || ((ELTDaSpostare == ELTCliccato) && (StrumentoMouse == toolStandard))) { // Attiva il trascinamento col mouse se il righello è attivo oppure in ogni caso se l'ELT era stato in precedenza cliccato.
                    switch(StrumentoMouse) {
                        ...
                        case toolDividiClip:
                            DisabilitaSchermata();
                            DividiClip(NumeroAudio, (e.clientX + window.scrollX - ContenitoreRighello.offsetLeft - ELTCliccato.offsetLeft) / ELTCliccato.clientWidth * DatiAudioRegistrato[NumeroAudio].Durata);
...
function CreaFinestraOpzioniClip(RiferimentoRegistrazione) {
    ...
    div = CreaElemento('div', ID_Opzioni + 'ContenitoreOpzioniBody', ID_Opzioni); div.className = "panel-body";
        ...
        /***********************************************************************/
               ...
               CreaSlide('TaglioFinale',    strTaglioFinale,     1, LunghezzaRegistrazione, 0.01, 1, LunghezzaRegistrazione, 0.01, 'secondi', ev_CambiaTaglioFinaleClip,    ev_CambiaTaglioFinaleClip,   datiAudio.taglioFinale);
...
window.addEventListener("message", (e) => {
  if ((e.origin !== "https://wedub.altervista.org") || (StoRegistrando) || (Righello.dataset.DisattivaClick == "si")) {return;}
    ...
    if (Dati.progetto != N) {alert("Il messaggio si riferisce ad un altro progetto!"); return;}
...
function ScorciatoieTastiera(Tasto) {
    ...
    switch(Tasto.code) {
        ...
        case "ShiftRight": if (pulMessaggioVocale.style.pointerEvents == "auto") {RegistraMessaggioVocale_slow(); TastoPremuto = "Shift destro"; SpiegazioneTasto = strSpiegazioneTastoRegistraMessaggioIstantaneo;} else {return;} break;  // [Shift destro]
    }
...
function ScorciatoieTastiera_TastiSollevati(Tasto) {
    switch(Tasto.code) {
        ...
        case "ShiftRight": MandaMessaggioVocale(); break;  // [Shift destro]
    }
}
...
function VideoGuidaPronto() {
    ...
    setTimeout(() => {VideoGuidaCaricaEAvvia(PercorsoVideoGuida); VideoGuidaImpostaEventoPrimoCaricamento(AttivaProgramma);}, 500);
}
