...
const opzCanaleRegistrazione = document.getElementById('opzCanaleRegistrazione');
...
var EffettuatoAutoTaglioIniziale = false;
...
function OpacitaRighello(LivelloOpacita) {
	const divOpacitaTraccia = document.getElementsByName('divOpacitaTraccia'), totDivOpacitatraccia = divOpacitaTraccia.length, proprietaBackgroundColor = "rgba(255, 255, 255, " + (1 - LivelloOpacita).toString() + ")";
...
function AggiornaParametriRegistrazione() {
    if (registrazione) {
        registrazione.port.postMessage({modalita: "Monitoraggio", numCanale: opzCanaleRegistrazione.value, bufferSize: Number(dimensioneBuffer)});
    }
}
...
function RiproduciClipInSync(MinutaggioVideo) {
    ...
    ClipDaRiprodurre.forEach((I) => {
        ...
        if (datiAudio.alPlay) {clearTimeout(datiAudio.tmrEventoAlPlay); datiAudio.tmrEventoAlPlay = setTimeout(() => {datiAudio.alPlay(datiAudio);}, ((+SecondiPartenzaAudio) + (+datiAudio.latenzaEventoAlPlay.secondi) - (InizioClip * ClipNelMinutaggioAttuale * datiAudio.latenzaEventoAlPlay.riduciSeClipNelMinutaggio)) * 1000);}
    });
...
function GeneraBufferCI(datiAudio, buffer, FunzioneAlTermine = () => {}) {
    ...
    function TrovaSpezzoneVicino(ab, distanza) {
        return AudioBufferColonnaInternazionale[AudioBufferColonnaInternazionale.indexOf(ab) + distanza];
    }
    ...
    function TerminaBufferCI() {
        ...
        if (TrovaSpezzoneVicino(datiAudio, 1)) {
            /** Dimezza il volume dell'ultimo pezzo per mixarlo col successivo **/
            ...
            if (((+datiAudio.MinutaggioRegistrazione) + (+datiAudio.Durata)) < totDurataVideoGuida) {
                ...
                /* Attiva l'evento alPlay poco prima della fine della clip */
        ...
        console.log("Spezzone CI", datiAudio.numspezzoneCI, "minutaggio", new MinutiESecondi(datiAudio.MinutaggioRegistrazione), "durata", datiAudio.Durata);
...
function DisabilitaElementiGenerali(Disabilita) {
    ...
    SelezioneMicrofono.disabled = Disabilita; pulMonitorMic.abilita(!Disabilita); opzCanaleRegistrazione.disabled = Disabilita; selectQualitaRegistrazione.disabled = Disabilita;
    if (ContenitoreStrumenti) {ContenitoreStrumenti.abilita(!Disabilita);}
...
function RegistraMessaggioVocale() {
    ...
    if (QualitaAltaRegistrazione) {
        ...
        registrazione.port.postMessage({numCanale: opzCanaleRegistrazione.value, modalita: "Registrazione"});
    } else {
        DisconnettiWorkletRegistrazione();
...
function MandaMessaggioVocale() {
    ...
    if (RegistraMessaggioVocale.RegistrazionePartita) {
        ...
        if (QualitaAltaRegistrazione) {
            ...
            registrazione.port.postMessage({numCanale: opzCanaleRegistrazione.value, modalita: "AcquisisciRegistrazione"});
        } else {
...
function startRecording() {
    ...
    if (!ModalitaUltraLightAttiva) {
        ...
        c.width = 4000; c.height = CT.offsetHeight;
...
function CreaRegistrazione_mediaRecorder() {
    ...
    CaricaAudio(0, {Registrazione: URLBuffer}, 'arraybuffer', 
        function (Contenuto) {
            ...
            audioContext.decodeAudioData(Contenuto).then((buffer) => {
                var ConversioneWav = audiobufferToWav(buffer.getChannelData(opzCanaleRegistrazione.value), buffer.sampleRate);
...
function EffettuaTrattamentoAudio(buffer, sampleRate) {
    ...
    const lunghezzaBufferIniziale = buffer.length;
    var somma = 0;
    for(let I = 0; I < lunghezzaBufferIniziale; I++) {
        somma += buffer[I];
    }
    var media = somma / lunghezzaBufferIniziale;
    for(let I = 0; I < lunghezzaBufferIniziale; I++) {
        buffer[I] -= media;
    }
    ...
    if (opzTagliaSilenzi.checked) {
        ...
        MinutaggioUltimaRegistrazione += (taglioiniziale / sampleRate);
    }

    /** Taglia ultimo pezzo (elimina click del mouse e altri suoni non intenzionali al termine della registrazione) **/
    buffer = buffer.slice(0, buffer.length - (TrattamentoClip.tagliofinale * sampleRate));

    /** Fade-out **/
    const lunghezzafadeout = TrattamentoClip.tempofadeout * sampleRate, LunghezzaBuffer = buffer.length;
...
function PlayVideoGuida() {
    pulPlay.style.pointerEvents = "none"; Monitors.style.pointerEvents = "none"; // Evita il doppio click involontario
    ...
    pulPlay.style.pointerEvents = ""; Monitors.style.pointerEvents = "";
}
...
function Download(Numero) {
    const datiAudio = DatiAudioRegistrato[Numero], IDUtente = datiAudio.ID_Utente, M = new MinutiESecondi(datiAudio.MinutaggioRegistrazione), NomeFile = `${NomeProgetto} ${NomeDoppiaggio} ${DatiDoppiatori[IDUtente].nome} ${strBattuta} ${strA} ${M.Minuti.duecifre()} min ${M.Secondi.toFixed(3)} sec${datiAudio.Registrazione.slice(datiAudio.Registrazione.lastIndexOf("."))}`.replace(/[^a-zA-Z0-9\.]/g, "_");
...
async function TermineCaricamentoClip() {
    if (!CaricamentoInizialeTerminato) {
        ...
        if (ModalitaStreaming) {
            ...
            DisattivaMessaggiAttesa();
            AttivaInterfaccia();
            CambiaVolumeVideoGuida.volume = 1; // mette il volume dell'originale al 100% se la Colonna Internazionale termina prima della fine del video.
...
function appendBuffer(vBuffer, partenzaprimobuffer) {
    ...
    var tmp = audioContext.createBuffer(CanaliAudio, lunghezzaTotale, SampleRate);
    for (var i = 0; i < CanaliAudio; i++) {
        ...
        for (let b = 0; b < totBuffer; b++) {
            ...
            channel.set(vBuffer[b].getChannelData(i).slice(partenzabuffer), partenza);
...
function CreaElementoLineaTemporale(ID, DoveInserirlo, PartenzaRegistrazione, LunghezzaRegistrazione, Stringa, RiferimentoRegistrazione, Posizione, Altezza, imgOndaSonora, DaVisualizzare) {
	...
	if ( ( (DatiAudioRegistrato[RiferimentoRegistrazione].ID_Utente == ID_Utente) && (SolaVisione == false) ) || (SonoCreatoreProgetto) ) {
        ...
        if (DaVisualizzare) {ContenutoELT.style.border = "3px dashed darkgreen"; DatiAudioRegistrato[RiferimentoRegistrazione].alPlay = AJAXSalvaAudioAscoltato; DatiAudioRegistrato[RiferimentoRegistrazione].latenzaEventoAlPlay = {secondi: 2, riduciSeClipNelMinutaggio: false};}
        ...
        switch(StrumentoMouse) {
            case toolStandard:
                ...
                if (SingoloELTDaModificare) {
                    ...
                    StoppaAutomaticamenteAscoltoInSolo(DatiAudioRegistrato[ELTDaSpostare.dataset.RiferimentoRegistrazione]);    // Stoppa l'eventuale ascolto in solo
...
function CreaFinestraOpzioniClip(RiferimentoRegistrazione) {
    ...
    div = CreaElemento('div', ID_Opzioni + 'ContenitoreOpzioniBody', ID_Opzioni); div.className = "panel-body";
        ...
        function CreaSlide(IDRiferimento, Stringa, minSlide, maxSlide, stepSlide, minCasella, maxCasella, stepCasella, labelCasella, FunzioneInputSlide, FunzioneChangeCasella, valoreIniziale, FunzioneOnFocus = () => {}) {
            tr = CreaElemento('tr', ID_Opzioni + 'TabellaOpzioniRiga' + IDRiferimento, Tabella.id);
                ...
                td = CreaElemento('td', tr.id + 'tdOpzioneSlide', tr.id);
                    var slide = CreaElemento('input', tr.id + 'Slide', td.id);
                        ...
                        slide.addEventListener('focus', FunzioneOnFocus);
                td = CreaElemento('td', tr.id + 'tdOpzioneValore', tr.id); td.style.textAlign = "left";
                    var c = CreaElemento('input', tr.id + 'Casella', td.id); c.style.width = "80px";
                        ...
                        c.addEventListener('focus', FunzioneOnFocus);
        ...
        /***********************************************************************/
               ...
               CreaSlide('TaglioFinale',      strTaglioFinale,   taglioclip_diffmin, LunghezzaRegistrazione, 0.01, taglioclip_diffmin, LunghezzaRegistrazione, 0.01, strsecondi, ev_CambiaTaglioFinaleClip,    ev_CambiaTaglioFinaleClip,   datiAudio.taglioFinale,   FunzioneOnFocusPerTagliInizialiEFinali);
                ...
                tr = CreaElemento('tr', ID_Opzioni + 'TabellaOpzioniRigaVarie', Tabella.id);
                    ...
                    td = CreaElemento('td', ID_Opzioni + 'tdOpzioneRiduciRumore', tr.id);
                        const pulRiduciRumore = CreaElemento('div', ID_Opzioni + 'pulRiduciRumore', td.id, "Riduzione rumore (test)"); pulRiduciRumore.className = "btn btn-default"; pulRiduciRumore.dataset.RiferimentoRegistrazione = RiferimentoRegistrazione;
                        pulRiduciRumore.onclick = async (e) => {
                            const pulsante = e.currentTarget, Numero = pulsante.dataset.RiferimentoRegistrazione, da = DatiAudioRegistrato[Numero], b = da.buffer.getChannelData(0), lunghezzaBuffer = b.length, LimiteSecondiSogliaRumore = da.buffer.sampleRate / DecimiSecondiSogliaRumore;
                            AggiornaModificheClip();
                            SalvaModificheClipAudio(Numero, async () => {
                                ...
                                const profilorumore = b.slice(0, b.indexOf(b.find((a) => {return a > TrattamentoClip.AutoTaglioIniziale.SogliaDB}))), lunghezzaprofilorumore = profilorumore.length;
                                for (let I = 0; I < lunghezzaprofilorumore; I++) {
                                    b[I] -= RiduzioneRumore * profilorumore[I];
                                }
                                ...
                                for (let I = 0; I < lunghezzaBuffer; I++) {
                                    const sogliasuperata = (Math.abs(b[I]) > TrattamentoClip.AutoTaglioIniziale.SogliaDB);
                                    contSogliaNonRaggiunta = ++contSogliaNonRaggiunta * !sogliasuperata;
                                    const condizioneraggiunta = ((sogliasuperata) || (contSogliaNonRaggiunta < LimiteSecondiSogliaRumore));
                                    b[I] *= condizioneraggiunta;
                                }
    
                                const bloburl = URL.createObjectURL(new Blob([audiobufferToWav(b, da.buffer.sampleRate, {float32: true, notrattamento: true})]));
                                ApriFinestra({currentTarget: {dataset: {larghezza: "100px", altezza: "100px", link: "prova_ffmpeg_nuovo.php?N=" + encodeURIComponent(da.NumeroUnivoco) + "&Percorso=" + encodeURIComponent(bloburl)}}});
    
                                function VerificaTermineProcesso() {
                                    CaricaAudio(0, {Registrazione: "FileProvvisori/Audio" + ID_Utente + bloburl.slice(bloburl.lastIndexOf('/') + 1) + "." + formatoQualitaAlta}, 'arraybuffer', (Contenuto) => {audioContext.decodeAudioData(Contenuto).then((buffer) => {da.buffer = buffer; CreaClipAudio(da.numero); console.log("Creato nuovo audio"); pulsante.abilita(true);}).catch((err) => {console.log("Errore nuovo audio", err);})});
                                }
        ...
        if (!datiAudio.buffer) {pulAscolta.abilita(false); pulRiduciRumore.abilita(false); pulAscolta.innerHTML = " <span class='fa fa-spin fa-spinner'></span> " + strCaricamento; CaricaBufferAudio(RiferimentoRegistrazione, () => {if (pulAscolta) {PulAscoltaPosizioneDefault(pulAscolta); pulAscolta.abilita(true); pulRiduciRumore.abilita(true);}});}
                ...
                td = CreaElemento('td', ID_Opzioni + 'CasellaSalva', tr.id); td.className = "text-right";
                    const pulSalva = CreaElemento('a', ID_Opzioni + 'Salva', td.id, "<span class='fa fa-check'></span> " + strSalva); pulSalva.className = "btn btn-success";
                          ...
                          pulSalva.addEventListener('click', function (e) {
                                ...
                                /* Chiude la finestra */
                                OpzioniClip(RiferimentoRegistrazione, false, true);
...
function ScorciatoieTastiera(Tasto) {
    if (['TEXTAREA', 'INPUT', 'SPAN'].includes(document.activeElement.tagName)) {return;}
    ...
    if (!ModalitaStreaming) {
        ...
        AltreFunzioniVisualizzaCopioneWeDub = () => {
            ...
            if (DatiDoppiatori[ID_Utente]) {
                (Array.from(OpzEvidenzia_TestoGuida).find(el => (new RegExp(DatiDoppiatori[ID_Utente].ruolo, 'i').test(el.value) || new RegExp(DatiDoppiatori[ID_Utente].ruolo.slice(0, (DatiDoppiatori[ID_Utente].ruolo + " ").indexOf(' ')), 'i').test(el.value))) || {selected: false}).selected = true 
