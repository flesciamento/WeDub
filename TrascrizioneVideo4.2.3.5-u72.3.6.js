...
function GestisciImmissioneTesto(e) {
    ...
    function MandaA_AttivaSceltaPersonaggio() {
        UltimaPosizioneCursore = InfoCursore;
...
/*		case "[": T.innerHTML = T.innerHTML.slice(0, PosizioneCursore) + VisualizzaMinutaggioAttuale() + "] " + T.innerHTML.slice(PosizioneCursore); T.selectionEnd = PosizioneCursore; setTimeout(() => {T.selectionStart = T.selectionEnd = T.innerHTML.length;}, 100); return 2;
        ...
        case "ArrowUp":   ((NumeroNodoCursore(InfoCursore.T, NodoAttuale) == 0) && (precedentetextarea = TrovaTextareaVicina(T, -1)) && (precedentetextarea.focus())); break;
        case "ArrowDown": ((NumeroNodoCursore(InfoCursore.T, NodoAttuale) == (InfoCursore.T.childNodes.length - 1)) && (textareasuccessiva = TrovaTextareaVicina(T, 1)) && (PosizionaCursore(textareasuccessiva.childNodes[0], 0)) && (textareasuccessiva.focus())); break;
		case "Enter":
            ...
            function MandaACapoEAttivaSceltaPersonaggio() {
                ...
                const NumNodoAttuale = NumeroNodoCursore(T, NodoAttuale);
                NodoAttuale.textContent = RigaCompleta.slice(0, PosizioneCursore);
                NodoAttuale.after(document.createElement('br'));
                T.childNodes[NumNodoAttuale + 1].after(RigaCompleta.slice(PosizioneCursore));
                PosizionaCursore(T.childNodes[NumNodoAttuale + 2], 0);
                MandaA_AttivaSceltaPersonaggio();
            }
...
function NumeroNodoCursore(T, Nodo) {
