...
async function ScriviPersonaggio(T, Personaggio) {
    ...
    const NodoCursore = UltimaPosizioneCursore.nodo, PosizioneCursore = UltimaPosizioneCursore.p, span = NodoCursore.parentElement;

    const RigaCompleta = NodoCursore.textContent, TestoPrimaInvio = RigaCompleta.slice(0, PosizioneCursore), TestoDopoInvio = RigaCompleta.slice(PosizioneCursore) || " ";
    ...
    if (Personaggio) {    
        ...
        s2.textContent = `${Personaggio}: ${TestoDopoInvio}`;
        PosizionaCursore(s2.childNodes[0], 0);
         
    } else {
        const NumNodo = NumeroNodo(span, NodoCursore), br = document.createElement('br');

        if (NumNodo < span.childNodes.length) {
            span.insertBefore(br, span.childNodes[NumNodo + 1]);
        } else {
            span.append(br);
        }
        
        br.insertAdjacentText('afterend', TestoDopoInvio);
