...
function DisabilitaELT_tranne(Numero, Disabilita) {
    const totAudio = DatiAudioRegistrato.length, StylePointerEvents = (Disabilita? "none" : "auto"), StyleOpacity = (1 * !Disabilita), StyleTransition = (Disabilita? "all 0.5s" : "");
    for (let I = 0; I < totAudio; I++) {
        if (I == Numero) {continue;}
        document.getElementById('ELTReg' + I).iStyle({pointerEvents: StylePointerEvents, opacity: StyleOpacity, transition: StyleTransition});
    }
}
