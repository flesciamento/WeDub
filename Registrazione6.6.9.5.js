...
function AggiornaElencoCandidatiRuoliDaAssegnare() {
    ...
    I = 0;
    for(var Doppiatore in DatiDoppiatori) {
        const DoppiatoreConsiderato = DatiDoppiatori[Doppiatore];
        if (DoppiatoreConsiderato.numeroTraccia == RuoliDaAssegnare_NumeroTraccia) {
            pulElenco = CreaElemento('div', 'pulCandidatoRuoliDaAssegnare' + I, divElencoCandidati.id, `<img src='${DoppiatoreConsiderato.FotoProfilo}' height=35 width=35 style='float: left;' /><span name="${DoppiatoreConsiderato.ID}_online"></span> <b>${DoppiatoreConsiderato.Nome}</b>`);
            ...
            pulElenco.dataset.idutente = DoppiatoreConsiderato.ID; pulElenco.onclick = CliccatoCandidatoRuoliDaAssegnare;
            I++;
        }
    }
