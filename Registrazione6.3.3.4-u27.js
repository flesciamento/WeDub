...
function GeneraBufferCI(datiAudio, buffer, FunzioneAlTermine = () => {}) {
    ...
    const b = buffer, SampleRate = b.sampleRate, numCanali = b.numberOfChannels, tempomixBufferPrec = 0.3, lunghezzabytetempomix = SampleRate * tempomixBufferPrec;

    datiAudio.MinutaggioRegistrazione = +InizioVideoGuida + (75 * datiAudio.numspezzoneCI);

    if ((datiAudio_SpezzonePrecedente = TrovaSpezzoneVicino(datiAudio, -1)) && (datiAudio_SpezzonePrecedente.buffer)) {
        const duratabufferspezzoneprec = datiAudio_SpezzonePrecedente.buffer.duration;
        ...
        /* Fade-in ultimo secondo del buffer precedente */
        ...
        datiAudio.MinutaggioRegistrazione -= tempomixBufferPrec;
        /* datiAudio.taglioIniziale = 75 - tempomixBufferPrec; */
    } else {
        datiAudio.buffer = b;
    }
    
    if (TrovaSpezzoneVicino(datiAudio, 1)) {
        /* Fade-out ultimo secondo (se ci sono altri spezzoni dopo) */
        for(c = 0; c < numCanali; c++) {
            bufferCanale = datiAudio.buffer.getChannelData(c);
            for(s = datiAudio.buffer.duration - tempomixBufferPrec; s < SampleRate; s++) {
                bufferCanale[s] *= (lunghezzabytetempomix - s) / lunghezzabytetempomix;
            }
    ...
    /* if (SpezzoneSuccessivo = SpezzoniAudioCI[+datiAudio.numspezzoneCI + 1]) {
        const datiAudioSucc = DatiAudioRegistrato_Registrazione[SpezzoneSuccessivo][0];
        datiAudioSucc.MinutaggioRegistrazione = ((+datiAudio.MinutaggioRegistrazione) + (+datiAudio.Durata)).arrotonda(2);
        console.log("Spezzone CI succ.", SpezzoneSuccessivo, datiAudioSucc.numspezzoneCI, "minutaggio", datiAudioSucc.MinutaggioRegistrazione, new MinutiESecondi(datiAudioSucc.MinutaggioRegistrazione));
    } */
...
function audiobufferToWav(buffer, opt) {
    ...
    return encodeWAV(buffer.getChannelData(opzCanaleRegistrazione.value), format, sampleRate, numChannels, bitDepth);
}
...
function appendBuffer(vBuffer, partenzaprimobuffer) {
    ...
    partenzaprimobuffer = partenzaprimobuffer * SampleRate;
