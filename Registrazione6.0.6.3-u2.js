...
function DisegnaMarcatore(ID_UtenteMarcatore, Minutaggio, Stringa) {
    const Modificabile = (SonoCreatoreProgetto) || (ID_Utente == ID_UtenteMarcatore);

    const Marcatore = CreaElemento('div', 'Marcatore' + (++DisegnaMarcatore.Num), 'Traccia' + DatiDoppiatori[ID_UtenteMarcatore].numeroTraccia); Marcatore.className = 'Marcatore'; Marcatore.dataset.id = 1;
          Marcatore.style.left = (Minutaggio / totDurataVideoGuida * 100) + "%";
            ...
            const Casella = CreaElemento('input', 'input' + Marcatore.id, Marcatore.id); Casella.value = Stringa; Casella.readOnly = !Modificabile; Casella.style.pointerEvents = (Modificabile? "auto" : "none");
                  Casella.onchange = (e) => {const C = e.currentTarget; C.size = C.value.length - 1;};
