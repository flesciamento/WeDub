...
function AutoAdattaElementiInterfaccia() {
    const differenzaAltezzaContenitore = ContenitorePannelloSx.offsetHeight - Monitors.offsetHeight;
    ScorrimentoCursore.distanzaMinimaDalBordo = (window.innerWidth / ScorrimentoCursore.frazioneLineaTemporale) | 0;
    if ((differenzaAltezzaContenitore > 0) && (pulSchermoIntero.dataset.schermointero == "no")) {AltezzaTracce = ContenitorePannelloSx.offsetHeight; VariaAltezzaTracce();} else {PosizionaElementiScroll(); AdattaDimensionePulsanti();}
}
/***************************************************************************************************************************************************************************/
...
function VariaAltezzaTracce() {
    ...
    ContenitoreLineaTemporale.style.top = `calc(${percAltezzaTracce} + ${(ComandiPlayer.offsetHeight - 15)}px)`;
