    ...
    AJAX("ElencoCandidatiRuoliDaAssegnare.php", "N=" + encodeURIComponent(N), 
        function (Dati) {
            ...
            for (var I = 0; I < totCandidati; I++) {
                ...
                pulElenco.setAttribute('name', 'pulCandidatoRuoliDaAssegnare'); pulElenco.className = 'btn btn-default alert'; pulElenco.setAttribute('title', "-- " + Dati[I].Nome + " -- "); pulElenco.iStyle({position: "relative", left: "0px", top: "0px", width: "100%", whiteSpace: "normal", opacity: 1, transition: "All 1s"});
...
function Posizionati(MinutaggioNuovo, RiabilitaTuttaLaSchermata = false) {
    ...
    function RiattivaVideoGuida() {
        ...
        if (stavoRiproducendo) {
            ...
            if (GestisciCopione.CopioneVisualizzato) {GestisciCopione.RiattivaCopione(); AttivaScorrimentoCopione();}
        }
    }
...
function AttivaScorrimentoCopione() {
    if (RiproduzioneInCorso) {GestisciCopione.AttivaTestoGuida();} else {GestisciCopione.AggiornaTestoGuida();}
}

/*** Gestione del copione interattivo ***/
const GestisciCopione = {
    AggiornaTestoGuida: function() {
        GestisciCopione.InviaComando({tipo: 'aggiorna'});
    },

    AttivaTestoGuida: function() {
        GestisciCopione.InviaComando({tipo: 'attivatesto'});
    },

    DisattivaTestoGuida: function() {
        GestisciCopione.InviaComando({tipo: 'disattivatesto'});
    },

    DisattivaCopione: function() {
        GestisciCopione.InviaComando({tipo: 'disattiva'});
    },

    RiattivaCopione: function() {
        GestisciCopione.InviaComando({tipo: 'riattiva'});
    },

    OpacitaCopioneDisattivato: function(opacita) {
        GestisciCopione.InviaComando({tipo: 'opacitasedisattivato', valore: opacita});
    },

    InviaComando: function(Comando) {
        if (GestisciCopione.FinestraSeparata) {
            InviaPostMessage(GestisciCopione.FinestraSeparata, Comando);
        } else {
            switch(Comando.tipo) {
                case 'aggiornatesto': FunzioniCopione.AggiornaTestoGuida(); break;
                case 'attivatesto': FunzioniCopione.AttivaTestoGuida(); break;
                case 'disattivatesto': FunzioniCopione.DisattivaTestoGuida(); break;
                case 'disattiva': FunzioniCopione.DisattivaCopione(); break;
                case 'riattiva': FunzioniCopione.RiattivaCopione(); break;
                case 'opacitasedisattivato': Funzionicopione.OpacitaCopioneDisattivato = Comando.valore; break;
            }
        }
    },

    FinestraSeparata: false
};
/****************************************/
...
function DisabilitaSchermata(ImmagineAttesa) {
    ...
    GestisciCopione.DisattivaCopione();
...
function RiabilitaSchermata(Aggiorna) {
	...
	EliminaImgAttesa();
    GestisciCopione.RiattivaCopione();
...
function startRecording() {
	...
	StopVideoGuida();
    GestisciCopione.OpacitaCopioneDisattivato(1); DisabilitaSchermata(true); pulPlay.style.opacity = 0;
...
function FermaRegistrazione() {
    ...
    GestisciCopione.OpacitaCopioneDisattivato(0.6);
...
function PlayVideoGuida() {
    ...
    if (RiproduzioneInCorso == false) {
        ...
        if (GestisciCopione.CopioneVisualizzato) {GestisciCopione.AttivaTestoGuida();}
        if (StoRegistrando == false) {
...
function StopVideoGuida() {
	if (RiproduzioneInCorso) {
        ...
        ScaricaMemoria(true);
        GestisciCopione.DisattivaTestoGuida();
...
async function TermineCaricamentoClip() {
    ...
    if (ModalitaStreaming) {
        ...
        let PosizioneInizialeCursore = InizioVideoGuida, datiAudioUtente;
        if (DoppiatoreConsiderato && (datiAudioUtente = DatiAudioRegistrato_Utente[DoppiatoreConsiderato])) {
        ...
        AttivaInterfaccia(PosizioneInizialeCursore);
...
function SwitchCopioneEditabile(e) {
    ...
    GestisciCopione.CopioneEditabile = 1 * (GestisciCopione.CopioneEditabile == 0);
    pulSwitchCopioneEditabile.className = pulSwitchCopioneEditabile.className.replace(classe_pulsante[1 - GestisciCopione.CopioneEditabile], classe_pulsante[GestisciCopione.CopioneEditabile]);
    const s = TestoGuida.querySelectorAll("[name='ContenutoEditabile']"), tots = s.length;
    if (GestisciCopione.CopioneEditabile) {
...
function VideoGuidaPronto() {
    ...
    if (!ModalitaStreaming) {
        ...
        GestisciCopione.FunzionePosizionaVideo = Posizionati;
