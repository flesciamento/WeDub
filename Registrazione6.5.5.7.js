...
function SeguiCursore() {
    const percZoom = (DimensioneRighello / slideZoom.max), CondizioneMinima = (percZoom > ScorrimentoCursore.percMinima), PosizioneAttualeCursore = Cursore.offsetLeft, PosizioneAttualeCursoreNellaFinestra = PosizioneAttualeCursore - window.scrollX;
    ((PosizioneAttualeCursoreNellaFinestra < 0) || (PosizioneAttualeCursoreNellaFinestra > ScorrimentoCursore.distanzaMinimaDalBordo)) && window.scrollTo(PosizioneAttualeCursore - (ScorrimentoCursore.distanzaMinimaDalBordo / ((percZoom * CondizioneMinima) + (ScorrimentoCursore.percMinima * !CondizioneMinima))), WindowScrollY);
}
