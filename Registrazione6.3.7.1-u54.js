                            ...
                            SalvaModificheClipAudio(Numero, async () => {
                                ...
                                function VerificaTermineProcesso() {
                                    AJAX("TrattamentoAudio_VerificaFile.php", "NomeFile=" + encodeURIComponent(nomefile), (Dati) => {
                                        if (Dati.Esiste) {
                                            CaricaAudio(0, {Registrazione: Dati.PercorsoCompleto}, 'arraybuffer', (Contenuto) => {audioContext.decodeAudioData(Contenuto).then((buffer) => {da.buffer = buffer; CreaClipAudio(da.numero); console.log("Creato nuovo audio"); pulsante.abilita(true);}).catch((err) => {console.log("Errore nuovo audio", err);})});
                                        } else {
