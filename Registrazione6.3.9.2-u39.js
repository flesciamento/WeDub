...
function AttivaProgramma() {
    ...
    if (!ModalitaStreaming) {
        ...
        setTimeout(() => {
            ...
            if ((RuoliDaAssegnare_NumeroTraccia !== false) && (SonoCreatoreProgetto)) {AdattaAltezzaTracciaRuoliDaAssegnare();}
            if (!SonoCreatoreProgetto && !SessioneOspite) {AltezzaTracce = ((window.innerHeight) - (window.innerHeight * 0.4)) - NomiTracce.children[0].offsetHeight - 100; VariaAltezzaTracce();} // Ingrandisce lo spazio dedicato al video per i doppiatori
...
function AdattaAltezzaTracciaRuoliDaAssegnare() {
    VariaAltezzaTraccia(RuoliDaAssegnare_NumeroTraccia, ((+document.getElementById('divContenutoNomeTraccia' + RuoliDaAssegnare_NumeroTraccia).offsetHeight) + 25));
    SpostaInBassoTracceSuccessive();
}
/******************************************/
