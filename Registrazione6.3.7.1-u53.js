                    ...
                    td = CreaElemento('td', ID_Opzioni + 'tdOpzioneRiduciRumore', tr.id);
                        ...
                        pulRiduciRumore.onclick = async (e) => {
                            ...
                            SalvaModificheClipAudio(Numero, async () => {
                                ...
                                ApriFinestra({currentTarget: {dataset: {larghezza: "100px", altezza: "100px", link: "prova_ffmpeg_nuovo.php?N=" + encodeURIComponent(da.NumeroUnivoco) + "&Percorso=" + encodeURIComponent(bloburl) + "&NomeFile=Prova"}}});
    
                                function VerificaTermineProcesso() {
                                    CaricaAudio(0, {Registrazione: "FileProvvisori/Audio" + ID_Utente + "Prova." + formatoQualitaAlta}, 'arraybuffer', (Contenuto) => {audioContext.decodeAudioData(Contenuto).then((buffer) => {da.buffer = buffer; CreaClipAudio(da.numero); console.log("Creato nuovo audio"); pulsante.abilita(true);}).catch((err) => {console.log("Errore nuovo audio", err);})});
                                }
