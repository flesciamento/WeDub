...
function GestisciImmissioneTesto(e) {
	...
	switch (e.key) {
		...
		case "[": T.value = T.value.slice(0, PosizioneCursore) + VisualizzaMinutaggioAttuale() + "] " + T.value.slice(PosizioneCursore); T.selectionEnd = PosizioneCursore; setTimeout(() => {T.selectionEnd = T.value.length;}, 100); return 2;
...
function MettiFocusTextareaCorrispondente() {
    ...
    for (let I = 0; I < totTextarea; I++) {
        ...
        if ((T.dataset.minutaggio <= MinutaggioCorrente) && (MinutaggioCorrente < textareasuccessiva.dataset.minutaggio) && (document.activeElement != T)) {T.focus(); T.selectionStart = T.selectionEnd = T.value.length; return;}
    }
