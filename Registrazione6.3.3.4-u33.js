...
function GeneraBufferCI(datiAudio, buffer, FunzioneAlTermine = () => {}) {
    ...
    const b = buffer, SampleRate = b.sampleRate, numCanali = b.numberOfChannels, tempomixBufferPrec = 1, lunghezzabytetempomix = SampleRate * tempomixBufferPrec;
    ...
    if ((datiAudio_SpezzonePrecedente = TrovaSpezzoneVicino(datiAudio, -1)) && (datiAudio_SpezzonePrecedente.buffer)) {
        ...
        for(c = 0; c < numCanali; c++) {
            ...
            for(s = 0; s <= lunghezzabytetempomix; s++) {
                bufferCanale[s] *= s / (lunghezzabytetempomix * 4);
            }
        }
    ...
    if (TrovaSpezzoneVicino(datiAudio, 1)) {
        ...
        for(c = 0; c < numCanali; c++) {
            ...
            for(s = lunghezzabytetempomix; s > 0; s--) {
                bufferCanale[LunghezzaBuffer - s - 1] *= s / (lunghezzabytetempomix / 2);
            }
        }
    }
