...
function CI_AttivaAudioOriginale() {
    if (ColonnaInternazionaleAttivata && RiproduzioneInCorso) {
        ...
        FadeInVolumeAudioOriginale(Volume);
    }
}
...
function FadeInVolumeAudioOriginale(Volume) {
    ...
    async function FadeIn() {
        I = (+I) + (+Incremento);
        VideoGuidaImpostaVolume(I / 100);
        await pausa(10);
        if (I < VolumeFinale) {FadeIn();}
    }
...
function CreaFinestraOpzioniClip(RiferimentoRegistrazione) {
    ...
    div = CreaElemento('div', ID_Opzioni + 'ContenitoreOpzioniBody', ID_Opzioni); div.className = "panel-body";
        ...
        if (!datiAudio.buffer) {pulAscolta.abilita(false); (!pulRiduciRumore.dataset.riduzionerumoreapplicata && pulRiduciRumore.abilita(false)); pulAscolta.innerHTML = " <span class='fa fa-spin fa-spinner'></span> " + strCaricamento; CaricaBufferAudio(RiferimentoRegistrazione, () => {if (pulAscolta) {PulAscoltaPosizioneDefault(pulAscolta); pulAscolta.abilita(true); datiAudio.nonscaricarebuffer = true; (!pulRiduciRumore.dataset.riduzionerumoreapplicata && pulRiduciRumore.abilita(true));}});}
