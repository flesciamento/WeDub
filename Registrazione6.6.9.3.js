...
async function TermineCaricamentoClip() {
    ...
    if (ModalitaStreaming) {
        ...
        if (RuoliDaAssegnare_NumeroTraccia !== false) { // La traccia "RuoliDaAssegnare" è presente solo se si è il creatore del progetto oppure l'utente visitatore
            if (SonoCreatoreProgetto) {tmrAggiornaNotificheCandidatiRuoliDaAssegnare = setInterval(VerificaNuoveClipAltriCandidati, 180000); VerificaNuoveClipAltriCandidati(PosizionatiSuPrimaClipDaAscoltare);} else {SolaVisione = true; pulRegistra.disabled = true; SelezionaCandidatoRuoliDaAssegnare(ID_Utente); AttivaInterfaccia(PosizioneInizialeCursore);}
        } else {
