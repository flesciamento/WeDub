...
async function Posizionati(MinutaggioNuovo, RiabilitaLaSchermata = true) {
    ...
    async function RiattivaVideoGuida() {
        ...
        await pausa(500);
        if ((VideoGuidaMinutaggioCorrente() | 0) != (MinutaggioNuovo | 0)) {setTimeout(() => {Posizionati(MinutaggioNuovo + 0.01, RiabilitaLaSchermata || (Righello.dataset.DisattivaClick == "no"));}, 1000); return;} // Se si incanta prova a riposizionarsi leggermente più avanti, riabilita la schermata se previsto o se era stata riabilitata in precedenza.
...
function EscludiRipristinaTraccia(IDUtente) {
    ...
    if (TracceEscluse.includes(IDUtente)) {
        ...
        if (RiproduzioneInCorso) {
            if (DatiAudioRegistrato_Utente[IDUtente]) {DatiAudioRegistrato_Utente[IDUtente].forEach(StoppaClipAudio);}
        }
    }
}
...
function CestinaClip(Numero, SenzaAnimazione = false) {
    ...
    if (ELT) {ELT.className = "ELT transizione-morbida-" + (SenzaAnimazione? "no" : "si"); ELT.offsetWidth; ELT.iStyle({left: "-5px", width: "10px", visibility: "hidden"}); ELT.className = "ELT transizione-morbida-si";}
...
function DisattivaSpostamentoManualeClip() {
    ...
    VisualizzazioneGraficaTaglioClip(ELTDaSpostare.dataset.RiferimentoRegistrazione);
}
