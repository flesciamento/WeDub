                    ...
                    const tdTrattamentoAudio = CreaElemento('td', ID_Opzioni + 'tdTrattamentoAudio', tr.id);
                        const fieldsetTrattamentoAudio = CreaElemento('fieldset', tdTrattamentoAudio.id + "fieldset", tdTrattamentoAudio.id);
                            ...
                            pulSpezzaBattute.onclick = async (e) => {
                                divVetro.iStyle({display: "inline", opacity: 0});
                                const pulsante = e.currentTarget, Numero = pulsante.dataset.RiferimentoRegistrazione, da = DatiAudioRegistrato[Numero], b = da.buffer.getChannelData(0), lunghezzaBufferConsiderata = da.taglioFinale * SampleRate, SampleRate = da.buffer.sampleRate, SogliaAttacco = SpezzaBattute.SogliaAttaccoDB, LimiteSecondiSogliaAttacco = SpezzaBattute.SecondiSogliaAttacco * SampleRate, LimiteSecondiSogliaTermine = SpezzaBattute.SecondiSogliaTermine * SampleRate;
                                ...
                                if (NumeroAttacchi > 2) {
                                    ...
                                    for(A = 2; A < NumeroAttacchi; A += 2) {
                                        DuplicaClip(Numero, (ClipNuova) => {ClipNuova.taglioIniziale = attacchi[A]; ClipNuova.taglioFinale = attacchi[+A + 1]; SelezionaESpostaELT(ClipNuova.numero);});
                                    }
                                    divVetro.style.display = "none";
                                }
                            }
