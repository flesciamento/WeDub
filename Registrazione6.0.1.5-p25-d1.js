...
function PrecaricaClip(DalMinutaggio, FinoAlMinutaggio, FunzioneAlTermine = false, AltreOpzioni = {FunzioneAlTermineSingoloPrecaricamento: () => {}, SoloBuffer: false}) {
    ...
    if ((totClipDaPrecaricare == 0) && (FunzioneAlTerminePrecaricamento)) {FunzioneAlTerminePrecaricamento();} else {console.log("Settato Timeout fra ", (SecondiPrecaricamentoMax / 2), "sec. Precaricamento clip dal secondo", FinoAlMinutaggio); setTimeout(() => {MinutaggioPrecaricamentoClip = FinoAlMinutaggio; PrecaricaClipSuccessiva(); console.log("Timeout avviato.", MinutaggioPrecaricamentoClip);}, (SecondiPrecaricamentoMax / 2) * 1000);}
}
