...
function DisabilitaElementiGenerali(Disabilita) {
    ...
    document.getElementById('fieldsetContenitoreStrumenti').abilita(!Disabilita);
...
function VisualizzazioneGraficaTaglioClip(Numero, CropOnda = !VisualizzaOndaSonoraClipCompleta.MantieniAttivo) {
    const datiAudio = DatiAudioRegistrato[Numero], StratoColore = document.getElementById('ELTReg' + datiAudio.numero + 'StratoColore'), OndaSonora = document.getElementById('ELTReg' + datiAudio.numero + 'OndaSonora'), LunghezzaRegistrazione = datiAudio.Durata;
    ...
    if (StratoColore) {
        ...
        if (CropOnda) {OndaSonora.style.clipPath = "inset(0% " + StratoColore.style.right + " 0% " + StratoColore.style.left + ")";} else {OndaSonora.style.clipPath = "none";}
    }
}
/*********************************************************/
...
function DividiClip_puntatorerilasciato(e) {
    ...
    if ((+ELT.dataset.posizionePuntatoreInizioTaglio | 0) == (+ClientX | 0)) {DividiClip(ELT.dataset.RiferimentoRegistrazione, secondiInizioTaglio, secondiFineTaglio); return;}
    /************************************************************************************************/
...
function DividiClip(Numero, secondiInizioTaglio, secondiFineTaglio) {
     AJAX("DividiClip-prova.php", "N=" + DatiAudioRegistrato[Numero].NumeroUnivoco + "&InizioTaglio=" + encodeURIComponent(secondiInizioTaglio) + "&FineTaglio=" + encodeURIComponent(secondiFineTaglio),
        function (Dati) {
            AggiornaClip(() => {
                if (!Dati.SoloTaglioInizialeFinale) {
                    ...
                    ClipNuova.buffer = DatiAudioRegistrato[Numero].buffer;
                    OperazioniAlBufferCaricato(ClipNuova.numero);
                    setTimeout(PosizioneOriginaleClipSovrapposte, 300);
                }
        ...
        }, strDivisioneClip, (((secondiInizioTaglio | 0) == (secondiFineTaglio | 0)) ? strClipDivisa : strSegmentoTagliato), true
    );
}
...
function VisualizzaClipAudio(Numero, Attivo) {
    ...
    document.getElementById(id_ELT + "Contenuto").style.opacity = (Attivo? 1 : 0.2);
	...
	if ( ( (DatiAudioRegistrato[RiferimentoRegistrazione].ID_Utente == ID_Utente) && (SolaVisione == false) ) || (SonoCreatoreProgetto)) {
        ...
        ELT.addEventListener('mouseenter',
            function (e) {
                ...
                VisualizzaOndaSonoraClipCompleta(ELTAttuale, true);

                tmrRiposizionamentoAutomaticoELT = setTimeout(() => {RiposizionamentoAutomaticoClipSovrapposte(ELTAttuale);}, 500);
            }
        );

        ELT.addEventListener('mouseleave', (e) => {clearTimeout(tmrRiposizionamentoAutomaticoELT); VisualizzaOndaSonoraClipCompleta(e.currentTarget, VisualizzaOndaSonoraClipCompleta.MantieniAttivo);});
...
function GestioneEventoELTCliccato (e) {
    ...
    const ELT = e.currentTarget.parentNode.parentNode, NumeroAudio = +ELT.dataset.RiferimentoRegistrazione, datiAudio = DatiAudioRegistrato[NumeroAudio], X = e.clientX || e.touches[0].clientX;
...
function VisualizzaOndaSonoraClipCompleta(ELT, Visualizza = VisualizzaOndaSonoraClipCompleta.MantieniAttivo) {
    VisualizzazioneGraficaTaglioClip(ELT.dataset.RiferimentoRegistrazione, !Visualizza);
}
VisualizzaOndaSonoraClipCompleta.MantieniAttivo = false;
...
function AnteprimaSelezioneClip(ELTConsiderato, ClientX, FunzioneTouchEnd = () => {}) {
    ...
    ELT.style.zIndex = 500000000; VisualizzaOndaSonoraClipCompleta.MantieniAttivo = true;
    ...
    AnteprimaSelezioneClip.Elimina = () => {EliminaElemento(div); VisualizzaOndaSonoraClipCompleta.MantieniAttivo = false; ELT.style.zIndex = ""; clearTimeout(AnteprimaSelezioneClip.tmr);};
}
...
function RiposizionamentoAutomaticoClipSovrapposte(ELTAttuale) {
    if (ELTDaSpostare == false) {
        const IDTraccia = ELTAttuale.parentNode.id, ELTStratoColore = document.getElementById(ELTAttuale.id + 'StratoColore'), leftColoreELTAttuale = (+ELTAttuale.offsetLeft) + (+ELTStratoColore.offsetLeft) + 5, widthColoreELTAttuale = ELTStratoColore.offsetWidth - 5, TermineELTAttuale = (+leftColoreELTAttuale) + (+widthColoreELTAttuale);
        ...
        var ELTConsiderato, leftColoreELTConsiderato = 0, NuoviELTDaRiordinare = [];
        for (var I = 0; I < totELT; I++) {
            ELTConsiderato = ELTTimeline[I];
            
            if (ELTConsiderato.style.display != "none") {
                ELTConsideratoStratoColore = document.getElementById(ELTConsiderato.id + 'StratoColore');
                leftColoreELTConsiderato = (+ELTConsiderato.offsetLeft) + (+ELTConsideratoStratoColore.offsetLeft);
                if ((leftColoreELTConsiderato < TermineELTAttuale) && (leftColoreELTAttuale < ((+leftColoreELTConsiderato) + (+ELTConsideratoStratoColore.offsetWidth)))) {
...
function OpzioniClip(Numero, Apri, SalvaAllaChiusura) {
    ...
    if (Apri) {
        ...
        VisualizzaOndaSonoraClipCompleta.MantieniAttivo = true;
        ...
        VisualizzaOndaSonoraClipCompleta(ELT, false);
        RiabilitaSchermata();
